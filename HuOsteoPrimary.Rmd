...
rFeaturePlot <- function(object, features, pt.size = 1, order = T, ...) {
...
for(s in geo1$s) {
  ...
  raw[[s]] <- x %>%
    ...
    RunUMAP(dims = 1:20)
  p[[s]] <- rDimPlot(raw[[s]]) +
    ggtitle(str_c(s, " basic clustering"))
}
rm (x)
...
rDimPlot(tumor) +
  ggtitle("Subclustered tumor cells")

rDimPlot(tumor, split.by = "src", ncol = 4) +
  ggtitle("Subclustered tumor cells")

tumor <- kill_cc(tumor)
rDimPlot(tumor, group.by = "Phase") +
  ggtitle("Sublustered tumor cells - cell cycle")
```
...
write.xlsx(tumor_marks, "tumor_marks.xlsx")
