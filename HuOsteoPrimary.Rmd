...
for(i in 1:12) {
  ...
  opt$plot[[i]] <- rDimPlot(tumor, paste("Resolution", opt$res[i]))
  opt$score[i] <- silhouette_mean(silhouette_seurat(tumor))
  opt$clusters[i] <- levels(tumor$seurat_clusters)
}

plot_grid(plotlist = opt$plot, ncol = 3)
