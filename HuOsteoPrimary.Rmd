...
for(i in 1:9) {
  opt[i, "res"] <- 0.1 * i
  tumor <- FindClusters(comb, resolution = opt$res[i])
...
for(i in levels(Idents(tumor))) {
  ...
  # print(paste("Upregulated pathways for cluster", i))
  # print()
  
  enriched_up[[paste("Cluster", i, sep = "_")]] <- enrichr(up, dbs)
  enriched_down[[paste("Cluster", i, sep = "_")]] <- enrichr(down, dbs)
}
```