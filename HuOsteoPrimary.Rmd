...
enriched_down <- list()
for(i in levels(Idents(tumor))) {
  up <- rownames(tumor_marks[tumor_marks$cluster == i & tumor_marks$avg_log2FC > 0.7,])
  down <- tumor_marks[tumor_marks$cluster == i & tumor_marks$avg_log2FC < 0.7,]
  enriched_up[[i]] <- enrichr(up, "MSigDB_Hallmark_2020"))
  enriched_down[[i]] <- enrichr(down, "MSigDB_Hallmark_2020"))
}
```