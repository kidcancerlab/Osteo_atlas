...
for (item in unique(subset_file$unique)){
    ...
    for (i in seq_len(nrow(tmp_df))) {
        ...
        ncount_max <- subset_file$ncount_max[i]
        mt_percent <- subset_file$mt_percent[i]
        ...
        sobj <- subset(sobj,
                    nCount_RNA > 500 &
                    nCount_RNA < subset_file$ncount_max[i] &
                    percent.mt < subset_file$mt_percent[i])
        Hmeta_data <- sobj@meta.data %>%
                as.data.table
        counts <- Hmeta_data[, .N, by = "sample_name"]
        if (counts$N >= 300) {
