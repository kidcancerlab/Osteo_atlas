...
gsea <- parallel::mclapply(seq_len(nrow(tumor_list)), function(item) {
    ...
    for (item in names(degs_result)) {
        logfc_cutoff <- 0.6
        degs_result[[item]]$diffexpressed <- "NO"
        degs_result[[item]]$diffexpressed[degs_result[[item]]$log2FoldChange > logfc_cutoff & degs_result[[item]]$padj < 0.05] <- "UP"
        degs_result[[item]]$diffexpressed[degs_result[[item]]$log2FoldChange < -1 * logfc_cutoff & degs_result[[item]]$padj < 0.05] <- "DOWN"
        degs_result[[item]]$delabel[degs_result[[item]]$diffexpressed != "NO"] <- degs_result[[item]]$gene[degs_result[[item]]$diffexpressed != "NO"]
    ...
    for (i in seq_len(nrow(cat_tib))){
        ...
        for (item in names(gsea_result)) {
            ...
            gsea_plot[[cat_expl]][[item]] <- print(two_way_plot(top5up_down, x_col = "NES"))
        }
    }
