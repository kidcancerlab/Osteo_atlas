...
Run_GRA <- function(sobject,
    ...
    cols <- c(plot_cols, sample(rainbow(1000)))
    colVars <- cols[1:length(unique(cellInfo$CellType))]
    names(colVars) <- unique(cellInfo$CellType)

    #Running SCENIC begins
    if (organism == "human") {
        scenicoptions <- SCENIC::initializeScenic(org = "hgnc",
                                              dbDir = "/gpfs0/home2/gdrobertslab/lab/Analysis/Yogesh/CellTypeAnnRefs/input/downloads/SCENIC/human",
                                              dbs = SCENIC::defaultDbNames[[org]],
                                              datasetTitle = "SCENIC tutorial",
                                              nCores = parallelly::availableCores())   #may not work for ncores
    } else if (organism == "mouse") {
        scenicoptions <- SCENIC::initializeScenic(org = "mgi",
                                              dbDir = "/gpfs0/home2/gdrobertslab/lab/Analysis/Yogesh/CellTypeAnnRefs/input/downloads/SCENIC/mouse",
                                              dbs = SCENIC::defaultDbNames[[org]],
                                              datasetTitle = "SCENIC tutorial",
                                              nCores = parallelly::availableCores())   #may not work for ncores
    }
...
scenic_obj <- Run_GRA(sobject = sobject,
...
plot.new(); legend(0,1, fill=colVars, legend=names(colVars))
