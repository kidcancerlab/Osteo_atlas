...
for (item in seq_len(nrow(subset_file))) {
    sample_name <- subset_file$sample_name[item]
    ncount_max <- subset_file$ncount_max[item]
    mt_percent <- subset_file$mt_percent[item]
    sobj <- qs::qread(str_c("input/saint_jude/",
                           sample_name,
                           ".qs"))
    noqc_list[[sample_name]] <- sobj
    
    qc_list[[sample_name]] <- subset(sobj,
                            nCount_RNA > 1000 &
                            nCount_RNA < sample_details$ncount_max[i] &
                            percent.mt < sample_details$mt_percent[i])
}

noqc_merged <- merge(x = noqc_list[[1]],
                            y = noqc_list[2:length(noqc_list)]) %>%
                process_seurat()

qc_list[[sample_name]] <- subset(noqc_merged[[sample_name]],
                            nCount_RNA > 1000 &
                            nCount_RNA < sample_details$ncount_max[i] &
                            percent.mt < sample_details$mt_percent[i])
qc_merged <- merge(x = qc_list[[1]],
                            y = qc_list[2:length(qc_list)],
                            add.cell.id = names(qc_list)) %>%
                process_seurat()
...
qc_list[[sample_name]] <- subset(sobj_list[[sample_name]],
                              nCount_RNA < sample_details$ncount_max[i] &
                              percent.mt < sample_details$mt_percent[i])


# individual QC numbers
obj <- qs::qread("output/seurat_objects/single_sobj/patient/primary/OS_1human.qs") #nolint
...
todo for monday
1)see how many cells are left in total per group when used cutoff of min ncount of 1000 and max of from the table
2) do this for both our already exisitng samples and all the Saint Judes samples
3) Find out what min ncount cutoffs works the best to keep most of our cells 
4) Get rid of sobj which have less than 500 cells automatically

you can just print out tables with the cell numbers 
