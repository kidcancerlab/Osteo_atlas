...
## TRY Invisible algorithm
```{r}
# Load required libraries and source functions
invisible(lapply(c("dplyr","Seurat","HGNChelper","openxlsx","copykat",
                   "copykatRcpp","ggplot2", "SCEVAN","yaGST","cowplot",
                   "Rcpp", "Rclusterpp", "parallel","biomaRt","logger",
                   "httr", "jsonlite", "readr","future"),
                 library,
                 character.only = !0))
...
run_ensemble <- function(seurat_object, disease = "",
                         known_normal_cells = "",
                         custom_marker="https://raw.githubusercontent.com/kris-nader/TBD/main/sctype_aml_cellmarker20_cosmic.xlsx",   #nolint
                         genome_cp = "hg20",
                         plot = FALSE,
                         organism ="human") {
...
tumor_ensamble <- parallel::mclapply(seq_len(nrow(ensamble_table)), function(item) {
    ...
    object <- qs::qread(str_c("output/seurat_objects/annotated_sobjs/",
                              sobj,
                              ".qs"))
    object <- run_sctype(object, known_tissue_type="Immune system", plot=TRUE)
    norm_cells=get_normal_cells(object, c("Macrophages"))
    if (species == "human") {
        object <- run_ensemble(object,
                            disease="Cancer",
                            known_normal_cells=norm_cells,
                            #genome_cp = "mm10",
                            #organism = "mouse",
                            plot=FALSE)
    } else {
    ...
    plots <- visualize_ensemble_step(object)
    ggsave(str_c("output/plots/", sobj, "_ensamble.pdf"),
            width = 20,
            height = 15,
            plot = plots)
...
DimPlot(sobj, group.by = c("new", "sample_name"))

sobj <- run_sctype(sobj, known_tissue_type="Immune system", plot=TRUE)

norm_cells=get_normal_cells(sobj,c("Macrophages"))

sobj <- run_ensemble(sobj,
                    disease="Cancer",
                    known_normal_cells=norm_cells,
                    genome_cp = "mm10",
                    organism = "mouse",
                    plot=FALSE)
visualize_ensemble_step(sobj)
...
patient_sample=run_sctype(sobj,known_tissue_type="Immune system",plot=TRUE)
...
norm_cells=get_normal_cells(patient_sample,c("Macrophages"))

patient_sample=run_ensemble(patient_sample,disease="AML",known_normal_cells=norm_cells,plot=FALSE, genome_cp = "mm10",
                    organism = "mouse")
...
umaps_group <- 
    tribble(~sobj,            ~species,   ~combine,
            "patient_prim",    "human",    "alone",
            "patient_mets",    "human",    "alone",
            "xeno_prim",       "both",    "comb_prim",
            "xeno_mets",       "both",    "comb_mets",
            "mm_prim",         "mouse",    "alone",
            "mm_mets",         "mouse",    "alone")
...
for (item in seq_len(nrow(umaps_group))) {
    ...
    species <- umaps_group$species[item]
    if (combine == "alone") {
        ...
        plot <- patchwork::wrap_plots(plots) + 
                patchwork::plot_annotation(title = str_c("Umap of ", sobj_name),
                                           theme = theme(plot.title = element_text(hjust = 0.5,
                                                                                   size = 20,
                                                                                   face = "bold")))  #nolint
        ggsave(str_c("output/plots/umaps/", sobj_name, "_umap.pdf"),
            width = 15,
            height = 5,
            plot = plot)
    } else {
...
combined <- qs::qread("output/seurat_objects/harmony_sobjs/mm_prim.qs")

DimPlot(combined, group.by = c("sample_name", "seurat_clusters", "new"), label = T, ncol = 2, repel = T)

patient_mets_tumor <- qs::qread("output/seurat_objects/tumor_vs_stroma/patient_mets_tumor.qs")

DimPlot(patient_mets_tumor, group.by = c("sample_name", "seurat_clusters", "data_type"), label = T, ncol = 2, repel = T)


degs_result <- run_degs(sobject = mm_prim,
                        group_by = "seurat_clusters",
                        prefix = "patient_prim_tumor")
...
patient_mets <- qs::qread("output/seurat_objects/harmony_sobjs/patient_mets.qs")

DimPlot(patient_mets, group.by = c("sample_name", "seurat_clusters", "new", "data_type"), label = T, ncol = 2)
...
for(c in names(clusters)) {
  for(m in de_methods) {
    ...
    ggsave(paste0("Plots/Primary/panel-", m, "-", c, ".pdf"),
      plots[[c]][[m]]$panel,
      device = "pdf",
      width = 18,
      height = 12,
