...
patient_prim_tumor <- qs::qread("output/seurat_objects/tumor_vs_stroma/patient_prim_tumor.qs")

patient_prim_tumor <- optimize_silhouette(patient_prim_tumor)

patient_prim_tumor <- process_seurat(patient_prim_tumor, resolution = 0.1)

DimPlot(patient_prim_tumor, group.by = c("sample_name", "data_type", "seurat_clusters"), label = T, ncol = 3, repel = T, shuffle = T)



degs_result <- run_degs(sobject = patient_prim_tumor,
...
for (item in names(gsea_result)) {
    ...
    plot_name <- print(two_way_plot(top5up_down, x_col = "NES"))
...
DimPlot(patient_mets, group.by = c("sample_name", "seurat_clusters", "new"), label = T, shuffle = T)
...
## Stromal analysis
```{r}
normal_lungs <- qs::qread("input/downloads/normal_lung.qs")

normal_lungs$free_annotation <-
    str_replace_all(normal_lungs$free_annotation, c("/" = "_",
                                                  "\\+" = "_plus"))

normal_lungs$free_annotation <-
    stringr::str_replace_all(normal_lungs$free_annotation,
                              c("Alveolar Epithelial Type 1" = "Alveolar Epithelial",  #nolint
                                "Alveolar Epithelial Type 2" = "Alveolar Epithelial",  #nolint
                                "Basophil_Mast 1" = "Basophil_Mast",
                                "Basophil_Mast 2" = "Basophil_Mast",
...
#looks like there is no mitochondrial genes in the normal cells
#So need to remove the mitochondrial reads from the tumor associated too
for (item in rownames(normal_lungs)) {
  if (stringr::str_detect(item, "^CYTB") == TRUE) {
      print(item)
  }
}

for (item in rownames(normal_lungs)) {
  if (stringr::str_detect(item, "^RP[SL][[:digit:]]") == TRUE) {
      print(item)
  }
}

normal_lungs <- normal_lungs %>% process_seurat()

#set the idents
normal_lungs$sample_name <- normal_lungs$channel

normal_lungs$new <- normal_lungs$free_annotation
...
FeaturePlot(tumor_associated, features = abc)
FeaturePlot(normal_lungs, features = abc)
...
normal_lungs@meta.data$type <- "normal"
...
looks like all the of th resilts forom this analysis seem to have the same pathways upregulated. Something is wrong. Maybe the genee MT. 

Also, what is shuffle? RYan told me about?
Shuffle = TRUE in dimplot
