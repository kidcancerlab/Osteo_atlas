...
# sj_dir_list <- list.files(path = "/gpfs0/home2/gdrobertslab/lab/ExternalData/Patel_lab/",
#                       full.names = FALSE)

# NCH_dir_list <- list.files(path = "/gpfs0/home2/gdrobertslab/lab/Counts_2/",
#                            full.names = FALSE)

# dir_list <- c(sj_dir_list, NCH_dir_list)
...
for (i in seq_len(nrow(all_samples_csv))) {
    ...
    tumor_type <- all_samples_csv$tumor_type[i]
    ...
    if (file.exists(str_c(dir, "/", sobject))) {
        ...
        if (!is.null(path_h5)) {
                    ...
                    #                                          violin_plot = FALSE)
            object <- Read10X_h5(filename = path_h5)
...
obj <- qs::qread("/gpfs0/home2/gdrobertslab/lab/Counts_2/") #nolint
...
write code to count the numner of cells before filtration and after filtration QC
...
## Create metadata for Saint Jude
```{r}
all_samples_csv <- read_csv("allsample_details.csv")
...
for (item in seq_len(nrow(subset_file))) {
    sample_name <- subset_file$sample_name[item]
    species <- subset_file$species[item]
    tumor_type <- subset_file$tumor_type[item]
    data_source <- subset_file$data_source[item]
    sp_pattern <- subset_file$sp_pattern[item]
    organism <- subset_file$organism[item]
...
sobj <- qs::qread("output/seurat_objects/comb_sobjs/patient_mets.qs")
DimPlot(sobj)
...
marsh_liger <- normalize(marsh_liger) %>%
    ...
    rliger::runUMAP(distance = 'cosine',
            n_neighbors = 30,
            min_dist = 0.3)


Run harmony integration on this:::
look for immune cells markers

all.plots <- plotByDatasetAndCluster(marsh_liger, axis.labels = c('UMAP 1', 'UMAP 2'), return.plots = T)
all.plots[[1]] + all.plots[[2]]

gene_loadings <- plotGeneLoadings(marsh_liger, do.spec.plot = FALSE, return.plots = TRUE)
gene_loadings[[4]]
...
IFIT3 <- plotGene(marsh_liger, "MKI67", axis.labels = c('UMAP 1', 'UMAP 2'), return.plots = TRUE)
IFITM3 <- plotGene(marsh_liger, "SATB2", axis.labels = c('UMAP 1', 'UMAP 2'), return.plots = TRUE)
plot_grid(IFIT3[[1]],IFIT3[[2]],IFIT3[[3]],IFITM3[[1]],IFITM3[[2]],IFIT3[[3]], ncol=2)
...
parallel::mclapply(unique(sample_details$unique), function(item) {
    tmp_df <- subset(sample_details, subset = (unique == item))
    temp_list <- list()
    for (i in seq_len(nrow(tmp_df))) {
        species <- tmp_df$species[i]
...
liger_obj <- ScaleData(merged_sobj, split.by = "orig.ident", do.center = FALSE)

liger_obj <- SeuratWrappers::RunOptimizeALS(liger_obj, k = 20, lambda = 5, split.by = "orig.ident")
