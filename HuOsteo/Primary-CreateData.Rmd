...
plot_cols <- c("#D43F3AFF", "#EEA236FF", "#357EBDFF", "#5CB85CFF",
  "#B8B8B8FF",  "#3E7E3EFF", "#68A4E3FF", "#00f7ff", "#69D1F3FF",
  "#9632B8FF",  "#46B8DAFF", "#90302DFF", "#A66D04FF", "#2D577FFF",
  "#7D7D7DFF", "#6D1D87FF", "#07768f", "#FF6E6AFF", "#FFBB70FF",
  "#79D379FF", "#CDCDCDFF", "#BF6BE2FF", "#a5a314", "#0e171a",
  "#ff0000", "#d81d64", "#8400ff", "#69f369", "#00f7ff",
  "#69D1F3FF", "#ffd000", "#69f3c5")
...
geo1 <- tibble(
  s = c("BC5", "BC6", "BC11", "BC16", "BC20", "BC21", "BC22"),
  qc = c(18000, 25000, 30000, 70000,
         70000, 50000, 50000),
  path = c("Conventional", "Conventional", "Conventional",
           "Conventional", "Chondroblastic", "Intraosseous", "Chondroblastic"),
  type = c("Primary", "Primary", "Primary", "Primary",
...
for(s in names(raw)) {
  prim[[s]] <- subset(raw[[s]], cells = sample(Cells(raw[[s]]), 2000))
}

# Combine the downsampled datasets into a single seurat object and re-normalize
prim_comb <- merge(prim[[1]], y = prim[2:length(prim)],
...
#save before harmony
qs::qsave(prim_comb, "Seurat_objects/Primary/prim_comb.qs")
```

## Run Harmony
```{r}
# Run batch-correction/integration using harmony
prim_harmony_comb <- RunHarmony(prim_comb, group.by.vars = "src")
prim_harmony_comb <- prim_harmony_comb %>%
...
for(i in 1:9) {
  opt[i, "res"] <- 0.1 * i
  prim_harmony_comb <- FindClusters(prim_harmony_comb, resolution = opt$res[i])
  plot[[i]] <- rDimPlot(prim_harmony_comb, paste("Resolution", opt$res[i]))
  opt[i, "score"] <- silhouette_mean(silhouette_seurat(prim_harmony_comb))
  opt[i, "clusters"] <- length(levels(prim_harmony_comb$seurat_clusters))
}

plot_grid(plotlist = plot, ncol = 3)
tumor <- FindClusters(prim_harmony_comb,
...
qs::qsave(prim_harmony_comb, "Seurat_objects/Primary/prim_harmony_comb.qs")
```
