...
tumor <- subset(qs::qread("Seurat_objects/Primary/prim_harmony_comb.qs"),
                cells = selection)
...
#colnames(Embeddings(tumor, reduction = "umap"))[1] <- "UMAP_1"

for(i in 1:9) {
  opt[i, "res"] <- 0.1 * i
  tumor <- FindClusters(tumor, resolution = opt$res[i])
  plot[[i]] <- DimPlot(tumor)        #paste("Resolution", opt$res[i]))
...
tumor$psudobulk_col <- paste0(tumor$src, "-C", tumor$seurat_clusters)

pseudobulk <- AggregateExpression(tumor,
                                  group.by = "psudobulk_col",
                                  slot     = "counts")[[1]]
...
Gives the differentially expressed genes
```{r}
...
qs::qsave(tumor, "Seurat_objects/Primary/osteoshark.qs")

ggsave("Plots/Primary/UMAP-new-labels.pdf",
  plots$assignment,
  device = "pdf",
  width = 6,
  height = 5,
...
ggsave("Plots/Primary/UMAP-count-density.pdf",
  plots$density,
  device = "pdf",
  width = 6,
  height = 5,
...
ggsave("Plots/Primary/UMAP-cell-cycle.pdf",
  plots$cell_cycle,
  device = "pdf",
  width = 6,
  height = 5,
