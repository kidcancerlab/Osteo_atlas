...
for (item in seq_len(nrow(combine_tumor_per_species_new))) {
    ...
    object <- 
        qs::qread(str_c("output/seurat_objects/final_combined_sobjs/",
                        comb_group,
                        ".qs")) %>%
        FindClusters(resolution = res_value, verbose = FALSE)
...
for (names in c("patient_prim_cancer_cells",
    ...
    dotplot <-
        DotPlot(object,
                features = c("FGFR3", "FGFR1", "UNC5B", "CD109", "EPHA3", "MXRA8", "UNC5C", "FLRT2"),
                scale = F,
                group.by = "Ann_Level3")
    for (features in c("FGFR3", "FGFR1", "UNC5B", "CD109", "EPHA3", "MXRA8", "UNC5C", "FLRT2")) {
        featureplot <- 
            FeaturePlot(object,
                        features = features,
                        raster = F,
...
features <- c("UNC5C", "UNC5B")
vln_list <-
    list()
dot_list <-
    list()
for (names in c("patient_prim_mets",
                "xeno_prim_mets",
                "mouse_prim_mets",
                "dog_prim_mets")) {
    object <- 
...
# violin plots for UNC5C and UNC5B across the groups
panel_Vln <-
    wrap_plots(vln_list$patient_prim_mets, 
                vln_list$xeno_prim_mets,
                vln_list$mouse_prim_mets,
                vln_list$dog_prim_mets,
                ncol = 1) +
    plot_annotation(title = "Violin plots of UNC5C and UNC5B",
                    theme = theme(plot.title = element_text(hjust = 0.5, size = 16)))
ggsave("output/figures/tumor_only/UNC5C_UNC5B_violin_plots.png",
        plot = panel_Vln,
        width = 15,
        height = 15,
        bg = "white")
...
panel_dot <- 
    wrap_plots(dot_list$patient_prim_mets, 
                dot_list$xeno_prim_mets,
                dot_list$mouse_prim_mets,
                dot_list$dog_prim_mets,
                ncol = 1) +
    plot_annotation(title = "Dotplots of UNC5C and UNC5B",
                    theme = theme(plot.title = element_text(hjust = 0.5, size = 16)))

ggsave("output/figures/tumor_only/UNC5C_UNC5B_dotplots.pdf",
        plot = panel_dot,
        width = 7,
        height = 15,
        bg = "white")
...
vioin_boxplot_data <- 
    FetchData(object = patient_xeno_prim_mets,
                vars = c("FGFR3", "FGFR1", "UNC5B", "CD109", "EPHA3", "MXRA8", "UNC5C", "FLRT2",
                        "Ann_Level3", "unique")) %>%
    pivot_longer(cols = c("FGFR3", "FGFR1", "UNC5B", "CD109", "EPHA3", "MXRA8", "UNC5C", "FLRT2"),
                names_to = "gene",
                values_to = "expression")
