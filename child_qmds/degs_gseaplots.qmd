...
Idents(prim_mets) <- prim_mets$Ann_Level3
genes <-
    FindAllMarkers(prim_mets,
                    only.pos = TRUE,
                    min.pct = 0.25,
...
markers <-
    genes %>%
    filter(cluster == "Tumor_Progenitor") %>%
...
prim_patient_xeno <-
    qs::qread(str_c("output/seurat_objects/final_combined_sobjs/",
                    "prim_patient_xeno",
                    ".qs"))
dimplot_better(prim_patient_xeno, group_by = "Ann_Level3")

pprim <-
    subset(prim_patient_xeno,
            unique == "patient_prim")
dimplot_better(pprim, group_by = "Ann_Level3")

xeno_prim <-
    subset(prim_patient_xeno,
            unique == "xeno_prim_human")

dimplot_better(xeno_prim, group_by = "seurat_clusters")
dimplot_better(xeno_prim, group_by = "Ann_Level3")

VlnPlot(xeno_prim, features = "BNIP3", group.by = "seurat_clusters")
...
dimplot_better(mets_patient_xeno, group_by = "seurat_clusters")

dimplot_better(mets_patient_xeno, group_by = "Ann_Level3")

VlnPlot(mets_patient_xeno, features = "CD36", group.by = "seurat_clusters", pt.size = 0)
pmets <-
    subset(mets_patient_xeno,
            unique == "patient_mets")
dimplot_better(pmets, group_by = "Ann_Level3")
dimplot_better(pmets, group_by = "seurat_clusters")
xeno_mets <-
    subset(mets_patient_xeno,
            unique == "xeno_mets_human")

make_panel_plot(sobj = xeno_mets,
                    comparison_col = "seurat_clusters",
                    label = "seurat_clusters",
                    group = "xeno_mets_human",
                    aggregate_by = c("sample_name", "seurat_clusters"),
                    organism_col = "organism",
                    subset = FALSE,
                    batch_var = "data_source")

dimplot_better(xeno_mets, group_by = "Ann_Level3")
dimplot_better(xeno_mets, group_by = "seurat_clusters")

VlnPlot(xeno_mets, features = "CD36", group.by = "seurat_clusters", pt.size = 0)
```
