...
make_panel_plot(sobj = object1,
                comparison_col = "seurat_clusters",
                label = "seurat_clusters",
                group = "obj1_mm_prim_mets_cancer_cells",
                aggregate_by = c("sample_name", "seurat_clusters"),
                organism_col = "organism",
                subset = FALSE,
                batch_var = "data_source")
...
DotPlot(object1, 
        features = c("Bnip3", "Mki67", "Egln3", "Vegfa", "Ifit3", "Ifit1"))

FeaturePlot(object1, 
            features = c("Egln3", "Bnip3", "Vegfa", "Hif1a"), order = TRUE)
...
for (item in seq_len(nrow(combine_tumor_per_species_new))) {
    ...
    object <- 
        ...
        FindClusters(resolution = res_value)
    #dimplot_better(object, group_by = "seurat_clusters") +NoLegend()
    #DotPlot(object, group.by = "seurat_clusters", features = c("Bnip3","Egln3","Hif1a","Vegfa","Hilda"))
    make_panel_plot(sobj = object,
                    comparison_col = "seurat_clusters",
                    label = "seurat_clusters",
                    group = comb_group,
...
dimplot_better(obj_list$patient_mets_cancer_cells, group_by = "Ann_Level3")
...
cowplot::plot_grid(
    DotPlot(object = obj_list$patient_prim_cancer_cells,
                    features = fibrogenic_goldstandard_strong,
                    group.by = "Ann_Level3",
                    cols = "RdBu") + RotatedAxis(),
    DotPlot(object = obj_list$patient_mets_cancer_cells,
                    features =  fibrogenic_goldstandard_strong,
                    group.by = "Ann_Level3",
                    cols = "RdBu") + RotatedAxis(),
    ncol = 1
...
fibrogenic <-
    list(mouse = c(Bicc1, Setbp1, Antxr1, Lama4))
...
cowplot::plot_grid(
    DotPlot(object = obj_list$dogs_prim_cancer_cells,
            features = Basal_Progenitor,
            group.by = "Ann_Level3",
            cols = "RdBu") + RotatedAxis(),
    DotPlot(object = obj_list$dogs_mets_cancer_cells,
            features = Basal_Progenitor,
            group.by = "Ann_Level3",
            cols = "RdBu") + RotatedAxis(),
    ncol = 1
...
basal_p <-
    list(mouse = c("Pten", "Bmp4", "Aqp5", "Crip2"))
...
cowplot::plot_grid(
    DotPlot(object = obj_list$xeno_prim_human_cancer_cells,
            features = stress,
            group.by = "Ann_Level3",
            cols = "RdBu") + RotatedAxis(),
    DotPlot(object = obj_list$xeno_mets_human_cancer_cells,
            features = stress,
            group.by = "Ann_Level3",
            cols = "RdBu") + RotatedAxis(),
    ncol = 1
...
cowplot::plot_grid(
    DotPlot(object = obj_list$xeno_prim_human_cancer_cells,
            features = metabolic_oxphos_genes,
            group.by = "Ann_Level3",
            cols = "RdBu") + RotatedAxis(),
    DotPlot(object = obj_list$xeno_mets_human_cancer_cells,
            features = metabolic_oxphos_genes,
...
metabolically_primed_RP_genes1 <- 
    metabolically_primed_RP_genes %>%
    nichenetr::convert_human_to_mouse_symbols() %>%
    as.character()
...
synthetic <- c("MT-ND1", "MT-ND2", "MT-ATP6", "MT-ND3")
...
metabolic_progenitor_markers <- c(
  # Mitochondrial OXPHOS genes
  "MT-ND1", "MT-ND2", "MT-ND3", "MT-ND4",
  "MT-CO1", "MT-CO2", "MT-CO3",
  "MT-ATP6", "ATP5MC3", "ATP5MF",
  "NDUFA3", "NDUFB3", "NDUFB4", "UQCR11",
  "TIMM13", "TIMM8B", "PRDX2",
  
  # Ribosomal proteins (small and large subunits)
  "RPS2", "RPS3", "RPS6", "RPS10", "RPS12", "RPS13", "RPS14", "RPS15A",
  "RPS18", "RPS19", "RPS21", "RPS23", "RPS24", "RPS27", "RPS27A", "RPS28", "RPS29", "RPS4X",
  "RPL8", "RPL10A", "RPL11", "RPL12", "RPL13", "RPL15", "RPL19", "RPL22", "RPL23A",
  "RPL26", "RPL30", "RPL32", "RPL35", "RPL35A", "RPL36", "RPL37", "RPL37A",
  "RPL39", "RPL41", "RPLP1", "RPLP2",
  
  # Metallothioneins
  "MT1E", "MT1F", "MT1G", "MT1M", "MT1X", "MT2A",
  
  # Other metabolic-related genes
  "IFITM5", "PRDX2"
) %>%
unique()

# Combine all ribosomal genes from both lists (genes starting with RPL, RPS, MRPL, MRPS)
metabolic_oxphos_genes <- c(
  # OXPHOS and mitochondrial genes
  "NDUFAF8", "NDUFA2", "CYC1", "UQCRQ", "UQCRB", "COX5B", "MT-ND1", "MT-ND2", "MT-ND3",
  "MT-CO1", "MT-CO2", "MT-CYB", "MT-ATP6", "ATP5MC3", "SLC25A5", "STOML2",
  # Mitochondrial ribosomal proteins
  "MRPL12", "MRPL36", "MRPL51", "MRPS34",
  # Cytosolic ribosomal proteins
  "RPS2", "RPS10", "RPL13A", "RPS28", "RPS21", "RPL36", "RPS15", "RPL35", "RPL11", "RPL8",
  "RPL13", "RPL18", "RPLP1", "RPL22", "RPL14", "RPS7", "RPL26", "RPL29", "RPL34", "RPL41",
  "RPL27", "RPLP2", "RPS27", "RPS13",
  # Other metabolic genes
  "SCD", "MYC", "SLC1A5", "PRELID1", "FTH1", "PRDX2"
)
ribosomal_genes <- unique(grep("^RPL|^RPS|^MRPL|^MRPS", all_genes, value = TRUE))
ribosomal_genes
