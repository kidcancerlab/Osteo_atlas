...
for (i in seq_len(nrow(object_list))) {
    ...
    if (group == "mm_prim_cancer_cells") {
        object$Ann_Level3 <-
            str_replace_all(object$seurat_clusters,
                          c("0" = "Actively_Cycling",
                            "1" = "Metabolic",
                            "2" = "Interractive",
                            "3" = "Secretory",
                            "4" = "Inflammatory",
...
# Example data: concentrations and corresponding fold change responses
conc <- c(0, 0.01, 0.1, 1, 10, 100)
response <- c(4.318400041, 0.867375468, 0.831270818, 0.867279015, 0.844729305)

# Fit a four-parameter logistic (4PL) model
dose_response_model <- drm(response ~ conc, fct = LL.4(names = c("Slope", "Lower", "Upper", "IC50")))

# Summarize the model to see the IC50
...
plot(dose_response_model, log = "x", main = "Dose-Response Curve", xlab = "Concentration (µM)", ylab = "Fold Change")
