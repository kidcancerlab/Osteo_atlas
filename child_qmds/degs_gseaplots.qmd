...
## Markers for the tumor cells using pathway analysis
...
hyper <-
    intersect(HyperMetabolic$patient_prim_cancer_cells$gene,         HyperMetabolic$patient_mets_cancer_cells$gene)

fibrogenic_shared <- 
    intersect(Fibrogenic$patient_prim_cancer_cells$gene,                   Fibrogenic$patient_mets_cancer_cells$gene)

progenitor_shared <-
    intersect(Progenitor$patient_prim_cancer_cells$gene,                 Progenitor$patient_mets_cancer_cells$gene)

prolif <-
    intersect(Proliferative$patient_prim_cancer_cells$gene,             Proliferative$patient_mets_cancer_cells$gene)

inter <-
    intersect(Interactive$patient_prim_cancer_cells$gene,              Interactive$patient_mets_cancer_cells$gene)

groun <-
    intersect(Ground$patient_prim_cancer_cells$gene,                   Ground$patient_mets_cancer_cells$gene)


VlnPlot(obj_list$patient_prim_cancer_cells,
        features = groun,
        group.by = "Ann_Level3",
        pt.size = 0)


VlnPlot(obj_list$patient_mets_cancer_cells,
        features = groun,
        group.by = "Ann_Level3",
        pt.size = 0)


HyperMetabolic_markers <- c("MT1X", "SLC22A9", "SLC35F1")

fibrogenic_markers <- c("COL4A1", "ITGBL1", "COL4A2", "CCDC102B", "COL8A1", "TFPI", "POSTN")

progenitor_markers <- c("CD36", "ROBO2", "FAT3", "PLCB4", "TMTC2")

prolif_markers <- c("MKI67", "CENPE", "TOP2A", "GTSE1", "NUSAP1", "PRC1", "CENPF", "DIAPH3")

interactive_markers <- c("LAPTM5", "PTPRC", "RBM47", "PTPRE", "SRGN", "TYMP", "CTSB")

ground_markers <- c("CNTN5", "HAPLN1", "PRR16", "ID4")
...
objct_i <-
    qs::qread(str_c("output/seurat_objects/final_combined_sobjs/",
                    "harmony_human_xeno_cancer_cells.qs"))
...
DimPlot(objct_i,
    ...
    NoLegend()
objct_i <-
    FindClusters(objct_i,
                 resolution = 0.1)
dimplot_better(objct_i, group_by = "seurat_clusters")
 
patient_prim <-
    subset(objct_i,
           unique == "patient_prim")
dimplot_better(patient_prim, group_by = "Ann_Level3")
 
patient_mets <-
    subset(objct_i,
           unique == "patient_mets")
dimplot_better(patient_mets, group_by = "Ann_Level3")
...
harm_mets <-
    ...
    process_seurat(reduction = "harmony",
                    resolution = 0.2)
dimplot_better(harm_mets, group_by = "seurat_clusters")
...
harm_mets <-
    ...
    RunHarmony(group.by.vars = c("sample_name",
                                 ...
                                 "data_source"),
               theta = c(7, 12, 12),
               lambda = c(0.5, 0.1, 0.1)) %>%
    process_seurat(reduction = "harmony",
                    resolution = 0.2)
dimplot_better(harm_mets, group_by = "seurat_clusters")

harm_mets <-
    FindClusters(harm_mets,
                 resolution = 0.3)

dimplot_better(patient_me, group_by = "Ann_Level3")
dimplot_better(patient_me, group_by = "seurat_clusters")

oatient<-
    FindClusters(patient_me,
                 resolution = 0.25)
dimplot_better(oatient, group_by = "seurat_clusters")
```