...
for (item in seq_len(nrow(combine_tumor_per_species_new))) {
    ...
    object <- 
        ...
        FindClusters(resolution = res_value)
    #dimplot_better(object, group_by = "seurat_clusters") +NoLegend()
    # DotPlot(object, group.by = "seurat_clusters", features = "ENSCAFG00805016104")
    make_panel_plot(sobj = object,
                    comparison_col = "seurat_clusters",
                    label = "seurat_clusters",
                    group = comb_group,
...
FeaturePlot(object,
            features = c("RPL37A", "RPS18", "RPL39", "RPL35"),
            reduction = "umap",
            ncol = 2,
            order=T)
...
# human and xeno
markers <- list(
  Fibrogenic = c("COL3A1", "COL6A1", "COL6A2", "COL6A3", "POSTN", "LUM"),
  Synthetic  = c("RPL37A", "RPS18", "RPL39", "RPL35"),
  Progenitor = c("CD36", "COL24A1", "PTPRZ1", "PDGFD", "ANO5", "INSC"),
  Proliferative = c("MKI67", "ASPM", "TOP2A", "CENPE", "CENPF", "KIF4A"),
  Stressed = c("BNIP3", "NDRG1", "EGLN3", "HILPDA", "ANGPTL4", "VEGFA"),
  Interactive = c("PTPRC", "CD74", "MSR1", "CD163", "LAPTM5", "DOCK8")
)

#mouse
markers <- list(
    Proliferative = c("Mki67", "Ccnb1", "Ccna2", "Ube2c", "Aurkb"),
    Synthetic = c("Rpl29", "Rplp1", "Rps3", "Eif3g", "Eif4a1"),
    Progenitor = c("Adamts6", "Rictor", "Ambra1", "Akt3", "Ptprg"),
    Stressed = c("Pten", "Csf1r", "Ifi202b", "Clca3a1", "Eno3"),
    Fibrogenic = c("Aspn", "Col11a1", "Mgp", "Adamts9", "Lama4"),
    Interactive = c("Alcam", "Lcp1", "Tyrobp", "Ctss", "Fcer1g")
)

# dog
markers <- list(
    "Synthetic" = c("RPL29", "EIF3G", "ATP5PF", "LAMTOR2", "RPL11"),
    "Progenitor" = c("ST6GALNAC3", "INSC", "ESRRG", "LEPR", "DLG2"),
    "Proliferative" = c("KIF4A", "CENPF", "NCAPG", "ANLN", "ASPM"),
    "Stressed" = c("MMP9", "TYROBP", "CD84", "FGF10", "ZC3H12B"),
    "Interactive" = c("PTPRC", "LCP1", "SYK", "CD74", "HLA-DRB1"),
    "Fibrogenic" = c("COL11A2", "CD36", "CHST11", "COL24A1", "VIT")
)
...
for (names in unique(object$unique)) {
    ...
    markers_list[[names]] <- markers_to_save

    dimplot <-
        dimplot_better(object_subset,
                    group_by = "Ann_Level3") +
            NoLegend() +
            ggtitle(str_c("Annotation for ", names))
    dimplot_list[[names]] <- dimplot
    dim_dot_plot <-
        make_dim_dot_plot(object = object_subset,
                            group_by = "Ann_Level3",
                            title = names,
                            cell_markers = markers,
                            cols = c("#ae0600", "#00a1d6", "#f2c94c", "#27ae60", "#9b51e0", "#eb5757")) 
}
...
patchwork::wrap_plots(dimplot_list,
                    ncol = 2) +
    patchwork::plot_annotation(title = "Annotation for Tumor vs Stroma") &
    theme(plot.title = element_text(hjust = 0.5))


# Find shared markers for each phenotype in xeno_prim_human and xeno_mets_human
intersect(markers_list$xeno_prim_human$Interactive,
         markers_list$xeno_mets_human$Interactive)
intersect(markers_list$xeno_prim_human$Progenitor,
         markers_list$xeno_mets_human$Progenitor)
intersect(markers_list$xeno_prim_human$Proliferative,
         markers_list$xeno_mets_human$Proliferative)
intersect(markers_list$xeno_prim_human$Fibrogenic,
         markers_list$xeno_mets_human$Fibrogenic)
intersect(markers_list$xeno_prim_human$Synthetic,
         markers_list$xeno_mets_human$Synthetic)
intersect(markers_list$xeno_prim_human$Stressed,
         markers_list$xeno_mets_human$Stressed)


shared_markers <- lapply(
    c("interactive", "progenitor", "proliferative", "fibrogenic", "synthetic", "stressed"),
    function(type) intersect(markers_list$xeno_prim_human[[type]], markers_list$xeno_mets_human[[type]])
)
names(shared_markers) <- c("interactive", "progenitor", "proliferative", "fibrogenic", "synthetic", "stressed")
shared_markers
...
for (item in seq_len(nrow(combine_tumor_per_species))) {
    comb_group <- combine_tumor_per_species$combined[item]
    res_value <- combine_tumor_per_species$res_value[item]
    prim <- combine_tumor_per_species$prim[item]
    mets <- combine_tumor_per_species$mets[item]
    ...
    object$ann_lvl3 <-
        str_replace_all(object$seurat_clusters,
                        Annotation_list[[comb_group]])
    object$ann_lvl2 <- object$ann_lvl3
    object$ann_lvl1 <- "Tumor"
    object$ann_lvl0 <- "Tumor"
    qs::qsave(object,
            str_c("output/seurat_objects/final_tumor_vs_stroma/",
                    comb_group,
                    "_cancer_cells.qs"))

    #dimplot_better(object, group_by = "ann_lvl2")
    object1 <- 
    ...
    object1 <-
        object@meta.data %>%
        filter(rownames(.) %in% colnames(object1)) %>%
        select(ann_lvl3) %>%
        AddMetaData(object1, metadata = .)
    # dimplot_better(object1, group_by = "ann_lvl3")
    ...
    object11 <-
        subset(object,
            unique == prim) %>%
        process_seurat() %>%
        RunHarmony(group.by.vars = harmony_params[[prim]]$group_by,
                    theta = harmony_params[[prim]]$theta,
                    lambda = harmony_params[[prim]]$lambda) %>%
        process_seurat(reduction = "harmony")
    ...
    object2 <-
        object@meta.data %>%
        filter(rownames(.) %in% colnames(object2)) %>%
        select(ann_lvl3) %>%
        AddMetaData(object2, metadata = .)
    # dimplot_better(object2, group_by = "ann_lvl3")
    # dimplot_better(object2, group_by = "seurat_clusters")
    qs::qsave(object2,
            str_c("output/seurat_objects/final_tumor_vs_stroma/",
                    mets,
    ...
    object22 <-
        subset(object,
            unique == mets) %>%
        process_seurat() %>%
        RunHarmony(group.by.vars = harmony_params[[mets]]$group_by,
                    theta = harmony_params[[mets]]$theta,
                    lambda = harmony_params[[mets]]$lambda) %>%
        process_seurat(reduction = "harmony")
    # dimplot_better(object2, group_by = "ann_lvl3")
    # dimplot_better(object22, group_by = "seurat_clusters")

}



# Find the markers for the combined patient prim and mets
for (item in seq_len(nrow(combine_tumor_per_species))) {
    comb_group <- combine_tumor_per_species$combined[item]
    res_value <- combine_tumor_per_species$res_value[item]

    object <- 
        qs::qread(str_c(str_c("output/seurat_objects/final_tumor_vs_stroma/",
                    comb_group,
                    "_cancer_cells.qs")))
    ...
    markers <- 
        FindAllMarkers(object = object,
                        ...
                        logfc.threshold = 0.25,
                        only.pos = T)
    # DotPlot(object, group.by = "seurat_clusters", features = apoptosis_markers)
    object <- FindClusters(object, resolution = 0.35)
    dimplot_better(object, group_by = "seurat_clusters") +
        NoLegend()
    # FeaturePlot(object, features = features , order = TRUE)
    ...
    # save the dotplot
    width <- length(unique(object$ann_lvl3)) * 2
    height <- length(unique(object$ann_lvl3)) *2
    ...
    directory <-
        str_c("output/figures/tumor_vs_stroma/", comb_group)
    if (!dir.exists(directory)) {
        dir.create(directory, recursive = TRUE)
    }
...
subset_xeno_prim$Ann_Level2 <- subset_xeno_prim$Ann_Level3
subset_xeno_prim$Ann_Level1 <- "Tumor" # since all are tumors
subset_xeno_prim$Ann_Level0 <- "Tumor" # since all are tumors
...
subset_xeno_mets$Ann_Level2 <- subset_xeno_mets$Ann_Level3
subset_xeno_mets$Ann_Level1 <- "Tumor" # since all are tumors
subset_xeno_mets$Ann_Level0 <- "Tumor" # since all are tumors
...
# save the dotplot
width <- length(unique(object$seurat_clusters)) * 1.5
height <- length(unique(object$seurat_clusters)) * 2
