...
dog <-
    list(Fibrogenic = c("F13A1", "COL11A2", "CHST11",  "COL24A1"),
         Progenitor = c("ESRRG", "PTPRD", "ST6GALNAC3", "SRGAP3"),
         Proliferative =c("CENPF", "ANLN", "KIF4A", "NCAPG"),
         Interactive = c("ALCAM", "PTPRC", "CD74", "CSF1R"),
         Stressed =c("IGFBP7", "MAP3K5", "ROR1", "VEGFA"),
         Synthetic = c("RPL29.1", "RPL26", "RPL14", "RPL27A")
    )
...
```{r human_xeno_prim_mets_analysis}
## combine patient prim with xeno prim and patient mets with xeno mets
...
dimplot_better(object, group_by = "Ann_Level3", reduction = "fdl") +
    NoLegend() + ggtitle("Tumor FDL")
...
for (group in c(
    ...
    for (item in seq_len(nrow(percent_diff))) {
        ...
        if (length(genes_per_group) > 0) {
            if (tumor_sub == "Synthetic") {
                Synthetic[[group]] <- genes_per_group
            }
            if (tumor_sub == "Fibrogenic") {
                Fibrogenic[[group]] <- genes_per_group
            }
            if (tumor_sub == "Progenitor") {
                Progenitor[[group]] <- genes_per_group
            }
            if (tumor_sub == "Proliferative") {
                Proliferative[[group]] <- genes_per_group
            }
            if (tumor_sub == "Interactive") {
                Interactive[[group]] <- genes_per_group
            }
            if (tumor_sub == "Stressed") {
                Stressed[[group]] <- genes_per_group
            }
        }
    }
}
...
specific <-
    list(
    human_specific = c("COL6A3", "COL3A1", "COL6A1", "FBN1", "COL4A1", "COL4A2", #firbo
...
human_xeno_shared <- list(
    Fibrogenic = c("COL6A3", "COL6A2", "PLXDC2", "NFKBIZ", "PDGFRB", "PMEPA1", "CACNA1C", "MSC-AS1"),
    Progenitor = c("ANO5", "CD36", "ROBO2", "ANGPT1", "FHOD3", "PLCB4", "FAT3", "COL24A1", "SLC8A3", "AC105941.1", "DTNA", "SLC44A5", "PDGFD", "PTPRZ1", "CACNB4", "PCDH7", "PATJ", "SATB2"),
    Proliferative = c("TOP2A", "ASPM", "CENPF", "TPX2", "CENPE"),
    Stressed = c("EGLN3", "SOX9", "NDUFA4L2", "VEGFA", "BNIP3", "NDRG1", "SLC2A1", "ADM", "HILPDA", "HIF1A-AS3")
    )

patient <-
    list(Synthetic = c("RPS28", "RPL36", "RPS13", "RPS27"),
        Fibrogenic = c("COL3A1", "COL6A3", "POSTN","FBN1"),
        Progenitor = c("CD36", "ROBO2", "SATB2", "ANO5"),
        Proliferative = c("TOP2A", "ASPM", "CENPF", "MKI67"),
        Interactive = c("CSF1R", "PTPRC", "PTPRJ", "CD74"),
        Stressed = c("EGLN3", "VEGFA", "BNIP3", "HILPDA")) %>%
    unlist() %>%
    as.character() %>%
    unique()
...
mouse <-
    list(Synthetic = c("Rps2", "Rpl10", "Rpl26", "Rpl21"),
        Fibrogenic = c("Col3a1","Pdgfrb", "Bicc1", "Antxr1"),
        Progenitor = c("Pdgfc", "Zfp462", "Ahr", "Ptbp2"),
        Proliferative = c("Ccna2", "Cenpf", "Ube2c", "Cenpe"),
        Interactive = c("Alcam", "Ptprc", "Ptprj", "Cd74"),
        Stressed = c("Hspb7", "Eno3", "Ccng1", "Bnip3")) %>%
unlist() %>%
as.character() %>%
unique()
...
dog <-
    list(Fibrogenic = c("F13A1", "COL11A2", "CHST11",  "COL24A1"),
         ...
         Interactive = c("ALCAM", "PTPRC", "CD74", "CSF1R"),
         Stressed =c("IGFBP7", "MAP3K5", "ROR1", "VEGFA"),
         Synthetic = c("RPL26", "RPS28", "RPS13", "RPS27"))%>%
unlist() %>%
as.character() %>%
unique()
...
xenograft <-
    list(Synthetic = c("RPL26", "RPL8","RPL35", "RPS13"),
        Fibrogenic = c("COL3A1", "COL6A1", "POSTN", "COL6A3"),
        Progenitor = c("PTPRZ1", "ROBO2", "CD36", "PDGFD"),
        Proliferative = c("TOP2A", "ASPM", "CENPF", "MKI67"),
...
stress <-
   Reduce(intersect, list(
                        ...
                        #    Stressed$xeno_prim_human,
                        #    Stressed$xeno_mets_human,
