...
cluster6_topdegs <- c(
    "IL1B", "MS4A7", "SAMSN1", "CD163", "CD69",
    "FPR3", "MRC1", "MS4A6A", "CD14", "CCL4", "SLC16A10"
)

FeaturePlot(cluster6_sobj,
            features = cluster6_topdegs,
            pt.size = 0.3,
            cols = c("#EEEEEE", "#ae0600"),
            min.cutoff = 0.15,
            order = TRUE) +
    coord_fixed() +
    ggtitle(str_c("cluster6 top degs"))
...
DotPlot(object1,
    ...
    ggtitle(str_c("cluster6 reclustered top degs"))
...
dimplot_better(object1,
        group_by = "new_annot_clust") +
    NoLegend()
table(cluster6_sobj$annotations, cluster6_sobj$seurat_clusters)
```
