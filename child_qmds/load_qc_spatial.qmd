...
qs::qsave(rctd_res, "ouput/spacexr/granular_references/all_granular_results.qs")
```
...
### For Different Granularity References

```{r}
for (ref in names(ref_list)) {
    # make output directory
    dir.create(paste0("output/figures/spatial/spacexr/granular_", ref))
    
    # read in unannotated object list
    spatial_list <- qs::qread("output/spatial_objects/unannotated_spatial_list.qs")

    # read in rctd results
    rctd_list <- qs::qread(
        paste0("output/spacexr/granular_references/", ref, "_level3_rctd.qs")
    )
    
    # need to add names
    names(rctd_list) <- paste0("OS", c(1:8), "_Seurat")
    
    # loop over each sample now
    for (ob in names(spatial_list)) {
        # add rctd results to object
        norm_weights <- spacexr::normalize_weights(rctd_list[[ob]]@results$weight)
        sp_ob <- AddMetaData(spatial_list[[ob]], norm_weights)
        # get cell types from reference object
        ref_cell_types
        ann_mat <- spatial_list[[ob]]@meta.data[ , ref_cell_types]
        pdf(
            paste0(
                "output/figures/spatial/spacexr/granular_references/",
                ob,"_heatmap.pdf"),
            width = 900,
            height = 900
        )
