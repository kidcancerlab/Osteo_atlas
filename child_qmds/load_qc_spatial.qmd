...
tmp <- spatial_list[[2]]
SpatialFeaturePlot(tmp,
                   features = c("Tumor_Metabolic",
                                "Tumor_Inflammatory",
                                "Tumor_Proliferative",
                                "Tumor_Ground",
                                "Tumor_ImmuneRegulatory"), pt.size.factor = 1500) |
SpatialDimPlot(spatial_list[[8]], pt.size.factor = 1300)

tmp_out <- list()
for (ob in names(spatial_list)) {
    tmp <- spatial_list[[ob]]
    tmp$Tumor_Cummulative <- tmp$Tumor_Metabolic +
    ...
    tmp_out[[ob]] <- grid.arrange(grobs = plot_list, nrow = 1)
}
...
Composition.cluster.k(deconv_mat, 20)

set.seed(17)

tmp <- Composition.cluster(tmp, deconv_mat, 9)
...
tmp$CompositionCluster_CC <- tmp$seurat_clusters
