...
# time to pick the optimal models
opt_mods <- lapply(all_ldas, optimalModel, opt)
```
...
pdf("output/STdeconvolve/figures/kneed_K_correlation.pdf")
for (ob in names(spatial_list)) {
...
pdf("output/STdeconvolve/figures/min_K_correlation.pdf")
for (ob in names(spatial_list)) {
    s_obj <- spatial_list[[ob]]
    cor_plt <- make_cor_plt(min_lda[[ob]], s_obj)
    ...
    # get beta and theta from model
    beta_theta <-
        getBetaTheta(min_lda[[ob]], perc.filt = 0.05, betaScale = 1000)
    ...
    print(SpatialDimPlot(s_obj, pt.size.factor = 0))
}
dev.off()
```
...
# get fibrogenic module
fib_mod <- myeloid_modules[["Fibrogenic_TAMs"]]
# subset to genes present in objects
length(fib_mod)
...
as.list(setNames(names(myeloid_modules), paste0("myeloid", 1:13)))

SpatialFeaturePlot(
    spatial_list[[1]],
    features = paste0("myeloid", 1:13),
    pt.size.factor = size.factors[[1]],
    ncol = 7
)


# visualize 
fib_tam_plots <- lapply(
    names(spatial_list),
    function(x) {
        print(
            SpatialFeaturePlot(
                spatial_list[[x]],
                features = c("Fibrogenic_TAMs1", "Tumor_Cumulative"),
                pt.size.factor = size.factors[[x]]
            )
        )
    }
)
gridExtra::grid.arrange(grobs = fib_tam_plots)
```
