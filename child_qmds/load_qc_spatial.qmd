...
for (ob_name in names(spatial_list)) {
    ...
    rctd_multi <- run_rctd(sp_ob, ref = level2_ref, doublet_mode = "multi")
    qs::qsave(rctd_out, paste0("output/spacexr/Ann_Level2/rctd_level2_", ob_name, "_multi.qs"))
...
spacexr_res <- parallel::mclapply(names(spatial_list), function(ob_name) {
    ...
    rctd_multi <- run_rctd(sp_ob, ref = level3_ref, doublet_mode = "multi")
    qs::qsave(rctd_out, paste0("output/spacexr/Ann_Level3/rctd_level3_", ob_name, "_multi.qs"))
    norm_weights <- spacexr::normalize_weights(rctd_out@results$weights)
