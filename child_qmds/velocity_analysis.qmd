...
lapply(names(obj_list), function(ob_name) {
    ob <- obj_list[[ob_name]]
    if (grepl("xenograft", unique(ob$organism))) {
        species <- "mixed"
    } else {
...
### Re-Do it for Xenografts

Earlier my function couldn't handle PDX's since the only options for species were mouse or human, and the xenografts needed the mixed reference gtf since the chromosomes and genes have the species appended to them.

```{r}
pdxs <- obj_list[which(grepl("xeno", x = names(obj_list)))]

lapply(names(pdxs), function(ob_name) {
    ob <- obj_list[[ob_name]]
    r_make_loom_files(sobj = ob,
                      id_col = "sample_name",
                      out_dir = "loom_output",
