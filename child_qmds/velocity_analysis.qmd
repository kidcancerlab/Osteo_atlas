...
qs::qsave("output/seurat_objects/obj_list_03_03_2025.qs")
```
...
def write_ob_and_plots(ob):
    ...
    calc_velo(merged_ad)
    scv.tl.velocity_graph(merged_ad, backend = "threading", n_jobs = 30)
    merged_ad.write(filename = "loom_output/anndata/" + ob + ".ad")
    ...
    return(obj_name + " is done")


for ob in ["xeno_mets_mouse", "xeno_mets_human", "xeno_prim_mouse", "xeno_prim_human"]:
...
```{r}
tmp <- obj_list[["patient_mets_cancer_cells"]]
tmp_velo_genes <-
    read.table("loom_output/velocity_genes/patient_mets_cancer_cells.csv",
               sep = ",",
               header = TRUE)
...
for (ann in colnames(tmp_velo_genes)) {
    tmp <- AddModuleScore(tmp,
                          features = tmp_velo_genes[, ann],
                          name = ann)
}

FeaturePlot(tmp,
            features = paste0(colnames(tmp_velo_genes), 1:6),
            raster = FALSE,
            cols = rev(brewer.pal(n = 11, name = "RdBu"))) | r_dim_plot(tmp, group.by = "Ann_Level3", raster = FALSE)
```