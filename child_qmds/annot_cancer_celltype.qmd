...
for (group in c("patient_prim", "patient_mets",
    ...
    object <-
        qs::qread(str_c("output/seurat_objects/harmony_sobjs_annot/",
                        group,
                        ".qs"))
    #dimplot_better(object, group_by = c("annotations")) + NoLegend()
    # species and markers
    ...
    #                     name = "osteo_markers_module_score",)
    # object[["osteo_markers_module_score"]] <- object[[str_c("osteo_markers_module_score", "1")]]
    # object[[str_c("osteo_markers_module_score", "1")]] <- NULL

    #  bulk_ms_score_per_celltype <- 
    #     object@meta.data %>%
    #     dplyr::select(osteo_markers_module_score, new_annot_clust) %>%
    #     dplyr::group_by(new_annot_clust) %>%
    #     dplyr::summarise(median = median(osteo_markers_module_score, na.rm = TRUE)) %>%
...
threshold <-
    list(patient_prim = c(0.4, 0.2), #maybe 0.3 for the clusterbased annot
         patient_mets = c(0.5, 0.48),
         xeno_prim_human = 0.3,
         xeno_mets_human = 0.3,
         xeno_prim_mouse = 0.3,
         xeno_mets_mouse = 0.3,
...
for (group in c("patient_prim", "patient_mets",
    ...
    # mislabelled_cell_ids_normal <-
    #     read_tsv(list.files(path = str_c("misc/mislabelled_ids/", group),
    #                         pattern = "Epithelial_Endothelial|Mesenchymal|Immune_Lymphoid|Immune_Myeloid",
    #                         full.names = TRUE)) %>%
    #     distinct()

    # object$celltype_tumor_call[object$cell %in% mislabelled_cell_ids_normal$cell] <- "normal_cells"

    # mislabelled_cell_ids_tumor <-
    #     read_tsv(list.files(path = str_c("misc/mislabelled_ids/", group),
    #                         pattern = "Tumor",
    #                         full.names = TRUE)) %>%
    #     distinct()

    # object$celltype_tumor_call[object$cell %in% mislabelled_cell_ids_tumor$cell] <- "cancer_cells"
