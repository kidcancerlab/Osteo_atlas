...
for (group in c("patient_prim", "patient_mets", "mm_prim", "mm_mets")) {
    ...
    s_obj <-
        subset(x = object,
                celltype_tumor_call == "cancer_cells")

    s_obj$purified_tumorcall <- 
        if_else(s_obj$annotations %in% annotations_cell_list[[group]],
                "normal_cells",
                "cancer_cells")

    s_obj_new <-
            subset(x = s_obj,
                purified_tumorcall == "cancer_cells")
            # process_seurat() %>%
    ...
    object$purified_tumorcall <-
        if_else(colnames(object) %in% purified,
                "cancer_cells",
                "normal_cells")
    ...
    object@meta.data %>%
        as.data.frame() %>%
        rownames_to_column("cell") %>%
        select(cell, celltype_tumor_call, purified_tumorcall) %>%
        write_tsv(paste0("output/id_tumor/celltype/",
                 group,
                 "_metadata",
                 ".tsv"))
