...
order_TAMs <-
    c("Osteoclast_TAMs", "Prolif_TAMs", "Monocytes", "IFN_TAMs", 
     "Inflam_TAMs", "Scar_TAMs", "Alv_TAMs", "TAMs")
...
for (group in c("patient_prim_normal_cells", "patient_mets_normal_cells",
    ...
    # Filter for features present in the group
    order_TAMs_subset <-
        intersect(order_TAMs, unique(harm_mac_obj_list[[group]]$Ann_Level3))
...
FeaturePlot(harm_mac_obj_list$patient_mets_normal_cells,
            features = macrophage_features_cells$Inflam_TAMs, order = T)
...
## M1 and M2 Macrophages testing
```{r m1_m2_macrophages, echo = FALSE}

#human
M1 <- c("CD80", "CD86", "HLA-DRA",    # Surface markers
        "TNF", "IL12B", "CXCL9",     # Secreted cytokines/chemokines
        "CXCL10", "CXCL11", "IL23A", 
        "STAT1", "IRF5", "NOS2")      # Signaling/transcription factors

# Anti-inflammatory M2 markers (with subtypes)
M2 <- c("CD163", "CD206", "CD209",  # Surface markers
        "IL10", "TGFB1", "CCL18",   # Secreted factors
        "STAT3", "STAT6", "ARG1")    # Signaling/transcription factors

# Mouse equivalents
M1_mouse <- c("Cd86", "Cd80", "H2-Aa", "Nos2", "Il12b")
M2_mouse <- c("Cd163", "Cd206", "Arg1", "Ym1", "Fizz1") 

FeaturePlot(harm_mac_obj_list$mm_mets_normal_cells,
            features = M1_mouse)
