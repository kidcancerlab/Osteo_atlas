...
#for (i in 49:nrow(samples_to_run)) {
    ...
    if (!file.exists(qs_file)) {
        ...
        ggsave(paste0("output/id_tumor/snvs/plots/", sample_id, "_dim_plot.png"),
               plot = dim_plot_name,
               width = 20,
               height = 20)
        ...
        if (!dir.exists(paste0("output/id_tumor/snvs/", sample_id))) {
            dir.create(paste0("output/id_tumor/snvs/", sample_id), recursive = TRUE)
        }
        ...
        snp_tree <-
            tryCatch({
                get_snp_tree(cellid_bam_table = c_b_t,
                            ...
                            sbatch_base = paste0("output/id_tumor/snvs/", sample_id, "/shell"),
                            slurm_base = paste0("output/id_tumor/snvs/", sample_id, "/slurm"),
        ...
        for (depth in c("min_depth_5", "min_depth_10", "min_depth_20")) {
            if (!is.null(snp_tree[[depth]])) {
                ...
                png(paste0("output/id_tumor/snvs/", sample_id, "/", depth, "_", sample_id, ".png"),
                    width = 2500,
                    height = 2500,
                    res = 300)
                plot(snp_tree[[depth]])
        ...
        if (!is.null(snp_tree)) {
            plot_name <-
                DimPlot(s_obj,
                        group.by = c("snp_min_depth_5",
                                     "snp_min_depth_10",
                                     "snp_min_depth_20"))
            ggsave(paste0("output/id_tumor/snvs/", sample_id, "_dim_plot_calls.png"),
                   plot = plot_name,
                   width = 20,
                   height = 15)
            ...
            s_obj@meta.data %>%
                ...
                write_tsv(paste0("output/id_tumor/snvs/", sample_id, "_snp_calls.tsv"))
        }
    }
}
    #return(TRUE)
