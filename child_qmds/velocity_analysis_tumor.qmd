...
I'm also going to only write off single-cell reads for my objects. 

```{r}
yogi_prefix <- "/home/gdrobertslab/lab/Analysis/Yogesh/CellTypeAnnRefs/"
ob_names <- list.files(paste0(yogi_prefix, "output/seurat_objects/final_tumor_vs_stroma"))
ob_names <- grep("cancer_cells_fdl", ob_names, value = TRUE)

ob_list <- list()
for (ob in ob_names) {
    ob_list[[str_remove(ob, "_fdl.qs")]] <- qs::qread(
        paste0(
            yogi_prefix,
...
# make metadata dir
dir.create("loom_output/tumor_metadata/sc", recursive = TRUE)
dir.create("loom_output/tumor_metadata/sn", recursive = TRUE)

# now split objects and write metadata
for (ob in names(ob_list)) {
    num_sc <- sum(ob_list[[ob]]$method == "single_cell")    
    if (num_sc > 0) {
        subset(ob_list[[ob]],
           method == "single_cell") %>%
        write_off_md(
            id_col = "sample_name",
            output_dir = paste0("loom_output/tumor_metadata/sc/", ob),
    ...
    num_sn <- sum(ob_list[[ob]]$method == "single_nucleus")
    if (num_sn > 0) {
        subset(ob_list[[ob]],
            method == "single_nucleus") %>%
            write_off_md(id_col = "sample_name",
                    output_dir = paste0("loom_output/tumor_metadata/sn/",
                                        ob),
                    vars_to_keep = c("sample_name",
                                    "seurat_clusters",
                                    "Ann_Level1",
...
sc_obs = os.listdir("loom_output/tumor_metadata/sc")
sn_obs = os.listdir("loom_output/tumor_metadata/sn")

# Make directories for loom object output
os.makedirs("loom_output/tumor_anndata/sc")
os.makedirs("loom_output/tumor_anndata/sn")

# make directories for velocity figures
os.makedirs("output/figures/rna_velocity/sc")
os.makedirs("output/figures/rna_velocity/sn")

for snob in sn_obs:
    python_functions.write_obs(
        ob = snob,
        method = "sn",
        mode = "stochastic",
        metadata_dir = "loom_output/tumor_metadata/sn/",
        out_prefix = "loom_output/tumor_anndata/")
    python_functions.save_plots(
        ob = snob,
        method = "sn",
        ad_prefix = "loom_output/tumor_anndata/sn/",
        file_prefix = "output/figures/rna_velocity/"
        )


for scob in sc_obs:
    python_functions.write_obs(ob = scob, method = "sc", mode = "stochastic")
    python_functions.save_plots(ob = scob, method = "sc")

```

#### Make objects with Stochastic Model

```{python}
sc_obs = os.listdir("loom_output/tumor_metadata/")
for scob in sc_obs:
    if not os.path.exists("loom_output/tumor_anndata/sc/" + scob + ".ad"):
        write_obs(
            ob = scob,
            method = "sc",
            mode = "stochastic",
    ...
    # ad_ob = calc_velo(scob, mode = "stochastic")
    save_plots(ob = scob, method = "sc", file_prefix = "output/figures/rna_velocity/stochastic/", ad_prefix = "loom_output/tumor_anndata/sc/")


for snob in sn_obs:
    if not os.path.exists("loom_output/tumor_anndata/sn/" + snob + ".ad"):
        python_functions.write_obs(
            ob = snob,
            method = "sn",
            mode = "stochastic",
    ...
    # ad_ob = calc_velo(scob, mode = "stochastic")
    python_functions.save_plots(ob = scob, method = "sn", file_prefix = "output/figures/rna_velocity/", ad_prefix = "loom_output/tumor_anndata/sn/")


```

Going to test it with a sample object

```{r}
ob = "mm_mets_cancer_cells_fdl"

ad_ob = loom_to_an(
    ob,
    loom_dir = "loom_output/samples",
    metadata_dir = "loom_output/tumor_metadata"
)

calc_velo(ad_ob, mode = "stochastic")
