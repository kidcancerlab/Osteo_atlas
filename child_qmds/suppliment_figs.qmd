...
cell_cols <- list(
    ...
    "Mast" = "#097F9AFF",
    "Monocytes" = "#FF6E6AFF",
    "Endothelial_cells" = "#FFBB70FF",
    "MP_Progenitor" = "#68A4E3FF",
    "Fibrogenic" = "#79D379FF",
    "Basal_Progenitor" = "#CDCDCDFF",
    "Proliferative" = "#BF6BE2FF",
    "Interactive" = "#69D1F3FF",
    "COMA" = "#DB00FF",
    "Fibroblasts" = "#FA00FF",
    "Alv_Macrophages" = "#00FFD1",
    "Epithelial_cells" = "#0029FF",
    "Neutrophils" = "#FF00D6",
    "Smooth_muscle" = "#00FF47",
    "Erythrocytes" = "#00FFE0"
)
...
for (group in c(
    ...
    object <-
        qs::qread(str_c("output/seurat_objects/final_tumor_vs_stroma/",
                        ...
                        ".qs"))
    object$AtlasCall <-
        ifelse(object$celltype_tumor_call == "cancer_cells", "Tumor", "Normal")
    object$Ann_Level2 <-
        ifelse(object$AtlasCall == "Tumor", "Tumor", object$Ann_Level2)
    ...
    metadata <- 
        ...
        select(cell_id, 
                ...
                CopyKAT,
                CellTypes)
    # tileplot with celltype annotation
    # with new celltypes 
    prep_metadata <-
        metadata %>%
    ...
    prep_metadata$method <-
        factor(prep_metadata$method,
                levels = c("AtlasCall", "CopyKAT", "SCEVAN", "scATOMIC", "CellTypes"))
    ...
    tile_plot_new <-
        ggplot(prep_metadata, aes(x = method, y = cell_id_number, fill = call)) +
            geom_tile() +
            scale_fill_manual(values = unlist(cell_cols)) +
            theme_minimal() +
    ...
    matadata_mimic <-
        metadata %>%
        select(cell_id, cell_id_number,
                    scATOMIC, SCEVAN, CopyKAT) %>%
        arrange(CNV_status,
                scevan_tumor_call,
                scatomic_tumor_call) %>%
        pivot_longer(cols = -c(cell_id, cell_id_number),
...
for (ob_name in ob_names) {
    ...
    if (file.exists(tumor_path)) {
        ...
        dimplot5 <-
            dimplot_better(tumor, group_by = "Majority", cols = doublet_cols) + NoLegend()
