...
for (names in samples_for_validation) {
    ...
    # Calculate confusion matrix and metrics to compare final_tumor_call vs truth_metadata

    # Calculate precision for each predicted class (using columns as reference)
    precision_table <- table_ann
    for (pred_class in rownames(table_ann)) {
        for (ref_class in colnames(table_ann)) {
            TP <- table_ann[pred_class, ref_class]
            FP <- sum(table_ann[pred_class, ]) - TP
            precision <- if ((TP + FP) > 0) TP / (TP + FP) else NA
            precision_table[pred_class, ref_class] <- round(precision, 3)
        }
    }
    print("Precision table (rows: predicted, columns: reference):")
    print(precision_table)
    table_ann <- 
        table(combine_sobject$truth_metadata,
                combine_sobject$final_tumor_call)
    table_ann <-
        table(combine_sobject$truth_metadata,
                big_obj_with_tumor$annot_with_tumor_as_whole)

    # Calculate sensitivity (recall) and specificity for "tumor" label
    if (all(c("tumor", "stroma") %in% rownames(table_ann)) && all(c("tumor", "stroma") %in% colnames(table_ann))) {
        TP <- table_ann["tumor", "tumor"]
        TN <- table_ann["stroma", "stroma"]
        FP <- table_ann["tumor", "stroma"]
        FN <- table_ann["stroma", "tumor"]
        sensitivity <- TP / (TP + FN)
        specificity <- TN / (TN + FP)
        print(str_c("Sensitivity (tumor): ", round(sensitivity, 3)))
        print(str_c("Specificity (stroma): ", round(specificity, 3)))
    }
