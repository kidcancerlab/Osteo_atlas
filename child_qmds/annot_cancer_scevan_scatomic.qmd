...
for (group in c("patient_prim", "patient_mets", "mm_prim", "mm_mets")) {
    ...
    result_list <-
        parallel::mclapply(names(count_mtx_list),
                           mc.cores = min(5, parallelly::availableCores()),
                           function(x) {
            output <-
    ...
    write_tsv(bind_rows(result_list),
              str_c("output/id_tumor/SCEVAN/",
                    group,
                    "_metadata.tsv"))
}
```
...
for (group in c("patient_prim", "patient_mets")) {
    ...
    write.table(bind_rows(scATOMIC_results),
                str_c("output/id_tumor/scATOMIC/",
                      group,
                      "_metadata.tsv"),
                sep = "\t",
                quote = FALSE)
