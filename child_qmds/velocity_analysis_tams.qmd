...
```{r write-off-md}
dir.create("loom_output/tam_metadata/sc", recursive = TRUE)
dir.create("loom_output/tam_metadata/sn")

lapply(names(tam_list), function(obj_name) {
    obj <- subset(tam_list[[obj_name]], method == "single_cell")
    write_off_md(sobj = tam_list[[obj_name]],
                 id_col = "sample_name",
                 output_dir = paste0("loom_output/tam_metadata/sc/",
                                     obj_name),
    ...
    if ("single_nucleus" %in% names(table(tam_list[[obj_name]]$method))) {
        obj <- subset(tam_list[[obj_name]], method == "single_nucleus")
        write_off_md(sobj = tam_list[[obj_name]],
                     id_col = "sample_name",
                     output_dir = paste0("loom_output/tam_metadata/sn/",
                                         obj_name),
                     vars_to_keep = c("sample_name",
                                      "seurat_clusters",
                                      "Ann_Level1",
...
# write off objects and make plots for single cell

ob_names = os.listdir("loom_output/tam_metadata/sc/")
for ob in ob_names:
    print("starting on " + ob)
    merged_ad = loom_to_an(
        obj_name = ob,
        loom_dir = "loom_output/samples",
        metadata_dir = "loom_output/tam_metadata/sc/")
    print("made " + ob + " , now calculating velocity")
    calc_velo(merged_ad)
    merged_ad.write(filename = "loom_output/tam_anndata/sc/" + ob + ".ad")
    print("finished " + ob)

for ob in ob_names:
    ad_ob = anndata.read("loom_output/tam_anndata/sc/" + ob + ".ad")
    ...
    pyplot.savefig("output/figures/rna_velocity_tams/sc/" + ob + "_umap.svg")


# write off objects and make plots for single nucleus

ob_names = os.listdir("loom_output/tam_metadata/sn/")
for ob in ob_names:
    print("starting on " + ob)
    merged_ad = python_functions.loom_to_an(
        obj_name = ob,
        loom_dir = "loom_output/samples",
        metadata_dir = "loom_output/tam_metadata/sn/")
    print("made " + ob + " , now calculating velocity")
    python_functions.calc_velo(merged_ad)
    merged_ad.write(filename = "loom_output/tam_anndata/sn/" + ob + ".ad")
    print("finished " + ob)


for ob in ob_names:
    ad_ob = anndata.read("loom_output/tam_anndata/sn/" + ob + ".ad")
    scv.pl.velocity_embedding_stream(
        ad_ob,
        basis = "umap",
        color = "Ann_Level3",
    ...
    pyplot.savefig("output/figures/rna_velocity_tams/sn/" + ob + "_umap.svg")
