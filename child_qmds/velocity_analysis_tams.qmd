...
obj_names <- list.files(paste0(yogi_prefix, "output/seurat_objects/final_tumor_vs_stroma/")) %>%
    grep(pattern = "normal", value = TRUE)
obj_names <- c(obj_names, "xeno_mets_mouse.qs", "xeno_prim_mouse.qs")
...
for (ob_name in obj_names) {
    tam_list[[ob_name]] <-
        ...
        subset(Ann_Level2 %in% c("DC", "Monocytes", "TAMs"))
        # process_seurat() %>%
...
for ob in ob_names:
    ...
    #     method = "sc",
    #     mode = "stochastic",
    #     out_prefix = "loom_output/tam_anndata/stochastic/",
    #     metadata_dir = "loom_output/tam_metadata/"
    # )
    ad_ob = anndata.read("loom_output/tam_anndata/stochastic/sc/" + ob + ".ad")
    ...
    pyplot.savefig("figures/scvelo/tams/" + ob + "_umap.svg")
