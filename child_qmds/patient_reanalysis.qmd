...
This calculates the correlation between the entire harmonized dataset and
clusters from individual samples to be sure we are not overcorrecting the data
and losing the biological signal.
```{r, validation_clustering_batch_correction}
object_list <-
    tribble(
        ~group,                         ~res_value,  ~label,
        "patient_prim_cancer_cells",    0.3,         "Human Patient Primary Cancer Cells",
        "patient_mets_cancer_cells",    0.35,        "Human Patient Metastatic Cancer Cells",
        # "mm_prim_cancer_cells",         0.9,         "Mouse Primary Cancer Cells",
        "mm_mets_cancer_cells",         0.7,         "Mouse Metastatic Cancer Cells",
        "xeno_prim_human_cancer_cells", 0.3,         "Xenograft Primary Human Cancer Cells",
        "xeno_mets_human_cancer_cells", 0.4,         "Xenograft Metastatic Human Cancer Cells",
        "dogs_prim_cancer_cells",       0.2,         "Dog Primary Cancer Cells",
        "dogs_mets_cancer_cells",       0.45,        "Dog Metastatic Cancer Cells"
    )
...
for (i in seq_len(nrow(object_list))) {
    ...
    all_sample$Ann_Level2 <- str_replace_all(all_sample$Ann_Level2,
                                             "Stressed",
                                             "COMA")
    ...
    for (this_sample in unique(all_sample$sample_name)) {
        if (ncol(subset(all_sample, sample_name == this_sample)) < min_n_cells) {
            next
        }
        # Get one sample to compare to the whole dataset
        one_sample <-
            subset(all_sample,
        ...
        # optimize the resolution for this sample using silhouette scoring
        sil_val <-
        ...
        if (nrow(sil_val) != 0) {
            ...
            one_sample <- FindClusters(one_sample,
                                       resolution = sil_val,
                                       verbose = FALSE)
            ...
            n_cells_key <-
                table(one_sample$seurat_clusters, one_sample$sample_name) %>%
                as.data.frame() %>%
                dplyr::rename(cluster = "Var1",
                              sample_name = "Var2",
                              n_cells = "Freq") %>%
                mutate(cluster = paste0("clust", cluster)) %>%
                pull(n_cells, name = cluster)
            ...
            for (clusters in colnames(aver_exp1)[-1]) {
                for (cluster_names in colnames(aver_exp2)[-1]) {
                    ...
                    correlation <-
                        ...
                        mutate(small_cluster = str_c(this_sample, "_", clusters),
                               big_cluster = cluster_names,
                               n_cells = n_cells_key[clusters]
                        ) %>%
                        rbind(correlation)
                }
            }
            # Save the data from this sample to make a combined heatmap
            ...
            combine_plot <-
                cowplot::plot_grid(dimplot_main,
                                   dimplot_subset,
                                   ggplotify::as.ggplot(correlation_plot),
                                   ncol = 1)
    ...
    combined_corr_plot <-
        combined_corr %>%
            mutate(
                cluster = str_remove(small_cluster, ".+_") %>% str_replace("clust", "Cluster "),
                sample_name = str_remove(small_cluster, "_clust.+"),
                big_cluster = str_replace(big_cluster, "Progenitor", "P"),
        ...
        geom_text(aes(label = big_cluster_label),
                  size = 3.5,
                  fontface = "bold") +
        scale_fill_gradient2(low = "blue", mid = "white", high = "red") +
        labs(x = "Clusters in Individual Samples (if present)",
             y = "Individual Samples",
             title = paste0("Correlation of Harmonized Tumor Populations ",
    ...
    qs::qsave(
        combined_corr_plot,
        str_c("output/figures/validation/integration_validation/",
              object_list$group[i],
              "/",
              "All_combined_heatmap.qs")
    )

    height <- length(unique(all_sample$sample_name)) * 1 + 2
...
Making scatterplots of the average expression of each cluster in the combined
object to see how they correlate with each other. This will help in validating
the clustering and harmony of the data.
...
for (group in c("patient_xeno_prim_mets", "mouse_prim_mets", "dog_prim_mets")) {
    ...
    pca_data$unique <- object$unique[match(pca_data$Sample, object$sample_name)]
    pca_data$method <- object$method[match(pca_data$Sample, object$sample_name)]
    ...
    pca_plot <-
        ...
        theme_minimal() +
        labs(title = "PCA by Dominant Cell Type Composition per Sample",
             x = "PC1",
             y = "PC2",
             color = "dominant_cell_type")
    
    # define color for unique
    unique_types <- unique(pca_data$unique)
    unique_cell_type_colors <- setNames(RColorBrewer::brewer.pal(min(length(unique_types), 8), "Set1")[seq_along(unique_types)], unique_types)
    pca_plot_unique <-
        ggplot(pca_data, aes(x = PC1, y = PC2, label = Sample, color = unique)) +
        geom_point(size = 3) +
        ggrepel::geom_text_repel(size = 3, max.overlaps = 100) +
        scale_color_manual(values = unique_cell_type_colors) +
        theme_minimal() +
        labs(title = "PCA of site Composition per Sample",
             x = "PC1",
             y = "PC2",
             color = "dominant_cell_type")
    
    # deine color for method
    method_types <- unique(pca_data$method)
    method_cell_type_colors <- setNames(RColorBrewer::brewer.pal(min(length(method_types), 8), "Set1")[seq_along(method_types)], method_types)
    pca_plot_method <-
        ggplot(pca_data, aes(x = PC1, y = PC2, label = Sample, color = method)) +
        geom_point(size = 3) +
        ggrepel::geom_text_repel(size = 3, max.overlaps = 100) +
        scale_color_manual(values = method_cell_type_colors) +
        theme_minimal() +
        labs(title = "PCA of Method Composition per Sample",
             x = "PC1",
             y = "PC2",
             color = "dominant_cell_type")

    # Automatically determine optimal number of clusters using the elbow method
    max_k <- min(6, nrow(pca_data) - 1)
    ...
    combined_pca_plot <- 
        cowplot::plot_grid(pca_plot, pca_plot_unique,
                         pca_plot_method, pca_cluster_plot, ncol = 2)
