...
for (group in c("patient_prim", "patient_mets",
    ...
    if ("snv_call" %in% colnames(object@meta.data)){
        combineplot3 <-
            dimplot_better(object,
                           group_by = "snv_call") +
            NoLegend() +
...
for (group in c("patient_prim", "patient_mets",
    ...
    for (cell_group_name in c("cancer_cells", "normal_cells")) {
        ...
        if  (cell_group_name == "normal_cells") {
            ...
            set.seed(199820)
            if (sub_group == "dogs_prim_normal_cells" | group == "dogs_mets_normal_cells") {
                s_obj <- 
...
```{r map_the _clusters, echo = FALSE}
object <-
        qs::qread(str_c("output/seurat_objects/final_combined_sobjs/",
                        "dogs_prim",
                        ".qs"))
...
## Checking for the clusters where they map
```{r map_the _clusters_pmets, echo = FALSE}
()
obj_tumor <- 
