...
for (group in c("patient_prim", "patient_mets", "mm_prim", "mm_mets")) {
    ...
    # cancer_normal_metadata <-
    #     str_c(paste0("output/id_tumor/celltype/",
    #             group,
    #             "_metadata",
    #             ".tsv"))

    # if (file.exists(cancer_normal_metadata)) {
    #     cancer_normal_metadata <-
    #         readr::read_tsv(cancer_normal_metadata) %>%
    #         filter(cell %in% colnames(object)) %>%
    #         select(cell, celltype_tumor_call) %>%
    #         column_to_rownames("cell")
    #     object <- object %>%
    #         AddMetaData(metadata = cancer_normal_metadata)
    # }
