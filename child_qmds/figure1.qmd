...
dimplot_better(sobj_list$xeno_prim_mouse, group_by = "Ann_Level3")
...
##Sankey plot for stromal cells
# Prepare data for Sankey plot
cell_count_long <- cell_count_table %>%
    pivot_longer(cols = -celltypes, names_to = "group", values_to = "count") %>%
    filter(count > 0)

nodes <- data.frame(name = unique(c(cell_count_long$celltypes, cell_count_long$group)))

cell_count_long$source <- match(cell_count_long$group, nodes$name) - 1
cell_count_long$target <- match(cell_count_long$celltypes, nodes$name) - 1

links <- cell_count_long %>%
...
networkD3::saveNetwork(sankey_plot_stroma,
                      "output/figures/Final_Annotations/stromal_cell_count_sankey.html")
...
## Generate a Sankey plot for the cancer cell types
# Prepare data for Sankey plot
cell_count_long <- cell_count_table %>%
    pivot_longer(cols = -celltypes, names_to = "group", values_to = "count") %>%
    filter(count > 0)
...
# Save the Sankey plot
networkD3::saveNetwork(sankey_plot_tumor,
                      "output/figures/Final_Annotations/tumor_cell_count_sankey.html")
