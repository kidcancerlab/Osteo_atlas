...
png("figures/spatial/all_sample_correlation.png", width = 900, height = 900)
...
cell_type_heatmaps <- lapply(names(ann_mat_list), function(x) {
    p <- pheatmap::pheatmap(cor(ann_mat_list[[x]], method = "spearman"),
                       ...
                       angle_col = 45,
                       fontsize = 16)
    png(paste0("figures/spatial/cell_type_heatmap_", x, ".png"), width = 900, height = 900)
    print(p)
    dev.off()
    return(p)
})
...
for (feat in tumor_types) {
    ...
    png(paste0("figures/spatial/cell_type_correlation_", feat, "_level2.png"))
    print(p)
    dev.off()
}
```
...
for (ob_name in names(spatial_list)) {
    ...
    png(paste0("figures/spatial/composition_cluster_", ob_name, ".png", width = 600))
    ...
    png(paste0("figures/spatial/ISCHIA_enriched_cell_types_", ob_name, ".png"),
        height = 1600,
        width = 2000)
    grid.arrange(grobs = enriched_ct_plots, ncol = 4)
    dev.off()

    png(paste0("figures/spatial/ISCHIA_coocurring_cells_", ob_name, ".png"),
        height = 1600,
        width = 2000)
...
for (ob_name in names(spatial_list)) {
    ...
    png(paste0("figures/spatial/", ob_name, "_banksy_mean_deconv_heatmap.png"))
