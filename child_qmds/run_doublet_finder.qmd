...
ob_names <- c(
    ...
    "patient_mets",
    "patient_prim",
    "patient_xeno_prim_mets_cancer_cells",
    "xeno_mets_mouse",
    "xeno_prim_mouse"
)
...
cancer_obs[["patient_xeno_mets"]] <- subset(cancer_obs[["patient_xeno_prim_mets_cancer_cells"]], tumor_type == "metastatic")
cancer_obs[["patient_xeno_prim"]] <- subset(cancer_obs[["patient_xeno_prim_mets_cancer_cells"]], tumor_type == "primary")
...
for (name in names(cancer_obs)) {
    png(paste0("demuxafy_figs/cancer_dimplot_", name, ".png"))
...
```{r pdx-sanity-check}
pdx_details <- read.table("misc/allsample_details.txt", header = TRUE) %>%
...
```{r}
human_pdx <- qs::qread("/home/gdrobertslab/lab/Analysis/Yogesh/CellTypeAnnRefs/output/seurat_objects/final_tumor_vs_stroma/xeno_mets_human_cancer_cells.qs")
# we only want metastatic
human_pdx <- subset(human_pdx, tumor_type == "metastatic")

human_stroma <- qs::qread("/home/gdrobertslab/lab/Analysis/Yogesh/CellTypeAnnRefs/output/seurat_objects/final_tumor_vs_stroma/patient_mets_normal_cells.qs")
