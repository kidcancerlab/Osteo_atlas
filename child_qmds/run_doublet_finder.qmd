...
#create 10x counts folders for troy and anand's data
for (i in seq_len(nrow(sample_info))) {
    print(sample_info$sample_name[i])
    
    #Check if its a SJ or NIH sample; these are the only ones that need new
    #count folders
    if (sample_info$data_source[i] == "NCI_POB" | sample_info$data_source[i] == "SJ") {
        if (sample_info$data_source[[i]] == "NCI_POB") {
            #Make new data path if it's NIH
            new_data_path <- paste0(atlas_path,
                                    "DoubletFinder/troy_counts/",
                                    sample_info$sample_name[i])
            sample_info$data_path[i] <- new_data_path
        } else {
            #make new data path if it's SJ
            new_data_path <- paste0(atlas_path,
                                    "DoubletFinder/anand_counts/",
                                    sample_info$sample_name[i])
            sample_info$data_path[i] <- new_data_path
        ...
        if (!dir.exists(new_data_path)) {
            #Read in counts matrix if a folder hasn't already been made for that sample
            tmp_h5 <- Read10X_h5(sample_info$h5_path[i]) 
            if (class(tmp_h5) == "list") {
                tmp_h5 <- tmp_h5[["Gene Expression"]]
            }
            DropletUtils::write10xCounts(x = tmp_h5,
                                         path = new_data_path,
                                         version = "3")
        }

    } else {
        sample_info$data_path[i] <-
...
```{bash}
sbatch DoubletFinder/run_doubletfinder.sh
```
...
ob_names <- c("mm_mets_cancer_cells",
...
for (ob_name in ob_names) {
    ...
    tmp_ob <- qs::qread(paste0("/home/gdrobertslab/lab/Analysis/Yogesh/CellTypeAnnRefs/output/seurat_objects/comb_sobjs/",
                                       ob_name,
                                       ".qs"))
...
p_mets <- obj_list[["patient_mets_cancer_cells"]]
...
obj_list <- lapply(obj_list, reconstruct_cell_types)
...
for (name in names(obj_list)) {
    ...
    dimplot_better(obj_list[[1]], group = c("Ann_Level1", "MajoritySinglet_DropletType"))
    dev.off()
}
dim_list <- lapply(obj_list[c(1, 3, 5, 7)], function(ob) {
    dimplot_better(ob, group = c("Ann_Level1", "MajoritySinglet_DropletType"))
})

png("demuxafy_figs/all_cancer_dimplot.png", height = 1000, width = 3000)
dim_list[[1]] | dim_list[[2]] | dim_list[[3]] | dim_list[[4]]
dev.off()

png(paste0("demuxafy_figs/tumor_cell_doublet_call_heatmap.png"), height = 1000, width = 3000)
grid.arrange(grobs = df_result_heatmaps[c(1, 3, 5, 7)], ncol = 6)
dev.off()
```
...
I'm going to assume a doublet rate of 8% for all samples. This is based on 10x's established rate of 8% per 10,000 cells

```{r rDoubletFinder}
library(DoubletFinder)
df_results <- list()
for (ob_name in names(obj_list)) {
    #initialize list to hold results for this object
    curr_ob_res <- list()
    full_ob <- obj_list[[ob_name]]
    for (sid in unique(full_ob$sample_name)) {
        #SC072 only has 1 cell in patient_mets_cancer_cells
        if (sum(full_ob$sample_name == sid) > 30) {
            ob <- subset(full_ob, sample_name == sid) %>%
                NormalizeData() %>%
                FindVariableFeatures() %>%
                ScaleData() %>%
                RunPCA(npcs = 30)
            ob <- RunUMAP(ob, dims = 1:30, reduction = "pca") %>%
                FindNeighbors() %>%
                FindClusters()
            ...
            #Homotypic doublet proportion estimate
            annotations <- ob$Ann_Level3
            homotypic.prop <- modelHomotypic(annotations)
            #number of expected doublets based on 8% doublet rate
            nExp_poi <- round(0.08*ncol(ob))
            nExp_poi_adj <- round(nExp_poi*(1 - homotypic.prop))
    ...
    df_results[[ob_name]] <- curr_ob_res
}

df_obs <- lapply(df_results, function(x) {
    merge(x[[1]], x[2:length(x)]) %>%
        process_seurat()
})

df_obs <- list()
for (ob_list in names(df_results)) {
    print(ob_list)
    df_obs[[ob_list]] <- merge(df_results[[ob_list]][[1]],
                               df_results[[ob_list]][2:length(df_results[[ob_list]])])
}
...
for (ob_name in names(object_list)) {
    tmp_df_res <- list()
    for (sid in names(df_results[[ob_name]])) {
        tmp <- df_results[[ob_name]][[sid]]@meta.data
        df_res <- tmp[, startsWith(colnames(tmp), "DF.classifications")]
        tmp_df_res[[sid]] <- setNames(df_res, rownames(tmp))
    }
    df_by_ob <- AnnotationDbi::unlist2(tmp_df_res)
    
    object_list[[ob_name]] <- AddMetaData(object_list[[ob_name]],
                                          df_by_ob,
                                          col.name = "DoubletFinder_call")
}

doublet_finder_umaps <- lapply(object_list, function(x) {
    r_dim_plot(x, group.by = "Ann_Level3")
})
