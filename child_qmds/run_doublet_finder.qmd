...
ggsave("output/figures/Final_Annotations/results2/pdx_known_vs_calc_doublet_all_samples.pdf",
       all_combined,
       width = 3,
       height = 6,
       bg = "white")
...
# Convert mixed_read_results to a dataframe
mixed_read_df <-
    bind_rows(mixed_read_results) %>%
    mutate(proportion_doublets = as.numeric(proportion_doublets)) %>%
    filter(!is.na(proportion_doublets)) %>%
    filter(!is.nan(proportion_doublets)) %>%
    mutate(prop_normal = 1 - proportion_doublets) %>%
    select(-num_cells) %>%
    pivot_longer(cols = c("proportion_doublets", "prop_normal"),
                 names_to = "type",
                 values_to = "proportion")

# plot the mixed_read_df into bar/tileplot)
doublet_per_sample <- 
    ggplot(mixed_read_df, aes(x = sample, y = proportion, fill = type)) +
        geom_bar(stat = "identity", position = "dodge", color = "black") +
        scale_fill_manual(values = c("proportion_doublets" = "#D43F3AFF", "prop_normal" = "#EEA236FF"),
                        labels = c("Normal", "Doublets")) +
        labs(title = "Proportion of Mixed Barcodes Called Doublets/Normal by Demuxafy",
            x = "Sample ID",
            y = "Proportion") +
        theme_minimal() +
        theme(axis.text.x = element_text(angle = -45),
...
ggsave("output/figures/Final_Annotations/results2/pdx_known_vs_calc_doublet_per_sample.pdf",
       doublet_per_sample,
       width = 10,
       height = 6,
       bg = "white")
...
mixed_read_df <-
    ...
    filter(!is.na(proportion_doublets)) %>%
    filter(!is.nan(proportion_doublets)) %>%
    mutate(number_of_doublets = num_cells)

# Plot proportion of mixed barcodes called doublets
ggplot(mixed_read_df, aes(x = sample, y = proportion_doublets)) +
    geom_tile() +
    geom_label(aes(label = proportion_doublets), fill = "white") +
    theme(axis.text.x = element_text(angle = -45)) +
    labs(title = "Proportion of Mixed Barcodes Called Doublets by Demuxafy",
         x = "Sample ID",
         y = "Proportion of Mixed Barcodes Called Doublets")
