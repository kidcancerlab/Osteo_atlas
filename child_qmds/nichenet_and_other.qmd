...
SpatialFeaturePlot(spatial_list$OS8_Seurat,
                    features = c("Mesenchymal_like", "AT2", "AT1", "tumor_cumulative"),
                    ncol = 2,
                    pt.size.factor = 2
                    #image.alpha = 0
                    ) +
SpatialDimPlot(spatial_list$OS8_Seurat,
                alpha = 0) + NoLegend()


# Check if FN1 exists in your dataset
visium_obj <- spatial_list$OS1_Seurat
"FN1" %in% rownames(visium_obj)
# If FALSE, check for lowercase or alternate gene symbol
grep("FN1", rownames(visium_obj), value = TRUE, ignore.case = TRUE)
...
SpatialFeaturePlot(spatial_list$OS4_Seurat,
                    features = "FN1",
                    min.cutoff = 2,
                    pt.size.factor = 1.5) +
# SpatialDimPlot(spatial_list$OS8_Seurat,
...
TGFb_pathways <-
    ...
    filter(str_detect(group, "prim")) %>%
    arrange(desc(mean_prob)) %>%
    #filter(patient_prim > 0) %>%
    select(Target_LR, group, mean_prob)

TGFb_long <- 
    TGFb_pathways %>%
    pivot_longer(cols = -Target_LR, names_to = "group", values_to = "mean_prob") %>%
    arrange(desc(mean_prob))

ggplot(TGFb_pathways, aes(x = fct_reorder(Target_LR, mean_prob, .fun = max), y = mean_prob, fill = group)) +
    geom_col(position = "dodge") +
    coord_flip() +
    labs(title = "TGFb interactions (mean_prob) — primary samples",
                x = "Interaction (Target_LR)", y = "Mean probability") +
    theme_minimal() +
...
for (site in c("prim", "mets")) {
    ...
    # make a barplot of the mean correlations per group
    # ensure consistent ordering of comparison groups
    plot_cooraltion_df_df$Var2 <- factor(plot_cooraltion_df_df$Var2, levels = c("Dogs", "PDX", "Mouse"))
    # plot_cooraltion_df_df <-
    #     plot_cooraltion_df_df %>%
    #     mutate(Freq = round(Freq, 1))

    barplot_correlation <-
        ggplot(plot_cooraltion_df_df, aes(x = Var1, y = Freq, fill = Var2)) +
            geom_bar(stat = "identity", position = position_dodge(width = 0.8), width = 0.7) +
            # geom_text(aes(label = sprintf("%.2f", Freq)),
            #                     position = position_dodge(width = 0.8),
            #                     vjust = -0.4, size = 3) +
            scale_fill_manual(values = c("Dogs" = "#EEA236FF", "PDX" = "#D43F3AFF", "Mouse" = "#5CB85CFF")) +
            labs(title = str_c("Correlation of Pathways - ", site, " Site"),
                    x = "Pathway",
    ...
    ggsave(str_c("output/figures/cellchat/sharedgenes_modules/correlation_barplot_pathways_",
                site, ".png"),
            barplot_correlation,
            width = 15,
            height = 7,
