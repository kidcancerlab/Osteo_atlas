...
pathway_modules <- list(
    ECM_Remodelling = c("COLLAGEN","LAMININ","FN1","THBS","TENASCIN",
                        "AGRN","TGFb","ANNEXIN"),
    Cell_Adhesion = c("APP","CADM","PTPRM","NCAM","ADGRL","ADGRG","CDH",
                        "CDH5","PECAM1","ICAM","SELL","JAM","UNC5","CLDN"),
    Angiogenesis = c("VEGF","PDGF","ANGPT","IGF","MK","PTN","FGF"),
    Immune_Signalling = c("CXCL","CCL","MHC-II","IL1","CD45","CD80",
                            "CD86","OSM","CSF","TNF","TWEAK"),
    Stemness = c("BMP","NOTCH","ncWNT","RA","GDF"),
    Neuronal = c("SLIT","FLRT","MPZ","EPHA","NRG","NRXN","NEGR","SEMA3","SEMA6"),
    Metabolic = c("Cholesterol","Glutamate","RBP4","DHEA")
) %>%
    unlist() %>%
    unique()
...
for (group in c(
    ...
    for (file_path in cell_chat_objs) {
        ...
        pathways_and_lr_prob <-
            ...
            mutate(Target_LR = paste0(source, "-to-", target, "-", interaction_name)) %>%
...
for (site in c("prim", "mets")) {
    ...
    site_specific <-
        ...
        # filter(rowSums(select(., -Target_LR, -pathway_name)) > mean_cutoff) %>%
        group_by(pathway_name) %>%
        group_split() %>%
    ...
    plot_cooraltion_df <-
        site_specific %>%
        pivot_wider(names_from = comparison_group, values_from = correlation, values_fill = 0) %>%
    ...
    pheatmap::pheatmap(
        ...
        cluster_rows = TRUE,
        cluster_cols = TRUE,
        color = colorRampPalette(c("#5e9ffaff","#f3f6faff", "#f84c4cff"))(100),
        main = str_c("Correlation Heatmap of Pathways - ", site, " Site"),
        fontsize_row = 8,
        fontsize_col = 10,
        display_numbers = round(plot_cooraltion_df, 2),
        number_color = "black"
    )
...
# define the modules of pathways
cellchat_modules <- list(
    ECM = c("FN1", "COLLAGEN", "LAMININ", "TENASCIN", "THBS", "AGRN", "APP",
            "PTPR", "PTPRM", "ADGRL", "ADGRB", "ADGRG", "ANNEXIN"),
    CellAdhesion = c("CDH", "CDH1", "CDH5", "CADM", "NCAM", "PECAM1", "ICAM",
                     "JAM", "NECTIN", "CLDN", "OCLN", "PCDH", "SELL"),
    Angiogenesis = c("VEGF", "ANGPT", "ANGPTL", "PDGF", "FGF", "VWF", "EDN",
                     "CALCR", "RANKL", "CSF", "KIT", "HGF"),
    Stemness = c("WNT", "ncWNT", "NOTCH", "BMP", "GDF", "RA", "GAP", "MK", "PTN"),
    Neuronal = c("SEMA3", "SEMA5", "SEMA6", "UNC5", "Netrin", "FLRT", "NRXN",
                 "EPHA", "EPHB", "NRG", "MPZ", "NEGR"),
    Metabolic = c("Glutamate", "Cholesterol", "Estradiol", "Desmosterol",
                  "RBP4", "ENHO", "CysLTs"),
    ImmuneModulation = c("CXCL", "CCL", "IL2", "IL1", "IL6", "IL16", "TNF", "TWEAK",
                    "Prostaglandin", "CSF", "CX3C", "TGFb", "MHC-II", "CD45",
                    "CD80", "CD86", "ICOS", "CD30", "APRIL", "BAFF", "CD39")
)

# cellltypes present 
celltypes_present <- c(
    "Alv_Macrophages", "B_cells", "Basal", "Basal_Progenitor", "CAFs", "Chondrocytes",
    "COMA", "DC", "Endothelial_cells", "Epithelial_cells", "Erythrocytes", "Fibroblasts",
    "Fibrogenic", "ILCs", "Interactive", "Mast", "Monocytes", "MP_Progenitor", "MSC",
    "Muscle_cells", "Neuronal", "Neutrophils", "NK_cells", "Osteoblasts", "Pericytes",
    "Progenitor", "Proliferative", "Smooth_muscle", "Stressed", "T_cells", "TAMs"
)

celltype_modules <- list(
    ECM = c("Alv_Macrophages", "B_cells", "Basal", "Basal_Progenitor", "CAFs",
                "Chondrocytes", "COMA", "DC", "Endothelial_cells", "Epithelial_cells",
                "Erythrocytes", "Fibroblasts", "Fibrogenic", "ILCs", "Interactive",
                "Mast", "Monocytes", "MP_Progenitor", "MSC", "Muscle_cells", "Neuronal",
                "Neutrophils", "NK_cells", "Osteoblasts", "Pericytes", "Progenitor",
                "Proliferative", "Smooth_muscle", "Stressed", "T_cells", "TAMs"),

    CellAdhesion = c("Alv_Macrophages", "B_cells", "Basal", "Basal_Progenitor", "CAFs",
                "Chondrocytes", "COMA", "DC", "Endothelial_cells", "Epithelial_cells",
                "Erythrocytes", "Fibroblasts", "Fibrogenic", "ILCs", "Interactive",
                "Mast", "Monocytes", "MP_Progenitor", "MSC", "Muscle_cells", "Neuronal",
                "Neutrophils", "NK_cells", "Osteoblasts", "Pericytes", "Progenitor",
    ...
    Angiogenesis = c("Alv_Macrophages", "B_cells", "Basal", "Basal_Progenitor", "CAFs",
                "Chondrocytes", "COMA", "DC", "Endothelial_cells", "Epithelial_cells",
                "Erythrocytes", "Fibroblasts", "Fibrogenic", "ILCs", "Interactive",
                "Mast", "Monocytes", "MP_Progenitor", "MSC", "Muscle_cells", "Neuronal",
                "Neutrophils", "NK_cells", "Osteoblasts", "Pericytes", "Progenitor",
    ...
    Stemness = c("Alv_Macrophages", "B_cells", "Basal", "Basal_Progenitor", "CAFs",
                "Chondrocytes", "COMA", "DC", "Endothelial_cells", "Epithelial_cells",
                "Erythrocytes", "Fibroblasts", "Fibrogenic", "ILCs", "Interactive",
                "Mast", "Monocytes", "MP_Progenitor", "MSC", "Muscle_cells", "Neuronal",
                "Neutrophils", "NK_cells", "Osteoblasts", "Pericytes", "Progenitor",
    ...
    Neuronal = c("Alv_Macrophages", "B_cells", "Basal", "Basal_Progenitor", "CAFs",
                "Chondrocytes", "COMA", "DC", "Endothelial_cells", "Epithelial_cells",
                "Erythrocytes", "Fibroblasts", "Fibrogenic", "ILCs", "Interactive",
                "Mast", "Monocytes", "MP_Progenitor", "MSC", "Muscle_cells", "Neuronal",
                "Neutrophils", "NK_cells", "Osteoblasts", "Pericytes", "Progenitor",
    ...
    Metabolic = c("Alv_Macrophages", "B_cells", "Basal", "Basal_Progenitor", "CAFs",
                "Chondrocytes", "COMA", "DC", "Endothelial_cells", "Epithelial_cells",
                "Erythrocytes", "Fibroblasts", "Fibrogenic", "ILCs", "Interactive",
                "Mast", "Monocytes", "MP_Progenitor", "MSC", "Muscle_cells", "Neuronal",
                "Neutrophils", "NK_cells", "Osteoblasts", "Pericytes", "Progenitor",
    ...
    ImmuneModulation = c("Alv_Macrophages","TAMs","Monocytes","Neutrophils",
...
corr_list <- list()
# we can loop over specific modules
for (module_name in names(cellchat_modules)) {
    pathways_in_module <- cellchat_modules[[module_name]]
    celltypes_in_module <- celltype_modules[[module_name]]
    message("Processing module: ", module_name)
    # filter the All_pathways_LR_combined to only include pathways in the module
    module_specific_df <-
        All_pathways_LR_combined %>%
        filter(pathway_name %in% pathways_in_module) %>%
        mutate(target = str_extract(Target_LR, "(?<=-to-)[^-]+")) %>%
        filter(target %in% celltypes_in_module) %>%
        select(group, Target_LR, mean_prob, pathway_name)
    ...
    for (site in c("prim", "mets")) {
        ...
        site_specific_module <-
            ...
            pivot_wider(names_from = group,
                        ...
                        id_cols = Target_LR) %>%
            column_to_rownames("Target_LR") %>%
            filter(rowSums(. > 0) >= 3)  # keep rows with >0 in  all columns  
        
        cor_matrix <-
            cor(site_specific_module, use = "everything", method = "spearman")
        corr_vals <- 
            cor_matrix %>%
            as.tibble(rownames = "group") %>%
            as.data.frame() %>%
            filter(group == str_c("patient_", site)) %>%
            pivot_longer(-group, names_to = "comparison_group", values_to = "correlation") %>%
            filter(comparison_group != group) %>%
            mutate(module = module_name, site = site)
        corr_list[[str_c(module_name, "_", site)]] <- corr_vals
        ...
        heatmap <-
            pheatmap::pheatmap(
                ...
                cluster_cols = TRUE,
                silent = TRUE,
                color = colorRampPalette(c("#f8ceceff", "#fc5b5bff", "#ff0c0cff"))(100),
                main = str_c("Correlation Heatmap of ", module_name, " - ", site, " Site"),
                fontsize_row = 8,
                fontsize_col = 10,
                display_numbers = round(cor_matrix, 2),
                number_color = "black")
        if (!dir.exists(str_c("output/figures/cellchat/sharedgenes_modules/", module_name))) {
            dir.create(str_c("output/figures/cellchat/sharedgenes_modules/", module_name),
            recursive = TRUE)
        }
        ggsave(str_c("output/figures/cellchat/sharedgenes_modules/", module_name, "/correlation_heatmap_", module_name, "_", site, ".png"),
            heatmap,
            width = 8,
            height = 6,
            bg = "white")
...
# make the plot of the corrlations from corr_list
corr_list_all <-
...
# for primary site
for (names in unique(corr_list_all$site)) {
    site_corr <-
        corr_list_all %>%
        filter(site == names) %>%
    ...
    corr_sum <-
        ggplot(site_corr, aes(x = comparison_group, y = module, fill = correlation)) +
        ...
        ylab("Pathway Module") +
        ggtitle(str_c("Correlation of Pathway Modules - ", names)) +
        theme(axis.text.x = element_text(angle = 45, hjust = 1),
            legend.position = "right")

    ggsave(str_c("output/figures/cellchat/sharedgenes_modules/correlation_summary_", names, ".png"),
        corr_sum,
        width = 10,
        height = 10,
        bg = "white")
