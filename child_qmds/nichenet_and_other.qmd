...
for (group in c(
    ...
    for (file_path in cell_chat_objs) {
        ...
        receiver_cells <-
            setdiff(levels(cellchat@idents), ligand_source)
...
for (site in c("prim", "mets")) {
    ...
    pheatmap::pheatmap(
        ...
        main = "Pathway Mean Probability (Tumor→Stroma, Shared Genes)",
        fontsize_row = 8,
        fontsize_col = 10,
    ...
    pheatmap::pheatmap(
        ...
        cluster_rows = TRUE,
        cluster_cols = TRUE,
        color = colorRampPalette(c("#596affff", "#fd3f3fff"))(100),
    ...
    library(GGally)
    GGally::ggpairs(site_specific_GGPAIRS,
...
#  [1] "COLLAGEN"    "LAMININ"     "FN1"         "APP"         "THBS"       
#  [6] "CADM"        "NCAM"        "PTPRM"       "TGFb"        "ADGRL"      
# [11] "PDGF"        "VEGF"        "MPZ"         "FLRT"        "PTN"        
# [16] "Cholesterol" "TENASCIN"    "CDH"         "MHC-II"      "UNC5"       
# [21] "NRXN"        "CXCL"        "NRG"         "EPHA"        "CCL"        
# > site_specific_paths$mets$Pathway
#  [1] "COLLAGEN"    "LAMININ"     "FN1"         "ADGRL"       "CADM"       
#  [6] "NCAM"        "PTPRM"       "APP"         "VEGF"        "THBS"       
# [11] "UNC5"        "PDGF"        "CDH"         "EPHA"        "TGFb"       
# [16] "FLRT"        "MPZ"         "Cholesterol" "PTN"         "SEMA3"      
# [21] "PECAM1"      "TENASCIN"    "BMP"         "ADGRG"       "ICAM"       
# [26] "CCL"         "ANNEXIN"
...
for (site in c("Primary", "Metastatic")) {
    ...
    plot <-
        ggplot(ttest_results_df_site %>% filter(group != "Human"),
            ...
            theme_minimal(base_size = 14) +
            theme(axis.text.x = element_text(angle = 45, hjust = 1)) +
            labs(title = str_c("Pathway Probability Comparison Across Species (", site, ", Excluding Human)"),
                x = "Pathway", y = "Normalized Probability log10") +
            geom_text(
                data = ttest_results_df_site %>%
                    filter(group != "Human") %>%
                    group_by(pathway_name, group, comparison) %>%
                    summarise(y_pos = min(max(log10(norm_prob+0.0001), na.rm = TRUE) + 0.1, 22), significance = first(significance), .groups = "drop"),
...
interesting_pathways <- c("TGFb", "FN1", "LAMININ")
average_pathways <- list()
# load the objects
for (group in c(
                "patient_prim",
...
for (group in c(
    ...
    # find the ligands that are expressed in the sender cell type
    for (ligands in c("TGFB1", "FN1", "LAMA1", "LAMB1", "LAMC1")) {
        if (object$organism[1] == "human") {
            lig <- ligands
        } else {
        ...
        if (!(lig %in% rownames(ligand_target_matrix_use))) {
            message(str_c("Ligand ", lig, " not found in ligand-target matrix for group ", group))
            next
        }
        ...
        downstream_genes <-
            ...
            pull(target)
        # Subset downstream_genes to max 50 if more than 50
