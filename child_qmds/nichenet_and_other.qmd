...
for (i in 1:nrow(groups_by)) {
    ...
    if (nrow(chosen_pathways) != 0) {
        ...
        ggsave(str_c("output/figures/cellchat/", group_prim, "_vs_", group_mets, "_pathway_boxplot.png"),
        plot_pathways,
        width = length(chosen_pathways$pathway_name) * 0.75 + 1,
        height = 6,
        bg = "white")
    }
}
...
downstream_to_target <-
    targets$target

mm_prim <-
    qs::qread("output/seurat_objects/final_tumor_vs_stroma/mm_primhuman_ortho.qs")

mm_mets <-
    qs::qread("output/seurat_objects/final_tumor_vs_stroma/mm_metshuman_ortho.qs")

VlnPlot(mm_prim,
        features = downstream_to_target,
        group.by = "Ann_Level2")
...
for (i in 1:nrow(groups_by)) {
    ...
    # fill up the missing interactions with 0 prob for each sample
    fill_prob_prim_mets <-
        pathways_LR_df %>%
        mutate(sample_group = paste(sample, group, sep = "-")) %>%
    ...
    # calculate the mean prob for each interaction in both primary and mets separately
    total_number_mets <- 
        n_distinct(pathways_LR_df$sample[pathways_LR_df$group == "Metastatic"])
    prob_filter <- mean(pathways_LR_df$prob)
    mean_per_LR_in_mets <-
        pathways_LR_df %>%
        filter(group == "Metastatic") %>%
        group_by(interaction_name) %>%
        filter(max(prob) >= prob_filter) %>%
        arrange(desc(prob)) %>%
        summarise(mean_prob = sum(prob)/total_number_mets) %>%  
        arrange(desc(mean_prob))
    
    mean_per_LR_in_primary <-
        pathways_LR_df %>%
        filter(group == "Primary") %>%
        group_by(interaction_name) %>%
        filter(max(prob) >= prob_filter) %>%
    ...
    # jopin the two means and keep the higher one
    mean_per_LR <-
        full_join(x = mean_per_LR_in_primary,
                  y = mean_per_LR_in_mets,
                  by = "interaction_name",
                  suffix = c("_prim", "_mets")) %>%
        mutate(mean_prob = pmax(replace_na(mean_prob_prim, 0), replace_na(mean_prob_mets, 0))) %>%
    ...
    # now filter and divide the prob of each interaction in each sample by the mean_prob_LR
    pathways_LR_df_filtered <- 
        fill_prob_prim_mets %>%
        filter(interaction_name %in% mean_per_LR$interaction_name) %>%
        arrange(desc(prob)) %>%
        left_join(mean_per_LR, by = "interaction_name") %>%
        mutate(prob_normalized = prob / mean_prob)

    # make a boxplot of the top 20 interactions across primary and mets using prob_normalized
    # Get top 10 interactions in primary and top 10 in metastatic (by mean prob_normalized)
    top10_primary <-
        pathways_LR_df_filtered %>%
        filter(group == "Primary") %>%
        group_by(interaction_name) %>%
        summarise(mean_prob = mean(prob_normalized, na.rm = TRUE)) %>%
        arrange(mean_prob) %>%
        slice_head(n = 10) %>%
        pull(interaction_name)

    top10_mets <- pathways_LR_df_filtered %>%
        filter(group == "Metastatic") %>%
        group_by(interaction_name) %>%
        summarise(mean_prob = mean(prob_normalized, na.rm = TRUE)) %>%
        arrange(desc(mean_prob)) %>%
    ...
    top_interactions_normalized <- unique(c(top10_primary, top10_mets))

    # make a boxplot of the top 20 interactions across primary and mets using prob_normalized
    plot_df_normalized <- 
        pathways_LR_df_filtered %>%
        filter(interaction_name %in% top_interactions_normalized) %>%
        arrange(desc(prob_normalized))

    # Ensure "Primary" is first in the group order
    plot_df_normalized$group <- factor(plot_df_normalized$group, levels = c("Primary", "Metastatic"))

    ggplot(plot_df_normalized, aes(x = reorder(interaction_name, -prob_normalized, FUN = median), y = prob_normalized, fill = group)) +
        geom_boxplot(outlier.shape = NA, position = position_dodge(width = 0.8)) +
        geom_jitter(width = 0.2, size = 2, alpha = 0.7, aes(color = group)) +
        labs(title = paste("Top 20 Interactions (Normalized) in", group_prim, "vs", group_mets),
             x = "Interaction",
             y = "Probability (Normalized)") +
        theme_minimal() +
        theme(axis.text.x = element_text(angle = 45, hjust = 1))
...
receiver_celltypes <- c(
    ...
    "TAMs", "DC", "Mast",
    "Monocytes", "Endothelial_cells",
    "Alv_Macrophages", "Epithelial_cells"
)
...
for (i in 1:nrow(groups_by)) {
    ...
    for (receiver_cell in receiver_celltypes) {
        ...
        if (length(unique(pathways_prob_df$group)) == 2) {
            ...
            if (total_number_primary > 1 & total_number_mets > 1) {
                ...
                if (nrow(chosen_pathways) != 0) {
                    plot_pathways <-
                        ...
                        scale_fill_manual(values = c("Primary" = "#87c9ffff", "Metastatic" = "#f87478ff")) +
                        scale_color_manual(values = c("Primary" = "#0088f7ff", "Metastatic" = "#f70004ff")) +
                        labs(title = str_c("Top Interactions from Tumor to TAMs in ", group_prim, " vs ", group_mets),
                            x = "Interaction",
                            y = "Normalized Probability (by mean)") +
                        theme_minimal() +
                                    theme(
                    ...
                    plot_pathways <-
                        ggplot() +
                        ggtitle(paste("No significant interactions from Tumor to", receiver_cell, "in", group_prim, "vs", group_mets)) +
                        theme_void()
                }
                ...
                ggsave(str_c("output/figures/cellchat/", group_prim, "_vs_", group_mets, "/tumor_to_", receiver_cell, "_boxplot.png"),
                plot_pathways,
            ...
            print(paste("Celltype ", receiver_cell, " not present in both", group_prim, "vs", group_mets))
        }
    }
}
