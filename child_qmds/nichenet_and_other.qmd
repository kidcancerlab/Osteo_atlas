...
for (comp in comparisons) {
    ...
    pathways_prob_df_filtered <- 
        ...
        mutate(fold_change = mean(prob) / max(mean_prob_human)) %>%
...
for (site in c("Primary", "Metastatic")) {
    ttest_results_df_site <-
        ...
        mutate(comparison = case_when(
            str_detect(group, "xeno") ~ "Human vs PDX",
            str_detect(group, "dogs") ~ "Human vs Dog",
            str_detect(group, "mm") ~ "Human vs Mouse"
        )) %>%
    ...
    plot <-
        ggplot(ttest_results_df_site %>% filter(group != "Human"),
            aes(x = pathway_name, y = norm_prob, fill = group)) +
            ...
            labs(title = str_c("Pathway Probability Comparison Across Species (", site, ", Excluding Human)"),
                x = "Pathway", y = "Normalized Probability (to Human Mean, capped at 20+)") +
            geom_text(
                data = ttest_results_df_site %>%
                    filter(group != "Human") %>%
                    group_by(pathway_name, group, comparison) %>%
                    summarise(y_pos = min(max(norm_prob, na.rm = TRUE) + 0.1, 22), significance = first(significance), .groups = "drop"),
