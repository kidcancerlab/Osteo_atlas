...
# now I want to look into specific pathways using the shared genes
ligand_source <- 
...
## no receiver as tumor and source as stroma
...
average_pathways_stroma_to_tumor <- list()
# load the objects
for (group in c(
                "patient_prim",
                "patient_mets",
    ...
    for (file_path in cell_chat_objs) {
        ...
        pathways_LR[[sample_name]] <-
            pathways_and_lr_prob %>%
            filter(source %in% sender_cells & target %in% ligand_receiver) %>%
            select(pathway_name, prob) %>%
            group_by(pathway_name) %>%
            summarise(prob = sum(prob)) %>%
            ungroup() %>%
    ...
    average_pathways_stroma_to_tumor[[group]] <- pathway_average

}

# make a heatmap from the average pathways
average_pathways_df <-
    do.call(rbind, average_pathways_stroma_to_tumor)
...
for (site in c("prim", "mets")) {
    ...
    pheatmap::pheatmap(
        ...
        cluster_cols = TRUE,
        color = colorRampPalette(c("white", "#ff6b6bff","#ff4a4aff","#ff2424ff", "#ff0e0eff"))(100),
        main = "Pathway Mean Probability (Stroma→Tumor, Shared Genes)",
        fontsize_row = 8,
        fontsize_col = 10,
        display_numbers = round(heatmap_df, 2)
    )
...
```{r ryans_test_to_classify_patient}

cellchat_list <- list()
pathways_prob <- list()
# get the pathways and lr with prob for primary
cell_chat_prim <-
    list.files(str_c("output/cellchat_objects/", "patient_prim"),
            pattern = "*.qs",
            full.names = TRUE)
...
# get the metastatic pathways and lr with prob
cell_chat_mets <-
    list.files(str_c("output/cellchat_objects/", "patient_mets"),
            pattern = "*.qs",
            full.names = TRUE)
for (file_path in cell_chat_mets) {
    sample_name <- str_replace(basename(file_path), ".qs", "")
...
# xeno prim
cell_chat_xeno_prim <-
    list.files(str_c("output/cellchat_objects/", "xeno_prim_cancer_normal"),
            pattern = "*.qs",
            full.names = TRUE)

for (file_path in cell_chat_xeno_prim) {
    sample_name <- str_replace(basename(file_path), ".qs", "")
    cellchat <- qs::qread(file_path)
    cellchat_list[[sample_name]] <- cellchat
    pathways_and_lr_prob <-
    ...
    pathways_prob[[sample_name]] <-
        ...
        summarise(prob = sum(prob)) %>%
        ungroup() %>%
        mutate(sample = sample_name, group = "Xeno_Primary") %>%
        arrange(desc(prob))
}

# xeno mets
cell_chat_xeno_mets <-
    list.files(str_c("output/cellchat_objects/", "xeno_mets_cancer_normal"),
            pattern = "*.qs",
            full.names = TRUE)
for (file_path in cell_chat_xeno_mets) {
    sample_name <- str_replace(basename(file_path), ".qs", "")
    cellchat <- qs::qread(file_path)
    cellchat_list[[sample_name]] <- cellchat
    pathways_and_lr_prob <-
    ...
    pathways_prob[[sample_name]] <-
        ...
        summarise(prob = sum(prob)) %>%
        ungroup() %>%
        mutate(sample = sample_name, group = "Xeno_Metastatic") %>%
        arrange(desc(prob))
}
