...
# Separate degs$2 from the degs list
degs_cluster <-
...
cluster_2_degs <-
    degs_all %>%
    filter(cluster == "2" & padj < 0.05 & log2FoldChange > 0.25) %>%
...
DotPlot(Epithelial_cells_tumor,
        features = c(AT1_markers, AT2_markers, TAEC_markers),
...
Epithelial_cells_tumor <-
    FindClusters(Epithelial_cells_tumor, resolution = 0.3) 

TAEC_markers <- c(
  "ZEB1",      # EMT transcription factor
  "VIM",       # Vimentin, mesenchymal marker
  "SERPINE1",  # ECM remodeling, TGF-β signaling
  "POSTN"     # Periostin, matrix remodeling
)
AT1_markers <- c(
...
DotPlot(combine_tumor_normal, features = c("SDC4"), group.by = "unique", scale = F)
...
dimplot_better(Epithelial_cells_tumor, group_by = "seurat_clusters") + NoLegend()
...
SpatialFeaturePlot(spatial_list[[ob_name]],
                    features = c("TAECs", "General_AT1_AT2", "MP_Progenitor", "Basal_Progenitor"),
                    ncol = 2)
...
tumor_asso_epi$TEACs_and_others <-
    ifelse(tumor_asso_epi$seurat_clusters %in% c(1, 5), "TAECs", "General_AT1_AT2")
