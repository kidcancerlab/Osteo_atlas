...
s_obj <-
    FindClusters(s_obj,
                resolution = 1.5)
dimplot_better(s_obj, group_by = "seurat_clusters") +NoLegend()
cells <-
    subset(s_obj, seurat_clusters == "5") %>%
    colnames()



DimPlot(object, cells.highlight = cells)

dimplot_better(object, group_by = "new_annot_clust") +NoLegend()

DATPs <- c("CLDN4", "KRT8", "CDKN1A", "SPRR1A", "TNIP3, LY6A", "NDRG1", "AREG")
caf_subtypes <- list(
  myCAFs = c("ACTA2", "FAP", "PDGFRB", "COL1A1", "COL1A2", "TAGLN", "CTGF"),
  iCAFs = c("IL6", "IL11", "CXCL12", "PDGFRA"),
  apCAFs = c("HLA-DRA", "HLA-DPB1", "CD74"),
...
FeaturePlot(s_obj, features = Mesenchymal$iCAFs)
```
...
dimplot_better(stroma_obj,
              group_by = "Ann_Level3",
              raster = F) + NoLegend()

FeaturePlot(s_obj, features = Mesenchymal$Myofibroblasts)
...
for (group in c("patient_prim", "patient_mets",
    ...
    combined_plot2 <-
        dimplot_better(object, group_by = "Ann_Level2", raster = F, pt.size = 0.7) + 
        NoLegend()

    combined_plot3 <-
        dimplot_better(object, group_by = "Ann_Level1", raster = F, pt.size = 0.7) +
        NoLegend()

    combined_plot4 <-
        dimplot_better(object, group_by = "Ann_Level0", raster = F, pt.size = 0.7) +
        NoLegend()
