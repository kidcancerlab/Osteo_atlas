...
details <-
    tribble(~group1,                                  ~resolution,  
            ...
            "dog_prim_mets_Immune_Myeloid",             0.7,
...
parallel::mclapply(seq_len(nrow(details)), function(item) {
    ...
    dimplot_better(combined_obj, group_by = "seurat_clusters") +
        NoLegend()
...
Inflam_TAMs = list("IL10", "NFKB1", "IL1B  ", "CCL4", "CXCL2", "CXCL1", "CCL3")

patchwork::wrap_plots(
    dimplot_better(combined_obj, group_by = "new_annot_clust") + NoLegend(),
    dimplot_better(combined_obj, group_by = "Ann_Level3") + NoLegend(),
    ncol = 2
)
celltype_top5_lymphoid <- list(
...
FeaturePlot(combined_obj, 
            features = c("TREM2", "FABP5", "GPNMB", "MS4A4A"))
DotPlot(combined_obj, 
            features = celltype_top5_lymphoid,
            group.by = "seurat_clusters") +
    RotatedAxis() +
    theme(axis.text.x = element_text(angle = 90, hjust = 1)) +
    ggtitle(str_c(group))
