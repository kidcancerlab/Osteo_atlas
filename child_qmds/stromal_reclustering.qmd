...
celltype_list <-
    list(patient_prim_normal_cells = list(Mesenchymal = c("^0$" = "Osteoblast",
                                                        "^1$" = "Osteoblast_Progenitor",
                                                        "^2$" = "Osteoblast_Activated",  #inflammatory
                                                        "^3$" = "Osteoblast_Activated",  #adipogenic
                                                        "^4$" = "pCAFs",   #proliferating
                                                        "^5$" = "Pericytes",  #myocaf
                                                        "^6$" = "myCAFs",   #matrix caf
                                                        "^7$" = "apCAFs",   #antigen presenting
                                                        "^8$" = "Chondrocytes",
                                                        "^9$" = "Osteoblast_Activated",
                                                        "^10$" = "Osteoblast",
                                                        "^11$" = "Osteoblast"),
                                         ...
                                         Immune_Myeloid = c("^0$" = "Inflam_TAMs",
                                                            ...
                                                            "^18$" = "pDC",
                                                            "^19$" = "Osteoclast_TAMs"),
                                         Epithelial_Endothelial = c("^0$" = "Endothelial_Activated",
...
human_prot <-
    read_tsv("output/human_protein_coding_genes.tsv", 
    col_names = FALSE)

parallel::mclapply(c("Mesenchymal",
                    "Immune_Lymphoid",
                    "Immune_Myeloid",
                    "Epithelial_Endothelial"), function(cell_type) {
    for (item in seq_len(nrow(combine_tumor_per_species))) {
        ...
        tree <- 
            clustree::clustree(clustree_sobj,
                                prefix = "RNA_snn_res.") +
            ggtitle(str_c(combined, " ", cell_type))

        ggsave(str_c("output/figures/tumor_vs_stroma/clustree/", combined, "_", cell_type, ".png"),
            width = 10,
            height = 10,
            plot = tree)
...
details <-
    tribble(~group1,                                  ~resolution,  
            "patient_prim_mets_Mesenchymal",            0.2,  
            "patient_prim_mets_Immune_Lymphoid",        0.3, 
            "patient_prim_mets_Immune_Myeloid",         1.4,
            "patient_prim_mets_Epithelial_Endothelial", 0.7, 
            "mouse_prim_mets_Mesenchymal",              0.3, 
            "mouse_prim_mets_Immune_Lymphoid",          0.8,
            "mouse_prim_mets_Immune_Myeloid",           0.8,
            "mouse_prim_mets_Epithelial_Endothelial",   0.3,
            "xeno_prim_mets_Mesenchymal",               0.4,
            "xeno_prim_mets_Immune_Lymphoid",           0.3,
            "xeno_prim_mets_Immune_Myeloid",            1.1,
            "xeno_prim_mets_Epithelial_Endothelial",    1.6,
            "dog_prim_mets_Mesenchymal",                0.8,
            "dog_prim_mets_Immune_Lymphoid",            0.3,
            "dog_prim_mets_Immune_Myeloid",             0.8,
            "dog_prim_mets_Epithelial_Endothelial",     0.3)
...
### Annotation of the cell types
```{r annotation_of_cell_types}
# now we can reluster the normals and annotate
...
parallel::mclapply(seq_len(nrow(details)), function(item) {
    ...
    for (names in unique(combined_obj$unique)){
        ...
        ggsave(str_c("output/figures/relustering_normals/",
                ...
                plot = dotplot2,
                width = length(unique(sub_object$ann_l3)) * 1.5,
                height = length(unique(sub_object$ann_l3)) * 0.5,
                limitsize = FALSE,
                bg = "white")
    }
}, mc.cores = parallel::detectCores(),
...
for (names in unique(combined_obj$unique)){
    ...
    sub_object$ann_l3 <-
        str_replace_all(sub_object$seurat_clusters,
                        celltype_annotation[[group]][[names]])
    list_sobj[[names]] <- sub_object
}
# Extract all IL and CXCL genes from the marker list that are likely inflammatory related




FeaturePlot(list_sobj$mm_prim, 
            features = infla)
DotPlot(list_sobj$mm_prim, 
            features = infla,
            group.by = "ann_l3",
            cols = "RdBu") +
    RotatedAxis() 

FeaturePlot(list_sobj$dogs_mets, 
            features = c("MARCO", "FABP4", "PPARG", "OLR1"))

patchwork::wrap_plots(
    dimplot_better(list_sobj$dogs_prim, group_by = "seurat_clusters") + NoLegend(),
    dimplot_better(list_sobj$dogs_mets, group_by = "ann_l3") + NoLegend()),
    dimplot_better(list_sobj$mm_mets, group_by = "new_id") + NoLegend(),
    dimplot_better(list_sobj$mm_mets, group_by = "new_annot_clust") + NoLegend(),
    ncol = 2
)
...
Idents(list_sobj$dogs_mets) <- "ann_l3"
markers <- 
    FindAllMarkers(object = list_sobj$dogs_mets,
                    logfc.threshold = 1,
                    min.pct = 0.1,
                    only.pos = T)
    # reduce data
...
pdc_markers <- c("BCL11A", "TCF4", "FCRLA", "SPATS2L")
neut <- c("S100A9", "CSF3R", "SELL", "SRGN")
neutrophil_markers <- c("S100A9", "CSF3R", "SELL", "CXCL8", "OLR1", "SRGN")

alveolar_macrophage_markers <- c("CHI3L1", "ALDH1A1", "ME1", "PPARG")


patchwork::wrap_plots(
    DotPlot(list_sobj$dogs_prim, 
            features = dog_prim_mets_Immune_Myeloid,
            group.by = "ann_l3",
            cols = "RdBu") +
    RotatedAxis(),
    DotPlot(list_sobj$dogs_mets, 
            features =dog_prim_mets_Immune_Myeloid,
            group.by = "ann_l3",
            cols = "RdBu") +
    RotatedAxis(),
