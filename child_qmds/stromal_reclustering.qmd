...
for (group in c("patient_prim", "patient_mets",
    ...
    if (group == "xeno_prim_human" || group == "xeno_prim_human" || 
        group == "xeno_prim_human" || group == "xeno_prim_human") {
...
#plots for tumor panel
tumor_plot_list <- list()
for (group in c("patient_prim_cancer_cells", "patient_mets_cancer_cells",
    ...
    tumor_plot <-
        dimplot_better(object, group_by = "Ann_Level3") +
            NoLegend() +
            ggtitle(str_c(group, " Ann_Level3"))

    tumor_plot_list[[group]] <- tumor_plot
}

tumor_panel_plot <- 
    patchwork::wrap_plots(plotlist = tumor_plot_list,
                        ncol = 2)

ggsave(filename = "output/figures/Final_Annotations/tumor_panel_plots.png",
        plot = tumor_panel_plot,
        width = 16,
        height = 24,
        limitsize = FALSE,
        bg = "white")
...
for (group in c("patient_prim_normal_cells", "patient_mets_normal_cells",
    ...
    stromal_plot_list[[group]] <- stromal_plot
}

stromal_panel_plot <- 
    cowplot::plot_grid(plotlist = stromal_plot_list,
                        ncol = 2)
