...
celltypes <-
     c("Osteoclast_TAMs", "Reg_TAMs", "Scar_TAMs",
                           "Inflam_TAMs", "Prolif_TAMs", "IFN_TAMs",
                             "Fibrogenic_TAMs", "TAMs")

new_obj <-
    subset(sobj_list$Immune_Myeloid,
           ann_l3 %in% celltypes)
Idents(new_obj) <- new_obj$seurat_clusters
markers_wanted<-
    FindAllMarkers(new_obj,
                logfc.threshold = 0.25,
                only.pos = T) %>%
    as.data.frame() %>%
    group_by(cluster) %>% # group by cluster
...
xyz <-
    markers_wanted %>% as.data.frame() %>% 
    group_by(cluster) 
write_tsv(xyz,
            "output/markers/ryanswants.tsv")

dimplot_better(new_obj, group_by = "seurat_clusters") +
    NoLegend()
...
Idents(sobjs$Immune_Myeloid) <- sobjs$Immune_Myeloid$ann_l3
markers <- 
    FindAllMarkers(object = sobjs$Immune_Myeloid,
                    min.pct = 0.5,
                    logfc.threshold = 0.25,
                    only.pos = T) %>%
    as.data.frame() %>%
...
for (cell_type in c("Mesenchymal",
                    "Immune_Lymphoid",
                    "Immune_Myeloid",
                    "Epithelial_Endothelial")) {
...
details <-
    tribble(~group1,                                  ~resolution,  
            ...
            "patient_prim_mets_Epithelial_Endothalial", , 
            "mouse_prim_mets_Mesenchymal",              0.3, 
            "mouse_prim_mets_Immune_Lymphoid",          0.8,
            "mouse_prim_mets_Immune_Myeloid",           ,
            "mouse_prim_mets_Epithelial_Endothalial",   ,
            "xeno_prim_mets_Mesenchymal",               ,
            "xeno_prim_mets_Immune_Lymphoid",           ,
            "xeno_prim_mets_Immune_Myeloid",            ,
            "xeno_prim_mets_Epithelial_Endothalial",    ,
            "dog_prim_mets_Mesenchymal",                ,
            "dog_prim_mets_Immune_Lymphoid",            ,
            "dog_prim_mets_Immune_Myeloid",             ,
            "dog_prim_mets_Epithelial_Endothalial",     
            )
...
parallel::mclapply(seq_len(nrow(details)), function(item) {
    ...
    ncol = 2)
    dimplot_better(combined_obj, group_by = "seurat_clusters", split_by = "unique")
