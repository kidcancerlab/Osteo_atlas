## Calculate Lee's statistic
```{r spatialee_calc_lee}
cell_types <-
    qs::qread("output/spacexr/granular_references/cell_groups.qs") %>%
    unlist() %>%
    as.vector()

# We'll add the output to this table
cell_combos <-
    combn(cell_types, 2, simplify = TRUE) %>%
    t() %>%
    as.data.frame()

nrow(cell_combos)

system(paste0(
    "sbatch --array=1-",
    nrow(cell_combos),
    " scripts/run_lee_perms.sh"
))
```

```{r spatialee_plot_all}
#| cache.vars: [make_lee_heatmaps, make_lower_triangle, lee_stats]
lee_stats <-
    read_tsv(
        list.files(
            "output/spacexr/granular_references/lee_perms",
            full.names = TRUE,
            pattern = "lee_perms_.*\\.tsv"
        ),
        show_col_types = FALSE
    ) %>%
    mutate(
        perm_p = if_else(
            perm_p == "p < 1e-05",
            "0.00001",
            perm_p
        ) %>%
            as.numeric(),
        fdr = p.adjust(perm_p, method = "fdr")
    )

make_lower_triangle <- function(lee_stats) {
    full_data <-
        lee_stats |>
        dplyr::full_join(
            lee_stats |>
                dplyr::mutate(
                    cell_type_1_temp = cell_type_2,
                    cell_type_2_temp = cell_type_1
                ) |>
                dplyr::select(-cell_type_1, -cell_type_2) |>
                dplyr::rename(
                    cell_type_1 = cell_type_1_temp,
                    cell_type_2 = cell_type_2_temp
                )
        )

    return(full_data)
}

# the order from te averaged out result of eight samples was used to make this list
use_order <- c(
    "myCAFs", "COMA", "MP_Progenitor", "Prolif_TAMs", "Inflam_TAMs", 
    "Basal_Progenitor", "Fibrogenic", "Endothelial_Capillary", "Interactive",
    "Fibrogenic_TAMs", "Osteoclast_TAMs", "Prolif_T", "Proliferative",
    "AT1", "Mast", "Capillary_Aerocyte", "Endothelial_Artery", "CD8_T",
    "Alv_Macrophages", "AT2", "Adv_Fibroblasts", "Endothelial_Vein",
    "Ciliated_cells", "Endothelial_Lymphatic", "Plasma_B", "Endothelial_Prolif", 
    "Endothelial_Activated", "Neuronal", "Scar_TAMs", "TAMs", "CD4_T",
    "apCAFs", "Naive_T", "DC2", "T_Reg", "DC1", "IFN_TAMs", "Memory_B",
    "pDC", "Pericytes", "Monocytes", "NK_cells"
) %>% 
    str_replace_all("_", " ")


make_lee_heatmaps <- function(lee_stats, ...) {
    sub_data <-
        lee_stats |>
        make_lower_triangle() |>
        mutate(cell_type_1 = stringr::str_replace_all(cell_type_1, "_", " "),
               cell_type_2 = stringr::str_replace_all(cell_type_2, "_", " "))

    # use_order <-
    #     lee_stats$cell_type_1 |>
    #     unique() |>
    #     sort()

    use_order <- use_order

    pheatmap_out <-
        sub_data |>
        dplyr::select(dplyr::all_of(c(
            "cell_type_1",
            "cell_type_2",
            "lee_stat"
        ))) |>
        tidyr::pivot_wider(
            names_from = "cell_type_2",
            values_from = "lee_stat",
            values_fill = NA
        ) |>
        dplyr::filter(cell_type_1 %in% use_order) |>
        dplyr::mutate(cell_type_1 = factor(cell_type_1, levels = use_order)) |>
        dplyr::arrange(cell_type_1) |>
        dplyr::select(cell_type_1, all_of(use_order)) |>
        tibble::column_to_rownames("cell_type_1") |>
        pheatmap::pheatmap(silent = TRUE,
                            cluster_rows = FALSE,
                            cluster_cols = FALSE,
                        ...)

    return()
}

lapply(
    unique(lee_stats$sample),
    function(x) {
        lee_stats %>%
            filter(sample == x) %>%
            make_lee_heatmaps(
                sample_name = x,
                file = paste0(
                    "output/figures/spatial/lees/", x, ".pdf"
                ),
                width = 10,
                height = 10,
                color = colorRampPalette(rev(RColorBrewer::brewer.pal(n = 7, name = "RdBu")))(100)
            )
    }
)


spat_pca <-
    lee_stats %>%
    filter(sample != "OS8_Seurat") %>%
    make_lower_triangle() %>%
    select(-perm_p, -fdr) %>%
    pivot_wider(
        names_from = sample,
        values_from = lee_stat,
        values_fill = 0
    ) %>%
    arrange(cell_type_1, cell_type_2) %>%
    mutate(label = paste(cell_type_1, cell_type_2)) %>%
    select(-cell_type_1, -cell_type_2) %>%
    column_to_rownames("label") %>%
    t() %>%
    prcomp()

clusters <-
    kmeans(spat_pca$x, 2)

spat_pca$x %>%
    as.data.frame() %>%
    rownames_to_column("sample") %>%
    full_join(
        clusters$cluster %>%
            as.data.frame() %>%
            rownames_to_column("sample") %>%
            rename(cluster = ".")
    ) %>%
    ggplot(aes(x = PC1, y = PC2, color = as.factor(cluster))) +
    geom_point() +
    ggrepel::geom_text_repel(aes(label = sample)) +
    theme_bw()

if (!dir.exists("output/figures/spatial/lees/")) {
    dir.create("output/figures/spatial/lees/",
               recursive = TRUE)
}
lee_stats %>%
    filter(sample != "OS8_Seurat") %>%
    #filter(sample %in% names(clusters$cluster[clusters$cluster == 2])) %>%
    group_by(cell_type_1, cell_type_2) %>%
    summarize(lee_stat = mean(lee_stat), .groups = "drop") %>%
    make_lee_heatmaps(
        file = "output/figures/spatial/lees/average.pdf",
        width = 10,
        height = 10,
        color = colorRampPalette(rev(RColorBrewer::brewer.pal(n = 7, name = "RdBu")))(100)
    )

order <-
    lee_stats %>%
    filter(sample != "OS8_Seurat") %>%
    #filter(sample %in% names(clusters$cluster[clusters$cluster == 2])) %>%
    group_by(cell_type_1, cell_type_2) %>%
    summarize(lee_stat = mean(lee_stat), .groups = "drop") %>%
    make_lower_triangle() %>%
    tidyr::pivot_wider(
            names_from = "cell_type_2",
            values_from = "lee_stat",
            values_fill = 1
    ) %>%
    column_to_rownames("cell_type_1") %>%
    as.matrix() %>%
    as.dist() %>%
    hclust(method = "complete")

# number of cluster and its members
tree <- cutree(order, k = 4)
groups <- 
    split(names(tree), tree)

names(groups) <- paste("Cluster", 1:4)

# use these names to make plots of groups sums

lee_stats %>%
    make_lower_triangle() %>%
    group_by(cell_type_1, cell_type_2) %>%
    summarize(lee_stat = mean(lee_stat)) %>%
    pivot_wider(names_from = "cell_type_2", values_from = "lee_stat") %>%
    write_tsv("output/spacexr/granular_references/lee_perms/average_lee.tsv")

lee_stats %>%
    make_lower_triangle() %>%
    group_by(cell_type_1, cell_type_2) %>%
    summarize(
        lee_stat = mean(lee_stat, na.rm = TRUE),
        n_signif = sum(fdr < 0.1),
        .groups = "drop"
    ) %>%
    mutate(
        group = if_else(
            n_signif > 6,
            if_else(
                lee_stat > 0,
                "Correlated",
                "Anti-correlated"
            ),
            "Not correlated"
        )
    ) %>%
    ggplot(
        aes(x = lee_stat)
    ) +
    geom_histogram(bins = 100) +
    facet_wrap(~group, ncol = 1)

lee_stats %>%
    make_lower_triangle() %>%
    group_by(cell_type_1, cell_type_2) %>%
    summarize(
        lee_stat = mean(lee_stat, na.rm = TRUE),
        n_signif = sum(fdr < 0.1),
        .groups = "drop"
    ) %>%
    mutate(
        group = if_else(
            n_signif > 6,
            if_else(
                lee_stat > 0,
                "Correlated",
                "Anti-correlated"
            ),
            "Not correlated"
        )
    ) %>%
    arrange() %>%
    write_tsv("output/spacexr/granular_references/lee_perms/average_lee_stats.tsv")
```

## Make plots with just a subset of types
```{r spatialee_plot_subset}
y_celltypes <-
    c(
        "Basal_Progenitor",
        "Fibrogenic",
        "Interactive",
        "MP_Progenitor",
        "Proliferative",
        "Fibrogenic_TAMs",
        "IFN_TAMs",
        "Osteoclast_TAMs",
        "Prolif_TAMs",
        "Scar_TAMs"
    )

colors_use <-
    colorRampPalette(rev(RColorBrewer::brewer.pal(n = 7, name = "RdBu")))(1000)

lee_stats %>%
    group_by(cell_type_1, cell_type_2) %>%
    summarize(
        lee_stat = mean(lee_stat),
        fdr = min(fdr),
        .groups = "drop"
    ) %>%
    dplyr::select(dplyr::all_of(c(
        "cell_type_1",
        "cell_type_2",
        "lee_stat"
    ))) |>
    make_lower_triangle() %>%
    tidyr::pivot_wider(
        names_from = "cell_type_2",
        values_from = "lee_stat",
        values_fill = NA
    ) %>%
    dplyr::filter(cell_type_1 %in% y_celltypes) %>%
    column_to_rownames("cell_type_1") %>%
    pheatmap::pheatmap(
        file = "output/figures/spatial/lees/subset_average.pdf",
        width = 15,
        height = 10,
        color = colors_use,
        cellwidth = 20,
        cellheight = 20,
        na_col = "darkgrey"
    )
```


## Make subplot of only the cell types within the tumor
```{r spatialee_plot_subset}
keep_celltypes <-
    c(
        "myCAFs",
        "COMA",
        "MP_Progenitor",
        "Prolif_TAMs",
        "Inflam_TAMs",
        "Basal_Progenitor",
        "Fibrogenic",
        "Endothelial_Capillary",
        "Interactive",
        "Fibrogenic_TAMs",
        "Osteoclast_TAMs",
        "Prolif_T",
        "Proliferative"
    )

celltype_order_x <-
    c(
        "myCAFs",
        "MP_Progenitor",
        "COMA",
        "Prolif_TAMs",
        "Inflam_TAMs",
        "Basal_Progenitor",
        "Fibrogenic",
        "Endothelial_Capillary",
        "Interactive",
        "Fibrogenic_TAMs",
        "Osteoclast_TAMs",
        "Prolif_T",
        "Proliferative"
    )

# I want zero to be white, but it's not centered, so I need to jump through some hoops
lee_vals <-
    lee_stats %>%
    group_by(cell_type_1, cell_type_2) %>%
    filter(
        sample != "OS8_Seurat" &
            cell_type_1 %in% keep_celltypes &
            cell_type_2 %in% keep_celltypes
    ) %>%
    summarize(
        lee_stat = mean(lee_stat),
        fdr = min(fdr),
        .groups = "drop"
    ) %>%
    pull(lee_stat)

red_colors_use <-
    colorRampPalette(
        rev(RColorBrewer::brewer.pal(n = 7, name = "Reds"))
    )(100)#(round(mean(lee_vals > 0) * 100))

blue_colors_use <-
    colorRampPalette(
        rev(RColorBrewer::brewer.pal(n = 7, name = "Blues"))
    )(100)#(round(mean(lee_vals < 0) * 100))

colors_use <- c(
    tail(blue_colors_use, n = round(mean(lee_vals < 0) * 100)),
    "#FFFFFF",
    head(rev(red_colors_use), n = round(mean(lee_vals > 0) * 100))
)

# stuff <- tibble(color_use = color_use,
#                 y = 1:length(color_use),
#                 x = 1:length(color_use))

# ggplot(stuff,
#        aes(y = x, x = 1, fill = color_use)) +
#        geom_tile() +
#        scale_fill_manual(values = color_use) +
#        theme(legend.position = "none") +
#        geom_text(aes(label = color_use))

breaks_use <-
    seq(min(lee_vals), max(lee_vals), length.out = length(colors_use) + 1)

zero_index <- which.min(abs(breaks_use))
breaks_use[zero_index] <- 0

lee_stats %>%
    group_by(cell_type_1, cell_type_2) %>%
    filter(
        sample != "OS8_Seurat" &
            cell_type_1 %in% keep_celltypes &
            cell_type_2 %in% keep_celltypes
    ) %>%
    summarize(
        lee_stat = mean(lee_stat),
        fdr = min(fdr),
        .groups = "drop"
    ) %>%
    dplyr::select(dplyr::all_of(c(
        "cell_type_1",
        "cell_type_2",
        "lee_stat"
    ))) |>
    make_lower_triangle() %>%
    tidyr::pivot_wider(
        names_from = "cell_type_2",
        values_from = "lee_stat",
        values_fill = NA
    ) %>%
    mutate(cell_type_1 = factor(cell_type_1, levels = keep_celltypes)) %>%
    arrange(cell_type_1) %>%
    column_to_rownames("cell_type_1") %>%
    select(all_of(celltype_order_x)) %>%
    pheatmap::pheatmap(
        file = "output/figures/spatial/lees/subset_tumor_average.pdf",
        width = 15,
        height = 10,
        cluster_rows = FALSE,
        cluster_cols = FALSE,
        color = colors_use,
        breaks = breaks_use,
        cellwidth = 20,
        cellheight = 20,
        na_col = "darkgrey"
    )
```




```{r spatiallee_prep_cell_cats}
qs::qread(
    "output/seurat_objects/final_tumor_vs_stroma/patient_mets.qs"
)@meta.data %>%
    select(starts_with("Ann_Level")) %>%
    distinct() %>%
    as_tibble() %>%
    write_tsv("output/human_met_cell_type_categories.tsv")
```


```{r spatiallee_graph}
lee_val_cutoff <- 0.1

prep_lee_stats <- function(lee_stats_table,
                           fdr_cutoff,
                           lee_val_cutoff) {
    lee_stats_table <-
        lee_stats_table |>
        make_lower_triangle() %>%
        left_join(
            read_tsv(
                "output/human_met_cell_type_categories.tsv",
                show_col_types = FALSE
            ) %>%
                rename(cell_type_1 = Ann_Level3)
        ) %>%
        arrange(cell_type_1, cell_type_2) %>%
        mutate(
            lee_stat =
                if_else(
                    fdr <= fdr_cutoff &
                        lee_stat >= lee_val_cutoff,
                    lee_stat,
                    0
                )
        )

    return(lee_stats_table)
}

make_adj_matrix <- function(lee_stats_table) {
    adjacency_matrix <-
        lee_stats_table %>%
        select(cell_type_1, cell_type_2, lee_stat) %>%
        pivot_wider(
            names_from = cell_type_1,
            values_from = lee_stat,
            values_fill = 0
        ) %>%
        column_to_rownames("cell_type_2") %>%
        as.matrix()

    adjacency_matrix <- adjacency_matrix[, rownames(adjacency_matrix)]

    return(adjacency_matrix)
}

make_edge_table <- function(adjacency_graph,
                            lee_stats_table) {
    graph_coords <-
        igraph::layout_with_kk(adjacency_graph) %>%
        as.data.frame() %>%
        as_tibble() %>%
        rename(x = "V1", y = "V2") %>%
        mutate(cell_type = rownames(adjacency_graph[]))

    edge_table <-
        lee_stats_table %>%
        left_join(
            rename(
                graph_coords,
                x_1 = x,
                y_1 = y,
                cell_type_1 = cell_type
            )
        ) %>%
        left_join(
            rename(
                graph_coords,
                x_2 = x,
                y_2 = y,
                cell_type_2 = cell_type
            )
        )

    return(edge_table)
}

make_network_plot_from_lee <- function(lee_stats_table,
                                       lee_val_cutoff = 0.1,
                                       fdr_cutoff = 0.1,
                                       plot_title = "") {
    lee_stats_table <-
        prep_lee_stats(
            lee_stats_table,
            fdr_cutoff = fdr_cutoff,
            lee_val_cutoff = lee_val_cutoff
        )

    adjacency_matrix <- make_adj_matrix(lee_stats_table)

    adjacency_graph <-
        igraph::graph_from_adjacency_matrix(
            adjacency_matrix,
            weighted = TRUE,
            diag = FALSE
        )

    edge_table <- make_edge_table(adjacency_graph, lee_stats_table)

    ggplot() +
        geom_segment(
            data = edge_table %>% filter(lee_stat != 0),
            aes(
                x = x_1,
                y = y_1,
                xend = x_2,
                yend = y_2,
                alpha = lee_stat,
                linewidth = lee_stat
            ),
            color = "darkblue"
        ) +
        geom_point(
            data = edge_table %>%
                select(cell_type_1, x_1, y_1, Ann_Level1) %>%
                distinct(),
            aes(
                x = x_1,
                y = y_1,
                color = Ann_Level1
            ),
            size = 5
        ) +
        ggrepel::geom_label_repel(
            data = edge_table %>%
                select(cell_type_1, x_1, y_1, Ann_Level1) %>%
                distinct(),
            aes(
                x = x_1,
                y = y_1,
                label = cell_type_1,
                fill = Ann_Level1
            ),
            size = 3,
            color = "white",
            label.size = 0,
            segment.colour = "black"
        ) +
        scale_color_manual(
            values = c(
                Epithelial_Endothelial = "#D43F3AFF",
                Immune_Lymphoid = "#EEA236FF",
                Immune_Myeloid = "#357EBDFF",
                Mesenchymal = "#5CB85CFF",
                Tumor = "#B8B8B8FF"
            )
        ) +
        scale_fill_manual(
            values = c(
                Epithelial_Endothelial = "#D43F3AFF",
                Immune_Lymphoid = "#EEA236FF",
                Immune_Myeloid = "#357EBDFF",
                Mesenchymal = "#5CB85CFF",
                Tumor = "#B8B8B8FF"
            )
        ) +
        labs(
            x = NULL,
            y = NULL,
            title = plot_title
        ) +
        theme_void() +
        theme(
            legend.position.inside = c(0.9, 0.9)
        )
}

pdf(
    "output/figures/spatial/lees/lee_sample_graphs.pdf",
    width = 15,
    height = 12
)
lapply(
    unique(lee_stats$sample),
    function(x) {
        lee_stats %>%
            filter(sample == x) %>%
            make_network_plot_from_lee(
                lee_val_cutoff = lee_val_cutoff,
                fdr_cutoff = 0.1,
                plot_title = x
            )
    }
)
dev.off()

averaged_lee_graph <-
    lee_stats %>%
    group_by(cell_type_1, cell_type_2) %>%
    summarize(
        lee_stat = mean(lee_stat),
        fdr = max(fdr),
        .groups = "drop"
    ) %>%
    make_network_plot_from_lee(
        lee_val_cutoff = lee_val_cutoff,
        fdr_cutoff = 0.1,
        plot_title = "Average Across All Samples"
    )

ggsave(
    "output/figures/spatial/lees/lee_sample_average_graph.pdf",
    width = 15,
    height = 12
)
```


## Sum of environments to show specific neighborhoods of interest
```{r spatialee_sum_environments}

# the order from te averaged out result of eight samples was used to make this list
use_order <- c(
    "myCAFs", "COMA", "MP_Progenitor", "Prolif_TAMs", "Inflam_TAMs", 
    "Basal_Progenitor", "Fibrogenic", "Endothelial_Capillary", "Interactive",
    "Fibrogenic_TAMs", "Osteoclast_TAMs", "Prolif_T", "Proliferative",
    "AT1", "Mast", "Capillary_Aerocyte", "Endothelial_Artery", "CD8_T",
    "Alv_Macrophages", "AT2", "Adv_Fibroblasts", "Endothelial_Vein",
    "Ciliated_cells", "Endothelial_Lymphatic", "Plasma_B", "Endothelial_Prolif", 
    "Endothelial_Activated", "Neuronal", "Scar_TAMs", "TAMs", "CD4_T",
    "apCAFs", "Naive_T", "DC2", "T_Reg", "DC1", "IFN_TAMs", "Memory_B",
    "pDC", "Pericytes", "Monocytes", "NK_cells"
)

Inside_tumor <-
    c("myCAFs", "COMA", "MP_Progenitor", "Prolif_TAMs", "Inflam_TAMs", 
    "Basal_Progenitor", "Fibrogenic", "Endothelial_Capillary", "Interactive",
    "Fibrogenic_TAMs", "Osteoclast_TAMs", "Prolif_T", "Proliferative")

Normal_alveoli <-
    c("AT1", "Mast", "Capillary_Aerocyte", "Endothelial_Artery", "CD8_T",
    "Alv_Macrophages", "AT2", "Adv_Fibroblasts", "Endothelial_Vein")

Tumor_periphery <-
    c("Ciliated_cells", "Endothelial_Lymphatic", "Plasma_B", "Endothelial_Prolif", 
    "Endothelial_Activated", "Neuronal", "Scar_TAMs", "TAMs")

Immune_cells <-
    c("CD4_T", "apCAFs", "Naive_T", "DC2", "T_Reg", "DC1", "IFN_TAMs", "Memory_B",
    "pDC", "Pericytes", "Monocytes", "NK_cells")


# plot the rctd results
spatial_list <-
    qs::qread("output/spatial_objects/spatial_list_with_TAECs.qs")

for (ob_name in names(spatial_list)) {
    for (region in c("Inside_tumor", "Normal_alveoli", "Tumor_periphery", "Immune_cells")) {
        spatial_list[[ob_name]]@meta.data[[region]] <-
            rowSums(
                spatial_list[[ob_name]]@meta.data %>%
                    select(all_of(get(region)))
            )
    }
}


size.factors <- list("OS1_Seurat" = 1.5,
                     "OS2_Seurat" = 1.4,
                     "OS3_Seurat" = 2.7,
                     "OS4_Seurat" = 1.7,
                     "OS5_Seurat" = 2.5,
                     "OS6_Seurat" = 2.5,
                     "OS7_Seurat" = 2.5,
                     "OS8_Seurat" = 1.3)


for (ob in names(spatial_list)) {
    hne <- 
        SpatialDimPlot(spatial_list[[ob]],
                       pt.size.factor = 0) +
        NoLegend()
    plts <- list("h_and_e" = hne)
    # plts <- list()
    for (site in c("Inside_tumor", "Normal_alveoli", "Tumor_periphery", "Immune_cells")) {
        plts[[site]] <-
            SpatialFeaturePlot(spatial_list[[ob]],
                                features = site,
                                stroke = 0,
                                pt.size.factor = size.factors [[ob]],
                                image.alpha = 0) +
            theme(legend.title.position = "top",
                    legend.title.size = 2,
                  legend.key.height = unit(0.3, "cm"),
                  legend.key.width = unit(0.9, "cm"))

    }
    combine_plot <-
        wrap_plots(grobs = plts,
                    nrow = 1) + plot_annotation(title = ob)

    if (!dir.exists("output/figures/spatial")) {
        dir.create("output/figures/spatial")
    }

    ggsave(
        filename = paste0("output/figures/spatial/", ob, "_neighbourhood.png"),
        plot = combine_plot,
        width = 25,
        height = 7,
        limitsize = FALSE
    )
}


```