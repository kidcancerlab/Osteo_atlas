...
for (subpop in unique(combine_all$Ann_Level3)) {
    org <- "human"
    ...
    #save the regulon activity
    if (!dir.exists(str_c("output/figures/regulon_analysis/human_only/", subpop))) {
        dir.create(str_c("output/figures/regulon_analysis/human_only/", subpop),
            recursive = TRUE)
    }
    saveRDS(scenicOptions,
            file= str_c("output/figures/regulon_analysis/human_only/", subpop, "/scenicOptions.rds"))
    qs::qsave(scenicOptions,
              str_c("output/figures/regulon_analysis/human_only/", subpop, "/try_scenicOptions.qs"))
    #save the regulon activity
    qs::qsave(regulon_activity_scaled,
              str_c("output/figures/regulon_analysis/human_only/",
                    subpop,
                    "/regulon_activity_scaled.qs"))
    #save the regulon activity
    write_tsv(as.data.frame(regulon_activity_scaled),
...
#     ggsave(str_c(filename = "output/figures/regulon_analysis/human_only/",
#                             subpop,
#                             "/average_reg_heatmap.png"),
#             plot = average_reg_heatmap,
#             width = plot_width,
...
GRA_output <-
    Run_GRA(sobject = combine_all,
            ...
            org = "hgnc",
            subset = "yes",
            subset_cell_number = min(2000, ncol(combine_all)))
...
#save the regulon activity
if (!dir.exists(str_c("output/figures/regulon_analysis/human_only/", "prim_mets_combined"))) {
    dir.create(str_c("output/figures/regulon_analysis/human_only/", "prim_mets_combined"),
        recursive = TRUE)
}
#save the regulon activity
qs::qsave(regulon_activity_scaled,
...
qs::qsave(scenicOptions,
            str_c("output/figures/regulon_analysis/human_only/", "prim_mets_combined", "/scenicOptions.qs"))

saveRDS(scenicOptions,
        file= str_c("output/figures/regulon_analysis/human_only/", "prim_mets_combined", "/scenicOptions.rds"))
...
for (subpop in unique(combine_all$Ann_Level3)) {
    ...
    scenicOptions <-
        qs::qread(str_c("output/figures/regulon_analysis/human_only/",
                        subpop,
                        "/scenicOptions.qs"))
    ...
    # cell that are not in
    non_matching_cells <- 
        auc_matrix %>%
        filter(!cellID %in% metadata_cellID$cellID) %>%
        pull(cellID)
    Ann_level3_cells_not_matching <- 
        combine_all@meta.data %>%
        rownames_to_column("cellID") %>%
        select(cellID, Ann_Level3) %>%
        filter(cellID %in% non_matching_cells)
    ...
    # find the direction which side is higher vs lower
    direction <- 
        long_df %>%
        group_by(feature, unique) %>%
        summarise(median_value = median(value), .groups = "drop") %>%
        tidyr::pivot_wider(names_from = unique, values_from = median_value) %>%
        mutate(direction = case_when(
            patient_prim > patient_mets ~ "higher_in_prim",
            patient_mets > patient_prim ~ "higher_in_mets",
            TRUE ~ "no_change")) %>%
        filter(feature %in% sig_features$feature) %>%
        select(feature, direction)

    # subset for only significant features and add the direction
    sig_features_table <- 
        sig_features %>%
        left_join(direction, by = "feature")
    ...
    box_plot <-
        ggplot(plot_df, aes(x = reorder(feature, value, FUN = median, decreasing = TRUE),
                            y = value, 
                            fill = factor(group, levels = c("Primary", "Metastatic")))) +
        geom_boxplot(outlier.shape = NA) +
        geom_jitter(aes(color = factor(group, levels = c("Primary", "Metastatic"))), 
        ...
        labs(
            title = str_c("Significant Features in ", subpop, " Primary vs Metastasis"),
            x = "Regulon",
            y = "Regulon activity / feature value",
            fill = "Group",
            color = "Group"
        ) +
        theme_minimal() +
    ...
    ggsave(str_c("output/figures/regulon_analysis/human_only/", subpop, "/box_plot.png"),
        plot = box_plot,
        width = 12,
        height = 6,
        bg = "white")
    ...
    #make tile plot
    mat <- 
        as.matrix(data_tile_plot[, c("Primary", "Metastatic")])
    # mat_scaled <- 
    #     t(scale(t(mat)))
    ...
    ggsave(str_c("output/figures/regulon_analysis/human_only/", subpop, "/heatmap.png"),
        plot = heatmap,
        width = 4,
        height = 8)
    ...
    df_long <- 
        ...
        as.data.frame() %>%
        rownames_to_column("Regulon") %>%
        pivot_longer(cols = c(Primary, Metastatic),
                    names_to = "Group",
                    values_to = "Activity")

    # tile plot
    tile_plot <-
        ggplot(df_long, aes(Group, Regulon, fill = Activity)) +
        geom_tile() +
        scale_fill_viridis_c() +
        theme_classic() +
        geom_point(aes(size = -log10(p_adj)))

    ggsave(str_c("output/figures/regulon_analysis/human_only/", subpop, "/tile_plot.png"),
        plot = tile_plot,
        width = 4,
        height = 8)
    ...
    data_tile_plot$logFC <- 
        log2(data_tile_plot$Metastatic / data_tile_plot$Primary)

    volcano_plot <-
        ggplot(data_tile_plot,
            aes(logFC, -log10(p_adj), label = rownames(data_tile_plot))) +
        geom_point() +
        geom_text_repel() 

    ggsave(str_c("output/figures/regulon_analysis/human_only/", subpop, "/volcano_plot.png"),
        plot = volcano_plot,
        width = 6,
        height = 6,
        bg = "white")
...
## figs for new analysis 
```{r figs_for_new_analysis}
...
files <- 
    list.files(path = "output/figures/regulon_analysis/human_only",
                recursive = TRUE,
                # full.names = TRUE,
                pattern = "scaled.tsv")

compiled_regulon_activity <- list()

for (file in files) {
    group_name <- 
        str_remove(file, "/regulon_activity_scaled.tsv")

    # Read the TSV file into a data frame
    regulon_activity_scaled <- 
        read_tsv(str_c("output/figures/regulon_analysis/human_only/", file))
    
    # add to the list
    compiled_regulon_activity[[group_name]] <- regulon_activity_scaled 
...
# make a heatmap for the combined data
prim_mets_combined <-
    compiled_regulon_activity[["prim_mets_combined"]] %>%
    as.data.frame() %>%
    # rownames_to_column("Regulon") %>%
    mutate(Regulon = str_replace(Regulon, " .*", "")) %>%
    mutate(Regulon = str_replace(Regulon, "_.*", "")) %>%
    ...
    column_to_rownames("Regulon") %>%
    as.matrix()
# heatmap to show average regulon activity per cell type or cluster
average_reg_heatmap <-
    pheatmap::pheatmap(prim_mets_combined,
                    name = "Regulon activity",
                    scale = "none",
                    cluster_cols = TRUE,
                    cluster_rows = TRUE,
...
# subpopulation prim vs mets
subpopulations <- 
    c("Basal_Progenitor", "Proliferative", "MP_Progenitor", "Fibrogenic", "Interactive", "COMA")

# remove the data that is not subpopulation
regulon_output <-
    compiled_regulon_activity[names(compiled_regulon_activity) %in% subpopulations]

plot(plot_data$patient_prim, plot_data$patient_mets)
abline(a=0, b=1, col="red")



for (name in names(regulon_output)) {
    plot_data <-
        regulon_output[[name]] %>%
        as.data.frame() %>%
        mutate(Regulon = str_replace(Regulon, " .*", "")) %>%
    ...
    ggsave(str_c("output/figures/regulon_analysis/human_only/",
                name, "/regulon_subpop.png"),
        plot = heatplot,
        width = 4,
        height = 10)
}
...
regulon_output <-
    regulon_output %>%
    bind_rows(.id = "Subpopulation") %>%
    mutate(Regulon = str_replace(Regulon, " .*", "")) %>%
    mutate(Regulon = str_replace(Regulon, "_.*", "")) %>%
    mutate(Regulon = str_replace(Regulon, "\\(.*\\)", "")) %>%
    pivot_wider(names_from = Subpopulation, values_from = -Regulon) %>%
    column_to_rownames("Regulon") 
