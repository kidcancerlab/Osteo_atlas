...
Idents(combine_all) <- combine_all$Ann_Level3
VlnPlot(combine_all,
        features = c("UNC5B", "UNC5C", "FLRT2"),
        group.by = "unique",
         pt.size = 0)

auc <- 
    loadInt(GRA_output$scenicOptions, "aucell_regulonAUC") 
...
results_list <- lapply(unique(long_df$Ann_Level3), function(Annotation) {
    long_df$comparison_col <- 
        if_else(long_df$Ann_Level3 == Annotation, "group1", "group2")
    ...
    return(list(Annotation = Annotation, sig_features = sig_features))
})



# give me a simulated data that has 20 regulons and 6 subpopulations, 
#with random values for the regulon activity, and their padj value,
#and then make a heatmap of it using pheatmap, with the same color scheme as before
# and with the same annotations for the columns as before, and with the same clustering options as before.
dataset <-
    expand.grid(feature = paste0("Regulon", 1:20),
                subpopulation = c("Fibrogenic", "Proliferative", "Interactive",
                                 "MP_Progenitor", "Basal_Progenitor", "COMA")) %>%
    mutate(
        value = case_when(
            # Shared regulons (1-5): high across all subpopulations
            as.numeric(gsub("Regulon", "", feature)) <= 5 ~ 
                runif(n(), min = 0.6, max = 0.9),
            # Fibrogenic-specific (6-10)
            as.numeric(gsub("Regulon", "", feature)) %in% 6:10 & subpopulation == "Fibrogenic" ~ 
                runif(n(), 0.7, 0.9),
            # Proliferative-specific (11-15)
            as.numeric(gsub("Regulon", "", feature)) %in% 11:15 & subpopulation == "Proliferative" ~ 
                runif(n(), 0.7, 0.9),
            # Interactive-specific (16-20)
            as.numeric(gsub("Regulon", "", feature)) %in% 16:20 & subpopulation == "Interactive" ~ 
                runif(n(), 0.7, 0.9),
            TRUE ~ runif(n(), min = 0, max = 0.3)
        ),
        p_adj = if_else(value > 0.5, runif(n(), min = 0, max = 0.05),
                        runif(n(), min = 0.05, max = 0.5))
    )

# plot a heatmap of the dataset using pheatmap, with the same color scheme as before, and with the same annotations for the columns as before, and with the same clustering options as before.
heatmap_data <-
    dataset %>%
    select(feature, subpopulation, value) %>%
