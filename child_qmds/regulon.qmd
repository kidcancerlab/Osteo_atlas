...
bubble_plot <- 
    ggplot(combined_regulon_data %>% 
           ...
           ungroup() %>%
           mutate(subpopulation = str_replace_all(subpopulation, "_", " ")) %>%
...
ggsave("output/SCENIC/prim_vs_mets_bubble_plot.pdf",
       plot = bubble_plot,
       width = 6,
       height = 15,
       bg = "white")
...
final_plot <-
    ...
    scale_size_continuous(range = c(4, 7), name = "-log10(p_adj)") +
    theme_minimal() +
    theme(axis.text.x = element_text(angle = 45, hjust = 1))
...
sig_regulon_auc %>%
    ...
    mutate(Ann_Level3 = str_replace_all(Ann_Level3, "_", " ")) %>%
    mutate(p_adj = if_else(is.na(p_adj), 0, p_adj)) %>%
    mutate(scaled_auc = if_else(p_adj == 0, NA_real_, scaled_auc)) %>%
    ggplot(aes(x = Ann_Level3, y = feature)) +
    geom_point(aes(size = p_adj,
                    fill = scaled_auc),
                shape = 21) +
    scale_fill_gradient2(low = "#0571B0", mid = "white", high = "#CA0020", 
                        name = "Scaled AUC") +
    scale_size_continuous(range = c(3, 7), name = "-log10(p_adj)") +
    theme_minimal() +
    theme(axis.text.x = element_text(angle = 45, hjust = 1),
          panel.grid.major = element_line(size = 0.2, color = "gray90"))
