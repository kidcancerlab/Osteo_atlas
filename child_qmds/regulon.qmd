...
# perform DEGs and GSEA TAMS GSEA TAMS
...
for (i in seq_len(nrow(object_list))) {
    ...
    #         as.data.frame() %>%
    #         rownames_to_column("Regulon") %>%
    #         write_tsv(str_c("output/figures/regulon_analysis/",
    #                     group,
    #                     "/regulon_activity_scaled.tsv"))
    object_big <-
...
# make a heatmap to show average regulon activity per cell type or cluster
    ...
    # get regulons that are not shared between the groups)
    # get regulons that are specific and unique to each site
    output_not_shared_TF <- 
        !(rownames(output) %in% rownames(output_shared))
    # Order columns: all prim_* first (in their subpopulation order), then mets_* (same order)
    subpop_order <- 
        c("Interactive", "Fibrogenic", "COMA", "MP_Progenitor", "Proliferative", "Basal_Progenitor")
    prim_cols <- 
        unlist(lapply(subpop_order, 
            function(x) grep(str_c("prim(_human)?_", x), colnames(output), value = TRUE)))
    mets_cols <- 
        unlist(lapply(subpop_order, 
                     function(x) grep(str_c("mets(_human)?_", x), colnames(output), value = TRUE)))
    ordered_cols <- 
        c(prim_cols, mets_cols)
    output_not_shared <-
        output[output_not_shared_TF, ordered_cols, drop = FALSE]

    # make a heatmap to show average regulon activity per cell type or cluster
    regulon_activity_heatmap_not_shared <-
        pheatmap::pheatmap(output_not_shared,
                            name = "Regulon activity",
                            scale = "none",
                            annotation_col = col_colors_df,
                            annotation_colors = col_colors,
                            ...
                            show_row_dend = FALSE,
                            silent = TRUE,
                            main = str_c(group3, " Regulon activity site-specific"))
    ggsave(str_c("output/figures/Final_Annotations/results2/suppliment/regulons/",
                group3, "_regulon_unshared.pdf"),
        plot = regulon_activity_heatmap_not_shared,
        width = 10,
        height = 8)
    ggsave(str_c("output/figures/Final_Annotations/results2/suppliment/regulons/",
                group3, "_regulon_unshared.png"),
        plot = regulon_activity_heatmap_not_shared,
        width = 10,
        height = 8)
...
# make a heatmap to show average regulon activity per cell type or cluster
    ...
    regulon_activity_heatmap_not_shared <-
        pheatmap::pheatmap(output_not_shared,
                            ...
                            show_row_dend = FALSE,
                            silent = TRUE,
                            main = str_c(group3, " Regulon activity (not shared)"))
}

```


## Ryans Cancer discovery
```{r ryans_cancer_discovery, eval = FALSE}
...
# we need to set a main directory first that is above all others
working_dir <- getwd()
# set the path for the motif data
path_for_motif_data <-
    "/gpfs0/home2/gdrobertslab/lab/Analysis/Yogesh/CellTypeAnnRefs/input/downloads/SCENIC"

for (group in 
    # unique(combine_all$Ann_Level3)) 
    c("Basal_Progenitor", "COMA"))
    {
    ...
    Idents(combine_all) <- combine_all$Ann_Level3
    object <- 
        subset(combine_all,
               Ann_Level3 == group)
    set.seed(199820)
    sobject <-
        object %>%
        subset(cells = sample(Cells(object),
                        min(5000, length(Cells(object)))))
    Idents(sobject) <- sobject$unique
    org = "hgnc"
...
## i need to comment this to avoid mistake running this again, it takes a week
# GRA_output <-
#     Run_GRA(sobject = combine_all,
#             idents = "Ann_Level3",
#             group = "prim_mets_combined",
#             org = "hgnc",
#             dbDir = "/gpfs0/home2/gdrobertslab/lab/Analysis/Yogesh/CellTypeAnnRefs/input/downloads/SCENIC",
#             subset = "yes",
#             subset_cell_number = 50000)

# set the working directory back to the original directory after the run is complete.
setwd(working_dir)

setwd(str_c("output/SCENIC/", "prim_mets_combined"))

scenicOptions <- 
    qs::qread(str_c("prim_mets_combined", "_scenicOptions.qs"))

auc <-
    loadInt(scenicOptions, "aucell_regulonAUC")


```

## Stats and figures for the new analysis

```{r sample_by_sample_regulon}
...
for (group in 
    # unique(combine_all$Ann_Level3)) 
    c("Basal_Progenitor", "COMA", "MP_Progenitor", "Proliferative", "Interactive", "Fibrogenic"))
    {
    ...
    object <- 
        subset(combine_all,
               Ann_Level3 == group)
    Idents(object) <- object$unique

    scenicOptions <-
        qs::qread(str_c(group, "_scenicOptions.qs"))
    ...
    # check the filtering step, its not making sense, its not 200 cell anymore
    metadata_cellID <-
        object@meta.data %>%
        rownames_to_column("cellID") %>%
        dplyr::select(cellID, sample_name, unique)

    table(metadata_cellID$cellID %in% auc_matrix$cellID)  # the filtering step is not making sense, its not 200 cell anymore

    joined_meta <-
        metadata_cellID %>%
        filter(cellID %in% auc_matrix$cellID) %>%
        left_join(auc_matrix, by = "cellID") %>%
        # remove all things after the first space in the regulon name
        rename_with(~ str_replace(., " .*", "")) %>%
        group_by(unique, sample_name) %>%
        summarise(across(-c(cellID), mean), .groups = "drop", number_of_cells = n())

    long_df <- 
        joined_meta %>%
        pivot_longer(
            cols = -c(unique, sample_name, number_of_cells),
            names_to = "feature",
            values_to = "value"
        )

    # perform wilcox test for each feature between the two groups
    wilcox_results <- 
        long_df %>%
        group_by(feature) %>%
        summarise(
            p.value = wilcox.test(value ~ unique)$p.value,
            statistic = wilcox.test(value ~ unique)$statistic,
            .groups = "drop"
        )
    ...
    # filter by padjusted p-value < 0.01
    sig_features_table <- 
        wilcox_results %>%
        filter(p_adj < 0.05)
    ...
    plot_df <- 
        long_df %>%
        filter(feature %in% sig_features_table$feature) %>%
        mutate(group = factor(recode(unique, patient_prim = "Primary", patient_mets = "Metastatic")))
    
    if (!dir.exists("plots")) {
        dir.create("plots", recursive = TRUE)
    }
    # ggsave(str_c("plots/box_plot.png"),
    #     plot = box_plot,
    #     width = 12,
    #     height = 6,
    #     bg = "white")
    ...
    data_tile_plot <-
        ...
        left_join(sig_features_table, by = "feature") %>%
        select(feature,  p_adj, Primary, Metastatic) %>%
        column_to_rownames("feature") 

    # log2 fold change volcano plat
    data_tile_plot <-
        as.data.frame(data_tile_plot) %>%
        mutate(across(c(Primary, Metastatic), ~. + 0.0001))

    data_tile_plot$log2FC <- 
        log2(data_tile_plot$Metastatic / data_tile_plot$Primary)
    ...
    tileplot_with_fc <-
        data_tile_plot %>%
        rownames_to_column("Regulon") %>%
        select(Regulon, log2FC, p_adj)
    
    regulon_list[[group]] <- tileplot_with_fc

    tile_plot_fc <-
        ggplot(tileplot_with_fc, aes(x = "Group", y = Regulon, fill = log2FC, size = -log10(p_adj))) +
        geom_point(shape = 21) +
        scale_fill_gradient2(low = "blue", mid = "white", high = "red", name = "log2 FC") +
        scale_size_continuous(name = "-log10(p_adj)", range = c(4, 10)) +
        theme_minimal() +
    ...
    ggsave(str_c("plots/tile_plot_fc.png"),
        plot = tile_plot_fc,
        width = 4,
        height = 8,
        bg = "white")
...
# fake data creation for the tile plot with log2 fold change and p-value

# Option 1: Combined volcano plot with facets (one per subpopulation)
volcano_combined <- function(data_list, subpop_names) {
    combined_data <- 
        Map(function(x, name) {
            x %>% mutate(subpopulation = name)
        }, data_list, subpop_names) %>%
        bind_rows()
    
    ggplot(combined_data, aes(log2FC, -log10(p_adj), color = log2FC)) +
...
# Option 2: Bubble heatmap (subpopulations as columns, regulons as rows)
bubble_heatmap <- function(data_list, subpop_names) {
    combined_data <- 
        Map(function(x, name) {
            x %>% mutate(subpopulation = name)
        }, data_list, subpop_names) %>%
    ...
    ggplot(combined_data, aes(subpopulation, Regulon, 
                                                        fill = log2FC, size = -log10(p_adj))) +
        geom_point(shape = 21, color = "black", stroke = 0.5) +
        scale_fill_gradient2(low = "blue", mid = "white", high = "red", 
                                                limits = c(-5, 5), name = "log2 FC") +
        scale_size_continuous(name = "-log10(p_adj)", range = c(2, 8)) +
        theme_minimal() +
        theme(axis.text.x = element_text(angle = 45, hjust = 1))
}

# Option 3: Focused heatmap (sig regulons only, p_adj < 0.05)
heatmap_sig_only <- function(data_list, subpop_names) {
    combined_data <- 
        Map(function(x, name) {
            x %>% filter(p_adj < 0.05) %>% mutate(subpopulation = name)
        }, data_list, subpop_names) %>%
        bind_rows() %>%
        pivot_wider(names_from = subpopulation, values_from = log2FC, 
                                values_fill = 0) %>%
        column_to_rownames("Regulon") %>%
        as.matrix()
...
combined_regulon_data <- 
    regulon_list %>%
    imap_dfr(~.x %>% mutate(subpopulation = .y)) %>%
    mutate(Regulon = str_replace_all(Regulon, "_", " "))

getwd()


# Create bubble heatmap with log2FC and p_adj
bubble_plot <- 
    ggplot(combined_regulon_data %>% 
           group_by(Regulon) %>%
           mutate(total_log2FC = sum(log2FC)) %>%
           ungroup() %>%
           arrange(desc(total_log2FC)), 
           aes(x = subpopulation,
              y = fct_reorder(Regulon, total_log2FC, .desc = FALSE), 
              color = log2FC,
              size = -log10(p_adj))) +
    geom_point() +
    scale_color_gradient2(low = "#0571B0", mid = "white", high = "#CA0020", 
                         name = "log2 FC") +
    scale_size_continuous(name = "-log10(p_adj)", range = c(3, 7)) +
    theme_minimal() +
    theme(axis.text.x = element_text(angle = 45, hjust = 1),
          panel.grid.major = element_line(size = 0.2, color = "gray90"),
          plot.title = element_text(hjust = 0.5)) +
    labs(title = "Regulon Activity Across Subpopulations",
         x = "Subpopulation", y = "Regulon")

ggsave("output/SCENIC/prim_vs_mets_bubble_plot.png",
       plot = bubble_plot,
       width = 7,
       height = 15,
       bg = "white")
...
# plots for combined sibpopulation regulon activity

```{r plot_groups_regulon, eval = FALSE}
...
# check the filtering step, its not making sense, its not 200 cell anymore
metadata_cellID <-
    combine_all@meta.data %>%
    rownames_to_column("cellID") %>%
    dplyr::select(cellID, sample_name, Ann_Level3)

table(metadata_cellID$cellID %in% auc_matrix$cellID) 
...
joined_meta <-
    ...
    # remove all things after the first space in the regulon name
    rename_with(~ str_replace(., " .*", "")) %>%
    group_by(Ann_Level3, sample_name) %>%
    summarise(across(-c(cellID), mean), .groups = "drop", number_of_cells = n())

long_df <- 
    joined_meta %>%
    pivot_longer(
        cols = -c(Ann_Level3, sample_name, number_of_cells),
        names_to = "feature",
        values_to = "value"
    )
...
for (Annotation in unique(long_df$Ann_Level3)) {
    long_df$comparison_col <- 
        if_else(long_df$Ann_Level3 == Annotation, "group1", "group2")
    ...
    wilcox_results <- 
        ...
        group_by(feature) %>%
        summarise(
            p.value = wilcox.test(value ~ comparison_col)$p.value,
            statistic = wilcox.test(value ~ comparison_col)$statistic,
            .groups = "drop"
        )
    ...
    sig_features_list[[Annotation]] <- sig_features
...
dataset_plot <- 
    ...
    select(feature, p_adj, subpopulation)


# plot
ggplot(dataset_plot, aes(x = subpopulation, y = feature)) +
  geom_point(aes(size = p_adj)) +
  scale_color_gradient2(low = "blue", mid = "white", high = "red") +
  theme_minimal() +
  theme(axis.text.x = element_text(angle = 45, hjust = 1))


# extract the average AUCell for each significant regulon
sig_regulon_auc <-
    long_df %>%
    group_by(feature, Ann_Level3) %>%
    summarise(mean_auc = mean(value), .groups = "drop") %>%
    filter(feature %in% dataset_plot$feature) %>%
    group_by(feature) %>%
    mutate(scaled_auc = scale(mean_auc)) %>%
    ungroup()
...
order <- 
    sig_regulon_auc %>%
    select(feature, Ann_Level3, mean_auc) %>%
    pivot_wider(names_from = Ann_Level3, values_from = mean_auc) %>%
    column_to_rownames("feature") %>%
    as.matrix() %>%
...
real_order <-
    c("E2F7_extended", "E2F1", "PBX3", "YY1_extended", "YY1", "MAZ_extended", "MAZ",
    "ETV6_extended", "MAFB_extended", "ETS1_extended", "SPI1_extended", "SPI1", "IKZF1_extended",
   "CREB5_extended", "FOSB_extended", "FOSB", "FOS_extended",
    "STAT3", "STAT1_extended", "ATF3_extended",
    "CEBPB", "CEBPB_extended", "CEBPD_extended",  "MEIS1_extended",
    "ZEB1", "PBX1_extended", "HNF4G_extended") %>%
    str_replace_all("_", " ")

plot_tile <- 
    sig_regulon_auc %>%
    mutate(feature = str_replace_all(feature, "_", " ")) %>%
    mutate(feature = factor(feature, levels = real_order)) %>%
    ggplot() +
    geom_tile(aes(x = Ann_Level3, y = feature, fill = scaled_auc)) +
    # geom_text(aes(x = Ann_Level3, y = feature, label = round(mean_auc, 3))) +
    scale_fill_gradient(low = "white", high = "firebrick3") +
    theme_minimal() +
    geom_tile(data = dataset_plot,
              aes(x = subpopulation, y = feature),
              color = "black",
              linewidth = .5, fill = NA) +
    scale_fill_gradient2(low = "blue", mid = "white", midpoint = 0, high = "firebrick3") +
    theme(axis.text.x = element_text(angle = 45, hjust = 1)) 


ggplot(sig_regulon_auc, aes(x = Ann_Level3, y = feature)) +
  geom_point(aes(size = mean_auc, color = scaled_auc)) +
  scale_color_gradient2(low = "blue", mid = "white", high = "red") +
  theme_minimal() +
  theme(axis.text.x = element_text(angle = 45, hjust = 1))
...
# Make plot using padj from the dataset_plot and mean_auc from the sig_regulon_auc
final_plot <-
    sig_regulon_auc %>%
    left_join(dataset_plot, by = c("feature" = "feature", "Ann_Level3" = "subpopulation")) %>%
    mutate(feature = str_replace_all(feature, "_", " ")) %>%
    mutate(feature = factor(feature, levels = real_order)) %>%
    mutate(Ann_Level3 = str_replace_all(Ann_Level3, "_", " ")) %>%
    mutate(p_adj = if_else(is.na(p_adj), 0, p_adj),
            sig = p_adj > 0) %>%   # define significance (adjust if needed)
    ggplot(aes(x = Ann_Level3, y = feature)) +
    geom_point(aes(size = p_adj,
                    fill = scaled_auc,
                    color = sig),   # map border conditionally
                shape = 21,
                stroke = 0.8) +
...
ggsave("prim_vs_mets_final_plot.png",
       plot = final_plot,
       width = 5,
       height = 10,
       bg = "white")

ggsave("prim_vs_mets_final_plot.pdf",
       plot = final_plot,
       width = 5,
       height = 10,
       bg = "white")
