...
run_degs <- function(sobject,
    ...
    pseudobulk <- sobject %>%
        Seurat::AggregateExpression(
            group.by = c("sample_name", group_by),
            slot = "counts",
            assay = "RNA"
        )
    ...
    degs_output <-
        parallel::mclapply(unique(sobject[[group_by]][[1]]),
    ...
    names(degs_output) <- unique(sobject[[group_by]])[[1]]
    degs_output <- setNames(degs_output, unique(sobject[[group_by]])[[1]])
