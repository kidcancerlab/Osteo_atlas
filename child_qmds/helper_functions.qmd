...
```{r annotate_function}
#| cache.vars = c('annotate_celltypes', 'hpca', 'huim', 'mord', 'moim', 'ref4')
...
run_degs <- function(sobject,
    ...
    degs_output <-
        ...
        #subset here for the specfic celltytpe
        if (subset == TRUE) {
            Idents(sobject) <- comparison_col
            pseudobulk <- sobject %>%
                subset(idents = cluster_no) %>%
                Seurat::AggregateExpression(
                    group.by = c("sample_name", "tissue_type"),
                    slot = "counts",
                    assay = "RNA"
                )
            pseudobulk <- pseudobulk[[1]]
        ...
        if (subset == TRUE) {
            batch_df_new2$group <- "other"
            batch_df_new2$group[batch_df_new2$cluster_no == "Tumor-associated"] <- "target"
            batch_df_new2$group <- factor(batch_df_new2$group)
        }
        ...
        if (subset == TRUE) {
            design <-
                as.formula(
                    paste0(
                        "~",
                        paste(colnames(batch_df_new2)[c(-1, -2, -4)], collapse = " + ")
                    )
                )
        }
