...
DT::datatable(arrange(counts_afterQC, unique, desc(N)))
...
# Split cancer vs the stroma - here {.tabset}
...
for (group in c("patient_prim", "patient_mets", "mm_prim", "mm_mets")) {
    object <-
        qs::qread(str_c("output/seurat_objects/harmony_sobjs/",
                        group,
                        ".qs"))
    #plot_my_stuff(object, what = c("new_annot_clust", "sample_name"))
    Idents(object) <-
    ...
    for (cell_group_name in c("cancer_cells", "normal_cells")) {
        ...
        print(str_c("Optimized resolution for ", group, " ", cell_group_name, " is ", res_normal))
        ...
        tree <- clustree::clustree(clustree_sobj,
                                   prefix = "RNA_snn_res.") + 
            ggtitle(str_c("Clustree for ", group, " ", cell_group_name))
        print(tree)
...
# finally clustering based of the silhouette score and tree and then going to the degs
```{r, fig.width = 10, fig.height = 10, fig.align = "center", echo = FALSE}
object_list <-
...
for (item in seq_len(nrow(umaps_group))) {
    ...
    if (combine == "no") {
        ...
        just_tumor[[sobj_name]] <- 
            plot_my_stuff(tumor,
                          what = c("seurat_clusters", "new_annot_clust")) +
