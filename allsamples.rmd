...
parallel::mclapply(unique(all_samples_csv$unique), function(group) {
    ...
    qs::qsave(merged_sobj, str_c("output/seurat_objects/comb_sobjs/",
                                    ...
                                    ".qs"))
    #run harmony
    if (group == "mm_prim" || group == "mm_mets" || group == "normal_bone") {
        run_harmony <- RunHarmony(object = merged_sobj,
                                  group.by.vars = "sample_name",
                                  theta = 3)
    } else if (group == "xeno_prim_human" || group == "xeno_prim_mouse") {
        run_harmony <- RunHarmony(object = merged_sobj,
                                  group.by.vars =c("sample_name", "method", "data_source", "location"),
                                  theta = c(7, 7, 7, 7))
    } else {
        run_harmony <- RunHarmony(object = merged_sobj,
                                  group.by.vars =c("sample_name", "method", "data_source"),
                                  theta = c(3, 3, 3))
    }
    #optimize resolution, silhouette score does not work for large dataset
    if (ncol(merged_sobj) > 65000) {
        subset_object <- subset(x = merged_sobj,
                                cells = sample(Cells(merged_sobj),
                                        min(65000, length(Cells(merged_sobj)))))
    } else {
        subset_object <- merged_sobj
    }
    ...
    plot_my_stuff(harmony_obj, what = c("sample_name", "method", "data_source"))
...
for (group in unique(all_samples_csv$unique)){
    ...
    harmo_obj <- qs::qread(str_c("output/seurat_objects/harmony_sobjs/",
                                group,
                                ".qs"))
    if (ncol(harmo_obj) > 65000) {
        subset_object <- subset(x = harmo_obj,
                                cells = sample(Cells(harmo_obj),
                                        min(65000, length(Cells(harmo_obj)))))
    } else {
        subset_object <- harmo_obj
    }
    res_score <- optimize_silhouette(sobject = subset_object,
                                    test_res = seq(0.1, 0.9, by = 0.1),
                                    summary_plot = FALSE) %>%
        ...
        slice_head(n=1) %>%
        dplyr::select(res_vals)
    print(str_c("Optimized resolution for ", group, " is ", res_score$res_vals))
...
for (group in unique(all_samples_csv$unique)){
    ...
    for (cluster in unique(ann_sobj$seurat_clusters)) {
        ...
        if (length(unique(subset_object$sample_name)) > 1 && ncol(subset_object) > 50 ) {
            ...
            harm_obj <- subset_object
        }
        harm_obj$re_cluster <- str_c(cluster, ".", harm_obj$seurat_clusters)
...
harmony_params <-
    list(patient_prim = list(group_by = c("sample_name", "method", "data_source"), 
                             theta = c(3, 3, 3)),
         patient_mets = list(c("sample_name", "method", "data_source"), c(3, 3, 3)),
        xeno_prim_human = list(c("sample_name", "method", "data_source", "location"), c(7, 7, 7, 7)),
        xeno_mets_human = list(c("sample_name", "method", "data_source", "location"), c(7, 7, 7, 7)),
        xeno_prim_mouse = list(c("sample_name", "method", "data_source", "location"), c(7, 7, 7, 7)),
        xeno_mets_mouse = list(c("sample_name", "method", "data_source", "location"), c(7, 7, 7, 7)),
        mm_prim = list(c("sample_name", "model", "data_source"), c(7, 7, 7)),
        mm_mets = list(c("sample_name", "model", "data_source"), c(7, 7, 7)))

harmoy_vars <- object$harmony_vars[[1]]
harmony_vars <- "sample_name,method,data_source,location,model"
harmony_vec <- stringr::str_split(harmony_vars, ",")[[1]]
RunHarmony(object = object,
           group.by.vars = object$harmony_vars[[1]] %>% stringr::str_split()[[1]],
           theta = c(7, 7, 7, 7))
...
for (group in unique(all_samples_csv$unique)) {
    object <- qs::qread(str_c("output/seurat_objects/harmony_sobjs/",
                              group,
                              ".qs"))
    Idents(object) <- object$new_annot_clust
    object$cancer_normal <- object$new_annot_clust
    ...
    if (group == "patient_prim" || 
        ...
        if (ncol(normal_obj) > 65000) {
            subset_object <- subset(x = normal_obj,
                                    cells = sample(Cells(normal_obj),
                                            min(65000, length(Cells(normal_obj)))))
        } else {
            subset_object <- normal_obj
        }
        res_normal <- optimize_silhouette(sobject = subset_object,
                                         test_res = seq(0.1, 0.9, by = 0.1),
                                         summary_plot = FALSE) %>%
            filter(sil_vals == max(sil_vals)) %>%
            slice_head(n=1) %>%
        ...
        normal_obj <- normal_obj %>%
            process_seurat(reduction = "harmony",
                           resolution = res_normal$res_vals)
        #save the normal object
        qs::qsave(normal_obj, str_c("output/seurat_objects/tumor_vs_stroma/",
                                    group,
                                    "_normal",
                                    ".qs"))
        #save the normal object metadata with the cell barcodes
        timestamp <- format(Sys.time(), "%Y%m%d_%H%M%S")
        readr::write.tsv(file = normal_obj@meta.data,
                         filename = str_c("output/metadata/"
                                        group,
                                        "_normal",
                                        timestamp,
                                        ".tsv"))
        ...
        cancer_obj <- subset(x = object,
                             idents = "cancer_cells") %>%
            #subset(location == "tibia") %>% 
            process_seurat()
        if (group == "patient_prim" || group == "patient_mets") {
            cancer_obj <- RunHarmony(object = cancer_obj,
                                     group.by.vars = c("sample_name", "method", "data_source"),
                                     theta = c(3, 3, 3))
        } else if (group == "mm_prim"){
            cancer_obj <- RunHarmony(object = cancer_obj,
                                     group.by.vars = c("sample_name", "model", "data_source"),
                                     theta = c(7, 7, 7))
        } else {
            cancer_obj <- RunHarmony(object = cancer_obj,
                                     group.by.vars = c("sample_name", "model"),
        ...
        timestamp <- format(Sys.time(), "%Y%m%d_%H%M%S")
        readr::write.tsv(file = cancer_obj@meta.data,
                         filename = str_c("output/metadata/",
                                        group,
                                        "_cancer",
                                        timestamp,
        ...
        #save the cancer object metadata with the cell barcodes
        timestamp <- format(Sys.time(), "%Y%m%d_%H%M%S")
        readr::write_tsv(file = object@meta.data,
                         filename = str_c("output/metadata/",
                                        group,
                                        "_cancer",
                                        timestamp,
...
# save metadata with the cell barcodes somewhere here
...
for (item in seq_len(nrow(umaps_group))) {
    ...
    if (combine == "no") {
        object <- qs::qread(str_c("output/seurat_objects/annotated_sobjs/",
                            sobj_name,
                            ".qs"))
        annotated[[sobj_name]] <- plot_my_stuff(object, "new_annot_clust")
...
#write a loop to print the seurat clusters and annotations for each obejct and also percentage of celltypes
