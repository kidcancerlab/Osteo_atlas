...
for (i in seq_len(nrow(all_samples_csv))) {
...
for (item in single_sobjs){
    ...
    counts <- Hmeta_data[, .N, by = c("sample_name", "unique")] %>%
        rename(N_cells = N) %>%
        rbind(counts)
}
...
# Run QC and create merged Seurat Object {.tabset}
...
for (group in unique(all_samples_csv$unique)){
    ...
    BI_clusters[[group]] <- plot_my_stuff(merged_sobj, "sample_name")
    BI_method[[group]] <- plot_my_stuff(merged_sobj, "method")
    BI_data_source[[group]] <- plot_my_stuff(merged_sobj, "data_source")

    # non_integrated_dimplot <- DimPlot(merged_sobj,
    #                                 group.by = c("sample_name", 
    #                                             "method",
    #                                             "data_source"),
    #                                 shuffle = TRUE,
    #                                 ncol = 1) +
    #     ggtitle(str_c("Dimplot before harmony ", group))

    qs::qsave(merged_sobj, str_c("output/seurat_objects/comb_sobjs/",
                                    group,
                                    ".qs"))
    before_integration[[group]] <- non_integrated_dimplot

    run_harmony <- harmony::RunHarmony(object = merged_sobj,
                                       group.by.vars = "sample_name")
    harmony_obj <- run_harmony %>% 
        process_seurat(reduction = "harmony")

    # harmony_integrated_dimplot <- DimPlot(merged_sobj,
    #                                     group.by = c("sample_name", 
    #                                                 "method",
    #                                                 "data_source"),
    #                                     shuffle = TRUE,
    #                                     ncol = 1) +
    #     ggtitle(str_c("Dimplot after harmony ", group))

    AI_clusters[[group]] <- plot_my_stuff(harmony_obj, "sample_name")
    AI_method[[group]] <- plot_my_stuff(harmony_obj, "method")
    AI_data_source[[group]] <- plot_my_stuff(harmony_obj, "data_source")

    qs::qsave(harmony_obj, str_c("output/seurat_objects/harmony_sobjs/",
                                    group,
                                    ".qs"))

    after_integration[[group]] <- harmony_integrated_dimplot
}
...
for (group in unique(all_samples_csv$unique)) {
    ...
    if (ncol(object) > 65000) {
        subset_object <- subset(x = object,
                                cells = sample(Cells(object),
                                        min(65000, length(Cells(object)))))
    } else {
        subset_object <- object
    }
    res_score <- optimize_silhouette(sobject = subset_object,
    ...
    res_object <- FindClusters(object,
                              resolution = res_score$res_vals)

    qs::qsave(x = res_object, 
...
for (group in unique(all_samples_csv$unique)) {
    ...
    cluster_celltypes <-
        ...
        slice_head(n=1)
    ann_sobj$new_annot <- ann_sobj$seurat_clusters
    for (item in seq_len(nrow(cluster_celltypes))) {
        ...
        celltype <- cluster_celltypes$Var2[item] %>%
            as.character()
        ann_sobj$new_annot <- str_replace_all(string = ann_sobj$new_annot,
...
# FeaturePlot(ann_sobj, features = proliferation)
...
#     clust_info <- tibble()
#     #for (cluster in ann_sobj$seurat_clusters) {
#     parallel::mclapply(ann_sobj$seurat_clusters, function(cluster) {
#         subset_object <- subset(ann_sobj, ident = cluster)
#         #DimPlot(subset_object)
#         subset_object <- process_seurat(subset_object)
#         harm_obj <- RunHarmony(subset_object,
#                             group.by.vars = "sample_name")
#         harm_obj <- harm_obj %>%
#             process_seurat(reduction = "harmony")
#         res_score <- optimize_silhouette(sobject = harm_obj,
#                                     test_res = seq(0.1, 0.9, by = 0.1),
#                                     summary_plot = FALSE) %>%
#         filter(sil_vals == max(sil_vals)) %>%
#         slice_head(n=1) %>%
#         dplyr::select(res_vals)
#         harm_obj <- harm_obj %>%
#             FindClusters(resolution = res_score)
#         #DimPlot(harm_obj, group.by = c("seurat_clusters", "annotations"))
#         harm_obj$re_cluster <- str_c(cluster, "_", harm_obj$seurat_clusters)
#         clust_info <- harm_obj@meta.data %>%
#             as.data.frame() %>% 
#             select(re_cluster) %>%
#             rbind(clust_info)
#         return(clust_info)
...
# Cancer cells vs normal cells using SCEVAN {.tabset}
Single CEll Variational Aneuploidy aNalysis
When run, the result is in three metadata column:
class: with tumor vs normal celltype information (filtered: low quality cells i think)
...
# Cancer cells vs normal cells using scATOMIC {.tabset}
...
for (group in unique(all_samples_csv$unique)) {
    object <- qs::qread(str_c("output/seurat_objects/harmony_sobjs/",
                              group,
                              ".qs"))
    Idents(object) <- object$new_annot
    object$cancer_normal <- object$new_annot
    for (cell in cancer_celltypes[[group]]) {
        object$cancer_normal <- str_replace_all(string = object$cancer_normal,
                                                pattern = cell,
    ...
    if (group == "patient_prim" || 
        group == "patient_mets" || 
        group == "mm_prim" || 
        group == "mm_mets") {
        normal_cells <- table(object$cancer_normal) %>%
            as.data.frame() %>%
            filter(Var1 != "cancer_cells")
        Idents(object) <- object$cancer_normal
        ...
        qs::qsave(normal_obj, str_c("output/seurat_objects/tumor_vs_stroma/",
                                    group,
                                    "_normal",
                                    ".qs"))
        ...
        qs::qsave(cancer_obj, str_c("output/seurat_objects/tumor_vs_stroma/",  #nolint
                                    ...
                                    "_cancer",
                                    ".qs"))
    } else  if (group == "xeno_mets_human" ||
                group == "xeno_prim_human"){
        qs::qsave(object, str_c("output/seurat_objects/tumor_vs_stroma/",
                                group,
                                "_cancer",
                                ".qs"))
    } else if (
        group == "xeno_mets_mouse" ||
        group == "xeno_prim_mouse"){
        qs::qsave(object, str_c("output/seurat_objects/tumor_vs_stroma/",
                                group,
                                "_normal",
                                ".qs"))
    print(str_c(group, " labelled with cancer cells and subsetted"))
    }
}
...
for (item in seq_len(nrow(umaps_group))) {
    ...
    if (combine == "no") {
        object <- qs::qread(str_c("output/seurat_objects/annotated_sobjs/",
                            sobj_name,
                            ".qs"))
        annotated[[sobj_name]] <- plot_my_stuff(object, "new_annot")

        merged[[sobj_name]] <- DimPlot(object,
                            group.by = "cancer_normal",
                            repel = T,
                            label = T,
                            shuffle= T,
                            cols = c(plot_cols, sample(rainbow(1000)))) +
        ...
        tumor <- qs::qread(str_c("output/seurat_objects/tumor_vs_stroma/",
                            sobj_name,
                            "_cancer.qs"))
        just_tumor[[sobj_name]] <- DimPlot(tumor,
        ...
        stroma <- qs::qread(str_c("output/seurat_objects/tumor_vs_stroma/",
                            sobj_name,
                            "_normal.qs"))
        just_stroma[[sobj_name]] <- DimPlot(stroma,
                            group.by = "cancer_normal",
                            repel = T,
                            label = T,
                            shuffle= T,
        ...
        #                                                                            size = 20,
        #                                                                            face = "bold")))
        # ggsave(filename = str_c("output/plots/umaps/", sobj_name, "_umap.png"))
        #         # width = 7,
        #         # height = 21,
        #         #plot = plot)
    } else {
        first_obj <- qs::qread(str_c("output/seurat_objects/annotated_sobjs/",
                                    sobj_name,
                                    "_human.qs"))
        just_tumor[[sobj_name]] <- DimPlot(first_obj,
                                group.by = "seurat_clusters",
                                repel = T,
                                label = T,
                                shuffle= T,
        ...
        second_obj <- qs::qread(str_c("output/seurat_objects/annotated_sobjs/",
                                    sobj_name,
                                    "_mouse.qs"))
        just_stroma[[sobj_name]] <- DimPlot(second_obj,
                                    group.by = "new",
                                    repel = T,
                                    label = T,
                                    shuffle= T,
...
for (group in unique(all_samples_csv$unique)) {
    ...
    degs_result[[group]] <- run_degs(sobject = object,
                            group_by = "seurat_clusters",
                            prefix = str_c(group, "_cancer"))
    for (item in names(degs_result[[group]])) {
        logfc_cutoff <- 0.6
        degs_result[[group]][[item]]$diffexpressed <- "Not Significant"
        degs_result[[group]][[item]]$diffexpressed[degs_result[[group]][[item]]$log2FoldChange > logfc_cutoff & degs_result[[group]][[item]]$padj < 0.05] <- "Upregulated"
        degs_result[[group]][[item]]$diffexpressed[degs_result[[group]][[item]]$log2FoldChange < -1 * logfc_cutoff & degs_result[[group]][[item]]$padj < 0.05] <- "Downregulated"
#make the volcano plot
        volc_plot[[group]][[item]] <- ggplot(data=degs_result[[group]][[item]],
...
#GSEA analysis for the dotplots
    for (i in seq_len(nrow(cat_tib))){
        ...
        if (subcategory == "NA") {
            gsea_result <- run_gsea(degs_result = degs_result[[group]],
                                    category = category,
                                    species = object$organism[1])
        } else {
            gsea_result <- run_gsea(degs_result = degs_result[[group]],
        ...
        for (item in names(gsea_result)) {
            ...
            if (rows >= 1) {
                ...
                gsea_plot[[group]][[cat_expl]][[item]] <- two_way_dotplot(top5up_down, x_col = "NES")
            } else {
...
#         # ggsave(str_c("output/gsea/tumor/", group, "/", clusters, ".png"),
#         #         width = 15,
#         #         height = 10,
#         #         plot = plots)
...
#volcplot
#datatable
DT::datatable(degs_result$patient_mets$patient_mets_cancer_Cluster_0)
#gsea plots
...
sobject <- qs::qread("output/seurat_objects/tumor_vs_stroma/patient_mets_tumor.qs")
