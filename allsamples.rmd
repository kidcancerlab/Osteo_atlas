...
for (group in unique(all_samples_csv$unique)){
    ...
    qs::qsave(merged_sobj, str_c("output/seurat_objects/comb_sobjs/",
                                    ...
                                    ".qs"))
    #run harmony
    if (group == "mm_prim" || group == "mm_mets") {
...
# Annotate all the datasets {.tabset}
...
for (group in unique(all_samples_csv$unique)) {
    ...
    singler_annotation[[group]] <- plot_my_stuff(ann_sobj, "annotations")
    ...
    anotation_cluster_assignment[[group]] <- plot_my_stuff(ann_sobj, "new_annot_clust")
    feature_plots[[group]] <- FeaturePlot(ann_sobj,
...
for (group in unique(all_samples_csv$unique)) {
    ...
    recur_object <- recurluster(sobj = object,
                                harmony = TRUE,
                                harmony_factors = "sample_name",
                                parallel = TRUE,
                                do_plots = FALSE,
                                max_level = 3)
             # JoinLayers() 
    ...
    ann_sobj <- annotate(sobject = recur_object,
                         species = object$organism[1],
                         aggr_ref = TRUE)

    plot_my_stuff(ann_sobj, what = c("clust_1", "clust_2", "clust_3"))

    cluster_celltypes <-
        table(ann_sobj$clust_1, ann_sobj$annotations) %>% 
        as.data.frame() %>% 
        group_by(Var1) %>%
        arrange(desc(Freq), .by_group = TRUE) %>%
        slice_head(n=1)
    ann_sobj$new_annot_clust <- ann_sobj$clust_1
    for (item in seq_len(nrow(cluster_celltypes))) {
        seurat_clust <- str_c("^", cluster_celltypes$Var1[item], "$") %>%
            as.character()
        celltype <- cluster_celltypes$Var2[item] %>%
...
for (group in unique(all_samples_csv$unique)) {
    ...
    clusters <- c("clust_1", "clust_2")
    for (names in clusters) {
        cluster_celltypes <-
            table(ann_sobj[[names]][[1]], ann_sobj$annotations) %>% 
            as.data.frame() %>% 
            group_by(Var1) %>%
            arrange(desc(Freq), .by_group = TRUE) %>%
            slice_head(n=1)
        new_col <- str_c("new_annot_", names)
        #ann_sobj[[new_col]] <- ann_sobj[[names]][[1]]
        celltype_vect <- cluster_celltypes %>%
            mutate(Var2 = as.character(Var2)) %>%
            pull(Var2, name = Var1)
        ann_sobj[[new_col]] <- str_replace_all(ann_sobj[[names]][[1]], celltype_vect)
    }
    anotation_cluster_assignment[[group]] <- plot_my_stuff(ann_sobj, what = c("new_annot_clust_1", "new_annot_clust_2"))
    feature_plots[[group]] <- FeaturePlot(ann_sobj,
                                          features = c("COL1A1", "COL1A2", "SATB2", "RUNX2"))
}
    ...
    for (i in seq_len(nrow(cat_tib))){
        ...
        for (item in names(gsea_result)) {
            ...
            if (rows >= 1) {
                ...
                gsea_plot[[cat_expl]][[item]] <- ggplot(data = NULL,
                                                        aes(x = as.factor(1),
                                                        y = as.factor(1))) +
