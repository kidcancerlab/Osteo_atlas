...
for (group in unique(all_samples_csv$unique)){
    ...
    non_integrated_dimplot <- DimPlot(merged_sobj,
                                    ...
                                    shuffle = TRUE,
                                    ncol = 1) +
        ggtitle(str_c("Dimplot before harmony int for ", group))
    qs::qsave(merged_sobj, str_c("output/seurat_objects/comb_sobjs/",
                                    group,
                                    ".qs"))
    print(non_integrated_dimplot)
    ...
    harmony_integrated_dimplot <- DimPlot(merged_sobj,
                                        ...
                                        shuffle = TRUE,
                                        ncol = 1) +
        ggtitle(str_c("Dimplot after harmony int for ", group))
    qs::qsave(harmony_obj, str_c("output/seurat_objects/harmony_sobjs/",
                                    group,
                                    ".qs"))
    print(harmony_integrated_dimplot)
}
...
DT::datatable(counts_afterQC)

```


## Clustering and setting the right resolution
...
# cancer_features <- c("COL1A1", "COL1A2", "SATB2", "COL6A1", 
#                     "PRDX2", "PSMD4", "MIF", "COL6A3", "COL3A1")
...
class: with tumor vs normal celltype information
subclone: clones numbered like a seurat clusters
```{r}
SCEVAN_dimplots <- list()
SCEVAN_annot <- list()
...
for (group in unique(all_samples_csv$unique)) {
    object <- qs::qread(str_c("output/seurat_objects/harmony_sobjs/",
                              group,
                              ".qs"))
    if (ncol(object) > 20000) {
        object <- subset(x = object,
                                cells = sample(Cells(object),
                                        min(20000, length(Cells(object)))))
    } else {
        object <- object
    }
    ...
    SCEVAN_dimplots[[group]] <- DimPlot(scv_object,
                                        group.by = c("subclone", "class", "new_annot")) +
        ggtitle(str_c("Dimplot after SCEVAN for ", group))
    SCEVAN_annot[[group]] <- table(scv_object$class)
...
print(SCEVAN_dimplots$patient_prim)
DT::datatable(SCEVAN_annot$patient_prim)

print(SCEVAN_dimplots$patient_mets)
DT::datatable(SCEVAN_annot$patient_mets)
...
The runs are made in subset of 20000 cells for each samples
...
```{r}
# I think this library brings sth into the environment because without doing this
# the run_ATOMIC fucntion wont work

sc_ATOMIC_dimplots <- list()
sc_ATOMIC_annot <- list()
for (group in unique(all_samples_csv$unique)) {
    object <- qs::qread(str_c("output/seurat_objects/harmony_sobjs/",
                              group,
                              ".qs"))
    ...
    sc_ATOMIC_dimplots[[group]] <- DimPlot(scATOMIC_object, 
                                  group.by = c("pan_cancer_cluster", "scATOMIC_pred"),
                                  ncol = 1) +
        ggtitle(str_c("Dimplot after scATOMIC for ", group))
    sc_ATOMIC_annot[[group]] <- table(scATOMIC_results$scATOMIC_pred)
    qs::qsave(scATOMIC_object, str_c("output/scATOMIC/",
                                    group,
                                    ".qs"))
}

print(sc_ATOMIC_dimplots$patient_mets)
DT::datatable(sc_ATOMIC_annot$patient_mets)

print(sc_ATOMIC_dimplots$patient_prim)
DT::datatable(sc_ATOMIC_annot$patient_prim)
```
