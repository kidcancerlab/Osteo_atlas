...
for (group in unique(all_samples_csv$unique)){
    ...
    if (group == "mm_prim" || group == "mm_mets" || group == "normal_bone") {
        run_harmony <- RunHarmony(object = merged_sobj,
                                  ...
                                  theta = 3)
    } else if (group == "patient_prim" || group == "patient_mets") {
        run_harmony <- RunHarmony(object = merged_sobj,
...
for (group in unique(all_samples_csv$unique)){
    ...
    AI_clusters[[group]] <- plot_my_stuff(harmo_obj, what = c("sample_name", "model"))
...
plot_my_stuff(y, what = c("sample_name", "data_source", "method", "location"), ncol = 2)
...
for (group in unique(all_samples_csv$unique)) {
    ...
    singler_annotation[[group]] <- plot_my_stuff(ann_sobj, what = "annotations")
