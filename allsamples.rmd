...
#harmony parameers
harmony_params <-
    list(patient_prim = list(group_by = c("sample_name", "method", "data_source", "model"), 
                             theta = c(3, 3, 3, 3)),
        patient_mets = list(group_by = c("sample_name", "method", "data_source", "model"), 
                            theta = c(3, 3, 3, 3)),
        xeno_prim_human = list(group_by = c("sample_name", "method", "data_source", "CL_or_PDX"),
                               theta = c(7, 7, 7, 7)),
        xeno_mets_human = list(group_by = c("sample_name", "method", "data_source", "CL_or_PDX"),
                               theta = c(3, 3, 3, 3)),
        xeno_prim_mouse = list(group_by = c("sample_name", "method", "data_source", "CL_or_PDX"),
                                theta = c(7, 7, 7, 7)),
        xeno_mets_mouse = list(group_by = c("sample_name", "method", "data_source", "CL_or_PDX"),
                               theta = c(3, 3, 3, 3)),
...
for (group in c("patient_prim", "patient_mets", "mm_prim", "mm_mets")) {
    object <- 
        qs::qread(str_c("output/seurat_objects/harmony_sobjs/",
                        group,
                        ".qs"))
    #plot_my_stuff(object, "cell_group")
    Idents(object) <-
...
for (group in object_list) {
    ...
    for (clusters in unique(object$seurat_clusters)) {
        ...
        cell_types_string <- 
            paste(cell_types_percent$Var1, cell_types_percent$Percentage %>% round(1), sep = ":", collapse = ",")
        med_cellscore_string <- 
            paste(cell_types_percent$Var1, cell_types_percent$med_cellscore, sep = ":", collapse = ",")

        # Append the string to temp_obj metadata
        temp_list[[`clusters`]]$cell_types_string <-
            cell_types_string
        temp_list[[`clusters`]]$med_cellscore_string <-
            med_cellscore_string
    }
